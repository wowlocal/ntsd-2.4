...
Каталог/пул ранее сравнивались отдельно; последующие10 bitmap и соединение
всей инициализации в настоящем `41bc90` остаются следующим шагом.
...
Следом —10 интерфейсных bitmap после пула (первый malloc в `41c2f5`,
последний constructor в `41c55e`, сброс флага в `41c577`), затем соединение
prefix/каталога/пула/bitmap в исходном загрузочном пути с обоснованным сбросом
44d05c. Звуки Object registry при включённом устройстве тоже надо связать с
этим loader: текущий полный catalog corpus использует disabled audio boundary.
