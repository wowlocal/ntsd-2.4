...
Приняты конечные собственные neutral/active19-stage и
[pause/step/resume](APPLICATION_PAUSED_GAMEPLAY.md) последовательности; последний
игровой коммит62565b8. Общие control/contact/hit механизмы реализованы, но полный
бой и матч в приложении не приняты. NTSDApp всё ещё вызывает OriginalMelee practice.

Реализованное без сквозной приёмки: damaging gameplay branches и переходы после
раунда в составе текущего application; shared Core наличие не равно проверке
их целого вызывающего пути. Незавершённого Native-кандидата предыдущей карточки нет.
Native paused2.job.json терминален exit0;9 release-методов прошли,13 файлов
закоммичены/отправлены, delivery1.json подтвердил чистое дерево. Не повторять job.

[Новая карта препятствий первому матчу](FIRST_MATCH_READINESS.md) сверяет текущий
код и сохранённые данные. В обоих active48 защита Actor+8 уменьшается58→10;
все96 contact endpoints ещё защищены, HP500. Это анализ источника, не новая
Native-приёмка. Reader2 завершён; ошибочное имя stage в reader1 сохранено.
Препятствия: ещё не проверен целый damaging caller после окончания защиты,
нет production app session/input/commit adapter, не закрыты raster/audio/device
контракты и фактические Windows/clean-Mac проверки.

NEXT — конечная Native-only диагностика первого повреждения из текущего own
Bootstrap после нейтрального родителя: максимум два сценария обычной атаки и160
новых вызовов на сценарий, без подстановки Actor/HP/позиции/RNG/защиты. До запуска
закрепить входы и план; остановиться на первом полном возврате с повреждением,
точной unsupported зависимости или лимите. Истечение защиты — промежуточная
граница. Результат определяет нужные ветви независимого сравнения; прогноз
Native не объявлять совпадением с оригиналом. Связь со сквозной вехой — перейти
от защищённой демонстрации управления к проверенному удару и полному матчу.
App adapter — готовящаяся независимая ветвь, с явными platform-input зависимостями.

Полный объём игры, первый матч, app/window/input/audio и прежние safety incidents
остаются открытыми. EXE envelope в этой карточке не пересчитывался.
