...
Нативный переход теперь использует общий
[INITIAL_MATCH_ENTRY](INITIAL_MATCH_ENTRY.md), сохраняющий интерфейс, звуковые
ресурсы, команды и результат раунда с откатом окружения при поздней ошибке.
Оба корпуса ниже и дальнейшие music/resource сравнения полностью сохранены.
