...
| R01/R15 | `0x43e9a0`, вход; `0x43ed01`, наблюдённый ret | Верхний диспетчер с загрузкой и развилками | S: читается data/data.txt; вызовы ниже. Связь с обычным боевым тактом требует R01.1 |
| R01/R15 | `0x43eca1 → 0x4151d0`, `0x43ecba → 0x4246b0`, `0x43ecd3 → 0x414b70`, call sites | Вызовы из ветвей верхнего диспетчера | S: адреса подтверждены; полные назначения и путь в бой пока кандидаты |
...
| R04/R17 | `0x419a60`, функция-кандидат | Путь ввода локальных игроков | S: [дизассемблирование](#reproduce); call site `0x41c5e0`; целиком ещё не проверен |
...
| R09/R10/R15/R16 | **Не установлен** | Общие захваты/оружие, регенерация, исход матча, режимы, сетевой протокол | Искать через их поля/вызовы и R01; адрес не угадывается по порядку соседних функций |
...
Условия входа, циклы, альтернативные ветви и пропущенные вызовы — задача R01.1.
...
| `0x41ef42` | `0x42e100`, разрешение | Вызывается для активных объектов |
| `0x41f299` | **Повторный call `0x42e100` в другом участке** | Контекст и применимость открыты; отдельный вызов не воспроизводится |
...
Наличие второго вызова разрешения контактов — установленный факт адресного
поиска; оно **не доказывает** пропущенный второй удар в каждом обычном такте.
Сначала нужно восстановить условие этой ветки.
...
| World `+4`, `+0x194`, `+0x7d4` | Активность слотов, указатели Actor, ссылка на каталог/фоновые данные | Частичная карта; последний указатель и большой объект по нему требуют полного разбора |
...
исходной игры. Размер и аллокацию оригинальных структур нужно устанавливать
по EXE. Нейтральные render-поля ProjectileState — также схема снимка, не оригинальные поля.
