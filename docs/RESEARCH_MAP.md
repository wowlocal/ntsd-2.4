...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, полный enabled catalog, CRT/table, menu/presentation, подготовка/replay init, первая загрузка, ввод/control/replay и исход раунда | Первая загрузка → input/replay → round control до паузного вывода/игрового прохода/menu/эпилога; отдельно CRT/table → menu/tail → подготовка/запись; новая основа ещё вне Practice | D полных записей/масок и порядка вызовов; обе первые цепочки непрерывны, ОС и playback buffers — явные границы | Paused render helpers41d73b, menu4229cc→429730 и игровые проходы41e339; ранний playback prefix, animated loading, остальные экраны, CRT lifetime, типы и нормализация |
