...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, каталог, CRT seed/table, mouse/menu/network, tail/World1→2, пролог/подготовка/replay init и продолжение, Frame/heap, первая загрузка18 звуков | Каталог → World/Actor → CRT/table → mouse/menu/tail/World2 → подготовка → запись; отдельно WAV и initial sound caller; storage вне игрового цикла | D загруженных записей, меню/CRT/presentation/lifecycle, подготовки/replay, WAV/initial sounds против Swift; сырой снимок шести прежних тактов |10 UI bitmap и полный первый loading path со сбросом44d05c, остальные экраны/World2→429730, CRT lifetime, типы и нормализация снимка |
...
| R14 — звук и музыка | Общий WAV helper и18 initial sounds, события, громкость музыки/пяти buffers и shutdown до COM | AVFoundation, частичная презентация; отдельно исходный WAV loader/ownership, initial caller, volume/shutdown | D всех409 WAV и контролей, initial sound passes, событий/volume/shutdown; MMIO/COM границы, сведение не проверено | Соединить enabled Object sound registry с общим loader; остальная громкость/панорама, одновременные события, музыка, задержка и запись оригинального выхода |
| R15 — меню, настройки и режимы | Все5главных пунктов, mouse, общий tail/World1→2, пролог/продолжение подготовки; полный поток экранов открыт | Тренировочное окно; отдельно recovered menu/state/presentation methods, пока не UI приложения | D1020main-menu probes/450mouse messages,1482tail/dispatcher returns; прежний prelude/continuation | Ранние экраны/ресурсы/вывод, World2→429730, выбор персонажей/арены, начало/конец/рестарт, настройки; отдельно Battle, Stage и Tournament с исходными условиями |
| R16 — повторы и сеть | Префикс, восстановление RNG, полный буфер записи43d2c0; сетевой init/address selection/bind/listen/error paths | Фиксированный seed практики; отдельно буфер/free и network-init logic с typed OS inputs | D seed/RNG, целых буферов и network EXE helpers в menu chain; OS transport — граница | Запись/чтение тактов, упаковка/файл, playback; socket lifetime, клиент/соединение, пакеты и синхронизация; реальная сеть/W |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [CRT/table](research/RANDOM_INITIALIZATION.md), [меню/mouse](research/MAIN_MENU.md), [tail/World1→2](research/MENU_PRESENTATION.md), [WAV/initial sounds](research/WAVE_LOADING.md), [пролог](research/MATCH_PRELUDE.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md), [продолжение](research/MATCH_CONTINUATION.md): эти участки перенесены; далее10 UI bitmap и полный loading path, остальные экраны/World2→429730/CRT lifetime, словарь и полный снимок |
