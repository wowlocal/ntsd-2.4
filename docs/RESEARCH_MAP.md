...
следующим исполнять BG `40c160`, затем Stage `40c910` и соединять загрузчики.
...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, родитель каталога, целые объявленные Object, частичный словарь/снимок | Конструкторы/пул/родитель и заголовки/bitmap Object с масками, RNG; новый storage ещё вне игрового цикла | D объявленных областей против Swift; сырой снимок шести случаев; S остальных областей каталога | BG/Stage и соединение каталога, выбранный матч, остальные типы/владельцы, provenance и полная нормализация снимка |
| R03 — загрузка DAT целиком | Режимы декодера, непрерывный поток Object, реестр, секции кадров | Общий Object/Frame loader и decoder; практика ещё отдельно использует импорт | D трёх целых исходных файлов/контроля и прежних секций; реальный CRT остаётся границей | Остальной реестр/зависимости в исходном порядке, настоящий CRT, raw Frame ссылки/маски, соединение с родителем и BG/Stage |
...
| 3 | **R03.1 — внешний загрузчик: в работе** | Объявленная готовая часть R02.1 | [Карточка](research/R03.1.md): целые Naruto/Sasuke/kunai + контроль совпали в явных CRT/device-границах; зависимости, остальной реестр и реальные числовые преобразования открыты |
