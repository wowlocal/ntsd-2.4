...
60 записей. Далее соединять загрузчики и расширять Object до всех 137 записей
реестра в исходном порядке.
...
| R03 — загрузка DAT целиком | Режимы декодера, непрерывные потоки Object/BG/Stage, реестр, секции кадров | Общие Object/Frame/BG/Stage loaders и decoder; практика ещё отдельно использует импорт | D трёх целых Object, всех 17 BG и 25 Stage, контроля и прежних секций; реальный CRT остаётся границей | Остальной реестр/зависимости в исходном порядке, настоящий CRT, raw Frame ссылки/маски, соединение с родителем и BG/Stage |
