...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, полный каталог, CRT seed/table, mouse/menu/network, tail/World1→2, пролог/подготовка/replay init и продолжение, Frame/heap, частичный словарь/снимок | Каталог → World/Actor → CRT/table → mouse/menu/tail/World2 → пролог → подготовка → запись; ret4/ret12; storage ещё вне игрового цикла | D загруженных записей, меню/CRT/presentation/lifecycle, пролога, подготовки, буферов и продолжения против Swift; сырой снимок шести прежних тактов | Остальные экраны/ресурсы и World2→429730, lifetime CRT consumers, типы/владельцы и нормализация снимка |
...
| R14 — звук и музыка | Пути, события, встроенные WAV; громкость музыки/пяти buffers и shutdown до COM | AVFoundation, частичная презентация; отдельно две исходные шкалы громкости и порядок query/get/set/release | D событий/volume/shutdown helper; сведение не проверено | Остальная громкость/панорама, одновременные события, прекращение звуков, музыка, задержка и запись оригинального выхода |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [CRT/table](research/RANDOM_INITIALIZATION.md), [меню/mouse](research/MAIN_MENU.md), [tail/World1→2](research/MENU_PRESENTATION.md), [пролог](research/MATCH_PRELUDE.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md), [продолжение](research/MATCH_CONTINUATION.md): эти участки перенесены; остальные экраны/ресурсы/World2→429730/lifetime CRT, словарь и полный снимок открыты |
