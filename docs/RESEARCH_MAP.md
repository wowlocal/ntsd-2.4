...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, полный каталог, CRT seed/table, пролог/подготовка/replay init и продолжение меню, Frame/heap, частичный словарь/снимок | Каталог → World/Actor → CRT/table → пролог → подготовка → запись; продолжение с полным ret12; storage ещё вне игрового цикла | D загруженных записей, CRT draws/table, пролога, подготовки, буферов и продолжения против Swift; сырой снимок шести прежних тактов | Ранние решения меню и lifetime CRT consumers, остальные типы/владельцы и полная нормализация снимка |
...
| R16 — повторы и сеть | Префикс, восстановление RNG, полное создание буфера записи 43d2c0 | Фиксированный seed практики; отдельно полный буфер начального состояния и free | D seed/RNG и 50 целых буферов в цепочке подготовки | Запись/чтение тактов, упаковка/файл, восстановление и воспроизведение, синхронизация; сеть отдельно |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [CRT/table](research/RANDOM_INITIALIZATION.md), [пролог](research/MATCH_PRELUDE.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md), [продолжение](research/MATCH_CONTINUATION.md): генератор и участки до ret12 перенесены; ранние решения меню/lifetime CRT, оставшийся словарь и полный снимок открыты |
