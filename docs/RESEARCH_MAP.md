...
Далее pause/menu после41d714, ранний playback prefix/camera, файловый
жизненный цикл и остальные основания полного такта. Practice и W ещё открыты.
...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, полный enabled catalog, CRT/table, menu/presentation, подготовка/replay init, первая загрузка и ввод/control/replay tail | Первая загрузка → общий local/remote/playback input/control → replay tail41d714; отдельно CRT/table → menu/tail → подготовка/запись; новая основа ещё вне Practice | D полных записей/масок и порядка вызовов; обе первые цепочки непрерывны, ОС и playback buffers — явные границы | Pause/menu после41d714 к429730; ранний playback prefix, animated loading, остальные экраны, CRT lifetime, типы и нормализация |
