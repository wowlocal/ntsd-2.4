...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, enabled catalog, CRT seed/table, mouse/menu/network, tail/World1→2, подготовка/replay, Frame/heap,18 initial sounds и10 UI bitmap | Каталог → World/Actor → CRT/table → menu/tail/World2 → подготовка → запись; отдельно common sounds, catalog→WAV и пул→UI→44d05c=0; storage вне игрового цикла | D записей/меню/CRT/presentation/lifecycle/replay/WAV/initial UI/enabled catalog против Swift; сырой снимок шести прежних тактов | Полный первый41bc90:18 sounds→enabled catalog→pool→UI; остальные экраны/World2→429730, CRT lifetime, типы и нормализация снимка |
...
| R04 — ввод, комбо и переходы | Local/remote/playback input, буферы, распознавание, часть перехода `mp` | Общий raw input в новой основе; в Practice допуск одной техники | D606 local/1452 received cases; ограниченные комбо/переходы | Соединить input с полным тактом; общий переход по полям DAT, стоимость, знаковые переходы и исходные приоритеты без допуска по имени персонажа |
...
| R14 — звук и музыка | Общий WAV helper,18 initial sounds, enabled Frame/weapon registry, volume/shutdown до COM | AVFoundation, частичная презентация; отдельно общий WAV loader/ownership, catalog sound loading/SetVolume и initial caller | D всех409 WAV, initial sounds,429 real catalog WAV calls, volume/shutdown; MMIO/COM границы, сведение не проверено | Полный first load; caller fault traces, остальная громкость/панорама, одновременные события, музыка, задержка и запись оригинального выхода |
...
| R16 — повторы и сеть | Префикс/RNG, буфер записи43d2c0, remote/playback input и caller; сетевой init/address selection/bind/listen/error paths | Фиксированный seed практики; отдельно буфер/free, декодирование команд и network-init logic с typed OS inputs | D seed/RNG, буферов,1452 received-input случаев и menu network helpers; OS transport — граница | Источник playback packet43dc50, checksum/recording41d4b7..41d714, упаковка/файл; phase0 socket/hotkeys, соединение/синхронизация и реальная сеть/W |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [CRT/table](research/RANDOM_INITIALIZATION.md), [меню/mouse](research/MAIN_MENU.md), [tail/World1→2](research/MENU_PRESENTATION.md), [WAV](research/WAVE_LOADING.md), [initial UI](research/INITIAL_INTERFACE.md), [enabled catalog sounds](research/CATALOG_SOUNDS.md), [пролог](research/MATCH_PRELUDE.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md), [продолжение](research/MATCH_CONTINUATION.md): [первый41bc90](research/INITIAL_LOADING.md): [локальный ввод](research/LOCAL_INPUT.md): [полученные команды](research/RECEIVED_INPUT.md): далее41c5e5/network/hotkeys, playback source/checksum/recording и pause/menu→429730, animated loading, остальные экраны/CRT lifetime, словарь и полный снимок |
