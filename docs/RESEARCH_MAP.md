...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, полный enabled catalog, CRT/table, menu/presentation, preparation/replay, round control и собственный запуск через contacts/hits/items | Собственная цепочка загрузка→меню→Naruto/Sasuke/District→Start→music/replay→новый игровой вход→control/physics/links/contacts/hits/items до41f2ac; новая основа ещё вне Practice | D полных записей/масок/вызовов и изменяемого Frame heap, две собственные цепочки без подстановки ожидаемого состояния; ОС/CPU — явные границы | На собственном41f2ac: cpoint418c30/4187b0, остальные связи/вывод/планировщик и полный такт; paused render, ранний playback, animated loading, остальные экраны/режимы, CRT lifetime/Windows |
...
| R09 — захваты, оружие, связанные объекты | Целая417f80 и hit-side catch/pickup/force42e100; последующие cpoint/link passes открыты | [OriginalWorldLinks](research/WORLD_LINKS.md) читает общие wpoint/центры/ссылки; [OriginalActorHits](research/WORLD_HITS.md) создаёт/меняет связи из ITR | D3018 WorldLinks и7845 WorldHits synthetic cases/полные pool masks/RNG; собственная цепочка до41f2ac, survey исходных10041 Frame отдельно | Непрерывные DAT-связки,41 out-of-array weaponact/достижимость, cpoint418c30/4187b0, остальные link helpers и второй417f80 в полном такте |
