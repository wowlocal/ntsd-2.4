...
не запускались для Swift. Новые трассы R01.1 выполняют исходный EXE, без
сравнения широкого пути с Swift. Корпуса пересекаются: числа **не суммируются**.
...
| 2 | **R02.1 — состояние такта: следующая** | Карта R01.1 | [Подготовленная карточка](research/R02.1.md): словарь World/Actor/Object/Frame и globals, инициализация, полный снимок; исходная аллокация Actor 0x420 установлена, stride 0x500 остаётся адресацией стенда |
...
R01.1 расширила знания об оригинале и выявила пропущенные стадии; новый корпус
не переносит их автоматически. Следующий технический результат — R02.1.
