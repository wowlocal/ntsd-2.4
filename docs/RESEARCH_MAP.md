...
**Ближайший шаг: закончить оба собственных продолжения
[таблицы результатов](research/RESULT_LAYOUT.md), затем соединить собственный
[вывод и возврат](research/GAMEPLAY_OUTPUT.md) до `422ab8/ret4`.**

Контролируемый вывод `422994` теперь соединяет mode label, сообщения/громкость,
present и включённый sound drain в исходном порядке. Все170 вызовов совпали
с Native по globals и119700 событиям; исходный пролог и настоящий `ret4`
проверяют сохранённые регистры, SEH и нормальные cookie checks. Поздняя ошибка
звукового запроса откатывает весь нативный вывод. Это отдельный контролируемый
контекст после пролога, ещё не возврат собственного инициализированного такта.
Raw и packaged проверки прошли;183 прежних fixtures сохранены, добавлен один.
Полный такт, приложение, законченный матч и Windows остаются открытыми.
