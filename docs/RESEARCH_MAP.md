...
с отдельной загрузкой/выгрузкой слоёв; следующим исполнять Stage `40c910`
и соединять загрузчики.
...
349 групп кадров остаются за границей **изолированной** проверки загрузчика: 290 — переполнение
...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, родитель каталога, целые объявленные Object и все исходные BG, частичный словарь/снимок | Конструкторы/пул/родитель, заголовки/bitmap Object и BG/layer lifecycle с масками, RNG; новый storage ещё вне игрового цикла | D объявленных областей против Swift; сырой снимок шести случаев; S остальных областей каталога | Stage и соединение каталога, выбранный матч, остальные типы/владельцы, provenance и полная нормализация снимка |
| R03 — загрузка DAT целиком | Режимы декодера, непрерывные потоки Object/BG, реестр, секции кадров | Общие Object/Frame/BG loaders и decoder; практика ещё отдельно использует импорт | D трёх целых Object, всех 17 BG, контроля и прежних секций; реальный CRT остаётся границей | Остальной реестр/зависимости в исходном порядке, настоящий CRT, raw Frame ссылки/маски, соединение с родителем и BG/Stage |
...
| 3 | **R03.1 — внешний загрузчик: в работе** | Объявленная готовая часть R02.1 | [Карточка](research/R03.1.md): целые Naruto/Sasuke/kunai + контроль совпали в явных CRT/device-границах; зависимости, остальной реестр и реальные числовые преобразования открыты; все исходные BG также перенесены отдельно |
