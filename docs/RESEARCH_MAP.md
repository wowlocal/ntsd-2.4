...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, полный enabled catalog, CRT/table, menu/presentation, подготовка/replay init, первая загрузка и ввод/control | Первая загрузка → общий local/remote/playback input/control; отдельно CRT/table → menu/tail → подготовка/запись; новая основа ещё вне Practice | D полных записей/масок и порядка вызовов; обе первые input/control цепочки непрерывны, ОС — явная граница | Playback source/checksum/recording → pause/menu429730; animated loading, остальные экраны, CRT lifetime, типы и нормализация |
...
| R16 — повторы и сеть | Префикс/RNG, буфер записи43d2c0, remote/playback input и caller, phase0 packet exchange/checks; сетевой init/address selection/bind/listen/error paths | Фиксированный seed практики; отдельно буфер/free, декодирование команд, общие hotkeys/error shutdown и network logic с typed OS inputs | D seed/RNG, буферов,1452 received/5986 control случаев и menu network helpers; OS transport — граница | Источник playback packet43dc50, checksum/recording41d4b7..41d714, упаковка/файл; соединение/синхронизация и реальная сеть/W |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [CRT/table](research/RANDOM_INITIALIZATION.md), [меню/mouse](research/MAIN_MENU.md), [tail/World1→2](research/MENU_PRESENTATION.md), [WAV](research/WAVE_LOADING.md), [initial UI](research/INITIAL_INTERFACE.md), [enabled catalog sounds](research/CATALOG_SOUNDS.md), [пролог](research/MATCH_PRELUDE.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md), [продолжение](research/MATCH_CONTINUATION.md): [первый41bc90](research/INITIAL_LOADING.md): [локальный ввод](research/LOCAL_INPUT.md): [полученные команды](research/RECEIVED_INPUT.md): [hotkeys/обмен](research/INPUT_CONTROL.md): далее playback source/checksum/recording и pause/menu→429730, animated loading, остальные экраны/CRT lifetime, словарь и полный снимок |
