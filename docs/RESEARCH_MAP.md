...
включая сброс счётчика RNG перед дальнейшим запуском. Далее — происхождение
входов, пролог режима, продолжение после `42d704`, оставшиеся типы и широкий такт.
...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, полный каталог, подготовка матча и replay init, Frame/heap, частичный словарь/снимок | Каталог → World/Actor → подготовка → запись начального состояния повтора; новый storage ещё вне игрового цикла | D загруженных записей, подготовки и 50 полных буферов против Swift; сырой снимок шести прежних тактов | Menu/RNG provenance, пролог и продолжение после 42d704, остальные типы/владельцы и полная нормализация снимка |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md): цепочка до 42d704 перенесена; происхождение menu/RNG, пролог, продолжение запуска, оставшийся словарь и полный снимок открыты |
