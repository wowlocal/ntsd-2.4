...
`450c2c==1`, команды 1–5 и полные записи/маски. Далее — происхождение ранних
входов меню/RNG, оставшиеся типы и широкий такт.
...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, полный каталог, CRT seed/table, mouse/main-menu/network init, пролог/подготовка/replay init и продолжение меню, Frame/heap, частичный словарь/снимок | Каталог → World/Actor → CRT/table → mouse/menu → пролог → подготовка → запись; продолжение с полным ret12; storage ещё вне игрового цикла | D загруженных записей, mouse/menu/network/CRT, пролога, подготовки, буферов и продолжения против Swift; сырой снимок шести прежних тактов | Shared menu tail, остальные решения/lifetime CRT consumers, типы/владельцы и нормализация снимка |
...
| R15 — меню, настройки и режимы | Все5главных пунктов, mouse events, пролог/продолжение подготовки; полный поток экранов открыт | Тренировочное окно; отдельно recovered menu/state methods, пока не UI приложения | D1020main-menu probes,450mouse messages; прежний prelude/continuation | Shared tail, ресурсы/вывод, выбор персонажей/арены, начало/конец/рестарт, настройки; отдельно Battle, Stage и Tournament с исходными условиями |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [CRT/table](research/RANDOM_INITIALIZATION.md), [главное меню/mouse](research/MAIN_MENU.md), [пролог](research/MATCH_PRELUDE.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md), [продолжение](research/MATCH_CONTINUATION.md): эти участки перенесены; shared tail/остальные решения меню/lifetime CRT, словарь и полный снимок открыты |
