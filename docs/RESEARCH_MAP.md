...
| R15 — меню, настройки и режимы | Все5главных пунктов, mouse, tail/World1→2, подготовка,10 initial UI resources; полный поток экранов открыт | Тренировочное окно; отдельно menu/state/presentation и initial UI lifecycle, пока не UI приложения | D1020main-menu probes/450mouse messages,1482tail/dispatcher returns,13initial UI passes; prelude/continuation | Полный loading path, ранние экраны/ресурсы/вывод, World2→429730, выбор персонажей/арены, начало/конец/рестарт, настройки; Battle, Stage и Tournament |
