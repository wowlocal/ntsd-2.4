...
заданы явно. Далее — их происхождение, пролог режима, создание повтора и
продолжение запуска, оставшиеся типы и обоснованное состояние широкого такта.
...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, полный каталог, общая подготовка матча, Frame/heap, частичный словарь/снимок | Каталог → World/Actor → общая подготовка, RNG; новый storage ещё вне игрового цикла | D загруженных записей/масок и 50 подготовок против Swift; сырой снимок шести прежних тактов | Menu/RNG provenance, пролог/продолжение запуска и replay init, остальные типы/владельцы и полная нормализация снимка |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [подготовка](research/MATCH_PREPARATION.md): общий участок с загруженным каталогом перенесён; происхождение menu/RNG, пролог, replay init/продолжение запуска, оставшийся словарь и полный снимок открыты |
