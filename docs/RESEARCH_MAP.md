...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, полный enabled catalog, CRT/table, menu/presentation, preparation/replay, round control и собственный запуск через hits/items/cpoints | Собственная цепочка загрузка→меню→Naruto/Sasuke/District→Start→music/replay→новый игровой вход→control/physics/links/contacts/hits/cpoints/camera/background до41f4ac с полным World draw; новая основа ещё вне Practice | D полных записей/масок/вызовов и изменяемого Frame heap, две собственные цепочки без подстановки ожидаемого состояния; ОС/CPU — явные границы | На собственном41f4ac: mode branches, CRT/GDI,4196f0, полный post-draw loop/планировщик/восстановление и полный такт; paused render, ранний playback, animated loading, остальные экраны/режимы, CRT lifetime/Windows |
...
| R06 — реестр объектов, создание и удаление | Общий каталог/400-slot pool, constructor и часть opoint; physics spawn998 и весь item caller41ef55..41f276 | Реестр из DAT, общий physics lifecycle и создание998; item RNG146..154/candidates/reconstruction; прежние голоса/змея/иглы остаются срезами | D loaders, preparation, [World physics](research/WORLD_PHYSICS.md) и150 whole hit/item caller cases в [WORLD_HITS](research/WORLD_HITS.md), включая reuse/alias/порядок слотов | Общие opoint, full-pool retained slot provenance, владельцы/команды и все виды создания/удаления на собственном полном такте |
...
| R10 — HP/MP, появление, смерть, исход боя | Общие стоимости control, physics death/respawn/reversion; подсчёт команд/Stage и таймеры исхода | Общий World physics lifecycle и round control с восстановлением/меню; полный матч ещё открыт | D control/physics и4 148 round случаев, включая275 последовательных проходов до350 на корпус; source fault без союзников отдельно | Регенерация/зарядка, полный матч и Stage progression, результаты/награды; оставшиеся стадии общего игрового цикла |
