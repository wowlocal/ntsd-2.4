...
**Ближайший шаг: HUD с `421a15`, затем хвост и возврат полного такта.**
[Команды и восстановление](research/POSTDRAW_COMMANDS.md) реализуют весь
`4214d5..421a15`: предметы, ресурсы, два таймера лечения и очистку слотов.
3 898 исходных случаев совпали по полному пулу/маскам/globals, сохранённому
слоту и 3 252 событиям; выполнены 318 из 327 инструкций тела.
[Оба собственных запуска](research/GAMEPLAY_COMMANDS.md) теперь продолжаются
до `421a15/SP1000e9bc`: каждый совпал по 417 293 записям,
736 179 550 байтам/маскам, 503 helper-вызовам и 57 снимкам;
1 593 FPU-наблюдения сохраняют 53-битную точность. Этот первый проход
не меняет уже очищенное состояние и не читает сохранённый SP+34.
Далее сброс командных флагов и весь HUD `41ae60..41b12d`, затем
диагностика/результаты/эпилог. По статическому коду аргумент HUD не читается;
отрисовка использует global455608. Полный такт, приложение, законченный
матч и Windows-проверка остаются открытыми.
...
Соединённый первый такт теперь дошёл до421a15. Префикс нельзя выносить
...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, каталог, CRT/table, меню, запуск, весь post-draw loop/commands/recovery и startup precision53 | Собственная инициализированная цепочка через post-draw/recovery до421a15 при53 битах; Practice отдельно | D полных записей/масок/вызовов; повторные53-bit проверки, исходные DAT,5432 lifecycle/3898 commands calls и оба [initialized consumer](research/GAMEPLAY_COMMANDS.md) | HUD/результаты и возврат полного такта; Windows/device/матч и остальные режимы |
...
| R05 — состояния, движение, планировщик | Целые413080/40e490/40d960, physics caller и полный post-draw41f550..4214cf | Общие управление/физика/планировщик и live-slot lifecycle; ресурсный проход и собственный World до421a15 | D55 433 [Actor physics](research/ACTOR_PHYSICS.md),515 World physics,6084 [scheduler](research/ACTOR_SCHEDULER.md),5432 [lifecycle](research/POSTDRAW_LIFECYCLE.md) calls и оба initialized consumer | Остаток такта с421a15, непрерывные DAT-последовательности, Windows и числовые границы |
