...
[UTM reference VM configured; Windows ISO download live](research/WINDOWS_UTM_REFERENCE_SETUP.md):
...
[Color diagnosis localizes a readback-profile discrepancy](research/APPLICATION_MAC_DISPLAY_COLOR_DIAGNOSIS.md):
...
[Physical macOS window candidate](research/APPLICATION_MAC_WINDOW.md): AppKit service now
...
[Resumable startup candidate](research/APPLICATION_OBSERVED_STARTUP.md): one typed exchange
...
оригинала. NEXT — независимое сравнение целого достигнутого damaging caller,
его lifecycle/graphics/sound/outer return; недостающие source inputs определить
по сохранённым доказательствам до нового исполнения. Production app adapter,
raster/audio/Windows/device/clean-Mac остаются открытыми.
Краткая передача — [CURRENT_WORK](research/CURRENT_WORK.md), исходная
[карта препятствий](research/FIRST_MATCH_READINESS.md) сохраняется.
...
**Порядок новой работы:** [WORKFLOW](research/WORKFLOW.md).
**Актуальная передача и первая карточка следующего goal:**
[CURRENT_WORK](research/CURRENT_WORK.md). Она учитывает сохранённые результаты и
открытые отказы Codex; исторические «далее» ниже не являются командами перезапуска.
...
Отдельно проверены [gameplay providers](research/APPLICATION_LOADED_GAMEPLAY_PROVIDERS.md):
...
16 последующих вызовов по [плану](research/APPLICATION_LOADED_GAMEPLAY_COMPARISON_PLAN.md).
...
Проверен [ограниченный scalar comparator gameplay](research/APPLICATION_GAMEPLAY_SCALAR_PROJECTION.md):
...
Принято [сравнение active camera/drawing/impulses](research/APPLICATION_ACTIVE_GRAPHICS.md):288 source и288
...
[сравнение active notices](research/APPLICATION_ACTIVE_NOTICES.md):96 source и96 собственных
...
[сравнение active layout](research/APPLICATION_ACTIVE_LAYOUT.md):96 source и96 собственных
...
[Evidence](evidence/application-active-layout.json), [план](research/APPLICATION_ACTIVE_LAYOUT_PLAN.md).
Принято [сравнение active output/return](research/APPLICATION_ACTIVE_OUTPUT.md):96 source
...
680 промежуточных числовых точек; перед Random арены сохраняется RNG выбора
музыки. Исходные6596 точек/7125784 записи/40878800788 байт и маски сверены;
295 из330 инструкций подготовки не означают все исходы ветвей.

Сохранены ошибка резерва стенда и нормальный исходный возврат с ареной9 вместо
ошибочно ожидаемой0. Этот случай не объявлен совпадением.234 завершённых вызова
фиксированных арен использованы побайтно, выполнены только22 новых Random-
вызова с учётом предшествующей музыки.40 старых родительских вызовов и десять
вызовов префикса воспроизведены; исходные процессы завершены и не перезапускаются.
Сырые256 случаев и шесть новых откатов прошли148.773с; прежние22/12 откатов —
17.200с. Заключительные два теста из встроенных файлов прошли206.977с. Семь
транспортов воспроизводят весь корпус и6140 блоков.318 прежних эталонов
неизменны,325 текущих/759 файлов пакета и архива проверены;23 посторонних файла
сохранены. Далее обычные ошибки ресурсов,43a860 War gameplay, собственная
инициализация/внешний цикл/приложение. Полный матч, вся игра, сеть, окно,
управление, звук, Windows и чистая macOS остаются открыты.

[Предварительная подготовка War](research/LIB_WAR_PREPARATION_NATIVE_PREFLIGHT.md)
совпала в22 вызовах:20 возвратов War/22 внешних, две полные подготовки,
16 участников,20 bitmap арены и два полных повтора. Четыре прежних родительских
вызова воспроизведены. Исходные444 точки/475960 записей/1359294944 байта и маски
сверены;291 из330 инструкций подготовки не означают все исходы ветвей.
Прежний корпус с шестью неизвестными входами записи и450be4 сохранён отдельно.
Новый корпус явно задаёт эти обычные параметры перед третьим вызовом каждой
цепочки; происхождение собственной полной инициализации этим не восстановлено.

Сырые22 вызова и12 поздних откатов прошли за17.241с; упакованные три теста —
438.190с, включая прежние574 War и210 Team. Сохранены ошибка Swift-замыкания
и уточнение музыкального отката до фактического RenderFile; исходные ожидания
и игровые правила не менялись. Два транспорта побайтно воспроизводят корпус
и1032 блока.316 прежних эталонов неизменны,318 текущих/751 файл пакета и архив
проверены;23 посторонних файла сохранены. Все процессы завершены. Окно/устройства
не проверены: CUA native pipe недоступен. Далее конечная [серия256 вызовов/
56 подготовок](research/LIB_WAR_PREPARATION_MATRIX_PLAN.md), затем ошибки ресурсов,
War gameplay и собственная инициализация/внешний цикл/приложение. Полная цель открыта.

[Настройка War](research/LIB_WAR_SETUP.md) принята в конечной области проверки:
806 исходных/нативных вызовов,805 внешних возвратов/801 возврат War и граница
перед43a21f. Дополнения воспроизводят четыре прежних родительских вызова.
Аудит574 случаев обнаружил только7 изменённых ячеек: один игрок передаёт лишь
первое действие. Отдельные192 вызова проверили все44 ячейки, ещё40 — все14
сочетаний Attack/Jump/Defense через трёх людей. Исходный корпус и запись пробела
сохранены. Проверены30 сочетаний пресетов,28 живых списков Random, порядок
вывода/звука и владельцы ресурсов. Все13104 точки и маски сверены;4003 исходных
адреса не означают все исходы ветвей.18 поздних откатов прошли.
Сырые7 тестов374.882с и упакованные7 тестов473.097с прошли;295 прежних эталонов
неизменны,316 текущих и745 файлов пакета/Git/архива проверены.23 посторонних
файла сохранены. Все текущие процессы завершены; исторический результат Swift
при исчезновении X5 остаётся неизвестным. NTSDNative связан, окна/устройства
этим не проверены. Далее43a21f подготовка боя и43a860 War, собственная полная
инициализация/внешний цикл/приложение. Матч, вся игра, сеть, Windows и чистая
macOS остаются открыты. [История](research/LIB_WAR_SETUP_STATUS.md) сохраняет
ошибки и ограничения; полная цель активна.

[Подготовка Team Tournament](research/LIB_TEAM_TOURNAMENT_PREPARATION.md) совпадает
во всех210 полных возвратах:154 меню и56 подготовок. Проверены22 исходных
конструкторных родителя, имена повторов,300 инициализаций Actor,302 пары
Release/free,649 загрузок bitmap и56 полных записей повтора. Повторные
контролируемые подтверждения сохраняют назначения CPU:22 из них дают6/8 активных
Actor при четырёх участниках; обычный результат боя очищает назначения.
Все точки, чтения, записи, маски и210 атомарных случаев сверены. Сырые8 тестов
с прежними377 Team bracket/51 Arena release и упакованные3 теста прошли;
шесть поздних откатов и явное отсутствие владельца освобождения проверены.
Ошибки ожидавшего процесса и двух предположений аудитора сохранены; исходные
результаты и нативные правила для принятия не менялись.290 прежних эталонов
неизменны;295 эталонов и715 файлов пакета проверены. Каталог5 завершился на
неизменённом резерве6 ГБ после28 объектов; полного возврата нет. Все48855 частей
трассы и23754 блока данных целы; нового нативного сравнения28 объектов нет.
Три прежних архива сохранены побайтно на X5. Далее War, собственная
инициализация, внешний цикл и приложение. Матч/Windows/устройства/чистая macOS открыты.

[Сетка Team Tournament](research/LIB_TEAM_TOURNAMENT_BRACKET.md) совпадает
в377 вызовах:375 полных возвратов и две границы перед436747 после расстановки
компьютеров. Проверены51 расчёт боя пар,102 обновления здоровья, четыре места
участников, отмена, удержание Attack, три результата и экран победителей.
Геометрия восстановлена как126 нативных констант. Все9062 точки и377 атомарных
случаев сверены. Сырые6 тестов включают прежние373 Team setup и250 Tournament;
упакованные2 теста и четыре поздних отката прошли. Исправлен адрес отдельной
проверки подписей в аудиторе; ожидания и игровые правила сохранены.
Все707 файлов пакета сверены;285 прежних эталонов неизменны,290 с новым корпусом.
Каталог5 продолжает тот же процесс, последнее наблюдение25 объектов.
Далее полная подготовка Team от436747, War, собственная инициализация, внешний
цикл и приложение. Полный матч, Windows, устройства и чистая macOS открыты.

[Настройка Team Tournament](research/LIB_TEAM_TOURNAMENT_SETUP.md) совпадает
в373 вызовах:372 полных возврата и граница Start перед435a58. Проверены четыре
пары,1100 перестановок обоих бойцов и номеров команд,56 списков Random,
перенумерация игроков, настройки и порядок вывода. Аудит6853 точек и373
промежуточных случаев прошёл. Сырые8 тестов включают прежние Tournament/Stage/VS;
упакованные2 теста и поздние откаты прошли. Все699 файлов сверены,281 прежний
эталон неизменен;285 с новым корпусом. Каталог5 продолжает тот же процесс,
последнее наблюдение23 объектов. Далее сетка Team, назначение четырёх участников,
подготовка/результаты, War и собственная инициализация/внешний цикл/приложение.
Полный матч, окно, устройства, Windows и чистая macOS остаются открытыми.

[Освобождение арен в Tournament](research/LIB_TOURNAMENT_ARENA_RELEASE.md)
совпадает в51 полной подготовке: все17 арен проходят загрузку, освобождение при
выборе99 и повторную загрузку. Проверены302 пары Release/free,604 конструктора
bitmap и51 полный буфер повтора; старые указатели не освобождаются повторно.
Все17 первых подготовок воспроизводят прежние результаты без изменения байтов.
Сырые6 тестов и упакованные3 прошли, включая прежние98 вызовов и поздние откаты.
693 файла пакета сверены;278 прежних эталонов неизменны,281 с новым корпусом.
Это контролируемые границы меню, без промежуточных сыгранных боёв. Отдельный
каталог5 продолжает тот же процесс: его20 объектов и все32258 упакованных частей
трасс побайтно воспроизводят принятый снимок20, кроме имён каталогов. Нового
нативного запуска каталога или его полного возврата этим не заявляется.
Далее Team Tournament/War, собственная библиотечная инициализация, внешний цикл
и приложение; полный матч, окно, устройства, Windows и чистая macOS открыты.

[Подготовка Tournament](research/LIB_TOURNAMENT_PREPARATION.md) совпадает в98
вызовах:77 вызовов меню и21 полная подготовка участников, арены и повтора с
настоящими ret12/ret4. Все8421 результата конструкторов имеют восстановленное
происхождение;21 буфер содержит136325112 байтов. Проверены332 загрузки слоёв,
2409 полных точек и22508 возвратов помощников. Неизвестное поле музыки сначала
было отклонено, затем его инициализация восстановлена из PE в новом корпусе;
все прежние фактические результаты сохранены. Сырые3 и упакованные3 теста прошли, включая
поздние откаты;687 файлов пакета сверены.275 прежних эталонов неизменны; с новым корпусом их278. Непустое
освобождение старой арены требует продолжения и пока не сравнивалось внутри
всего этого вызывающего кода. Затем нужны Team Tournament, War, собственный
каталог и приложение. Это подготовка боя, без заявления о сыгранном матче,
окне, Windows, устройствах или завершении полной цели.

[Турнирная сетка](research/LIB_TOURNAMENT_BRACKET.md) совпадает в250 вызовах:
249 настоящих возвратов и одна граница перед подготовкой боя434349. Проверены
расчёт компьютерных боёв, здоровье и уровни сетки, места управления с отменой,
приём результата, музыка и экран победителя. Все249 случаев первого полного
корпуса воспроизведены неизменными; добавлен настоящий случай удержания Attack.
Аудит проверяет5127 точек и11201 возврат помощников. Сырые2 теста и прежние373
настройки прошли на том же Core. Упакованные2 теста также прошли;680 файлов
пакета и271 прежний эталон сохранены,275 файлов с новым корпусом. Следующий шаг —
турнирная подготовка Actor/арены/повтора, затем Team Tournament и War. Каталог5
продолжает прежний процесс, последнее наблюдение16 объектов. Полный матч,
подключение движка к окну, Windows и устройства остаются открытыми.

[Настройка Tournament](research/LIB_TOURNAMENT_SETUP.md) совпадает в373 вызовах:
372 настоящих возврата и одна граница перед турнирной сеткой. Это отдельное
меню20/432ab0. Проверены выбор восьми участников, нумерация людей, возвраты,
жеребьёвка, повторный Random/выбор и настройки.42 маленьких портрета получены из
исходных DAT/BMP; короткая подпись сохраняет настоящий NUL из EXE. Все6107 точек,
56 списков Random и1100 перестановок проверены независимо. Ошибки синтаксиса
первой сборки и счёта арен в аудиторе сохранены; ожидания и игровые правила
не менялись. Сырые6 и упакованные2 теста прошли, включая Stage/VS и оба отката.
Все674 файла пакета и267 прежних эталонов сверены;271 файл с новым корпусом.
Каталог5 продолжает тот же процесс, последнее наблюдение12 Objects. Следом —
турнирная сетка/подготовка, Team Tournament и War, затем собственная библиотечная
цепочка, внешний цикл и окно. Полная цель, Windows и устройства ещё открыты.

[Полное меню Stage](research/LIB_STAGE_MODE.md) совпадает в279 вызовах:
277 настоящих возвратов и две границы Start. Проверены ноль/два компьютера,
возвраты к выбору, шесть этапов, музыка, сложность и повторный выбор.44 списка
Random и130 локальных строк проверены по реальным записям оригинала; неизвестный
стек не импортируется. Сырые6 и упакованные2 теста прошли, включая прежние VS
и поздние откаты. Ошибка индекса аргумента в первом аудиторе сохранена; исходные
ожидания и игровые правила не менялись. Все668 файлов пакета/263 старых эталона
сверены,267 файлов с новым корпусом. Разработка использует X5; приложение его
не требует. Каталог5 продолжает тот же запуск, последнее наблюдение9 Objects.
Далее режимы2/3/4 и собственное соединение библиотеки/подготовки/цикла/окна;
полный матч, Windows, устройства, весь контент и чистая macOS остаются открыты.
...
Подробный порядок, конечный пакет доказательств, независимый review, критерии
допуска и обработка safety refusal заданы в [WORKFLOW](research/WORKFLOW.md).
Следующие шаги описывают зависимости работы, а не обязанность заново исполнить
оригинал при наличии подходящего неизменяемого захвата.
...
3. Использовать существующее доказательство; при необходимом новом исследовании
   выполнить оригинальный участок по конечному плану на воспроизводимых входах.
   Описать все подменённые функции и различия начального состояния с матчем.
   Завершённые захваты и заблокированные операции автоматически не повторять.
...
6. Запустить подходящие регрессии и независимый review контракта. Публиковать новые
   fixtures/evidence после сравнения и проверки сохранности; старые expected,
   masks и отказы не переписывать. Самопроверка Swift не заменяет эталон.
7. Обновить карточку, эту карту и адресный справочник; записать ограничения,
   ...
   В CURRENT_WORK оставить короткую передачу; полную историю вести в исследованиях.
