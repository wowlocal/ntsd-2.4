...
непрозрачными дочерними загрузчиками; следующим исполнять Object `40ef70`.
...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, загрузочный пул, родитель каталога, частичный словарь/снимок | Конструкторы/пул и записи родителя с масками/ссылками по индексам, RNG; новый storage ещё вне игрового цикла | D конструкторов/пула/родителя против Swift; сырой снимок шести случаев; S остальных областей каталога | Дочерние загрузчики каталога, инициализация выбранного матча, остальные типы/владельцы, provenance и полная нормализация снимка |
| R03 — загрузка DAT целиком | Декодер, секции кадров, часть заголовков, порядок запросов родителя | Секции кадров и родительский реестр; предварительный импорт | D секций/реестра; заголовки игрового стенда ещё задаются вручную | Исполнить дочерние загрузчики и соединить их с родителем; загрузка целых объектов, преобразования CRT, повторы, пропуски, ссылки и исключённые группы |
