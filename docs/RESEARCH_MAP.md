...
[War](research/LIB_WAR_SETUP_PLAN.md) сейчас **не завершён**. Выполнены574
исходных вызова; корпус3456631996 байтов завершён и хеширован, но полный аудит
и нативное сравнение ещё не выполнены. [Поправка входов](research/LIB_WAR_SETUP_INPUTS.md)
сохраняет отдельный реальный возврат загрузки перед объявленным готовым составом.
Четыре пробных вызова проверены по всем83 точкам; прежние145 промежуточных
случаев сохранены. [Нативное меню и его подключение](research/LIB_WAR_SETUP_NATIVE_PENDING.md)
теперь написаны и компилируются. Один тест на объявленном каталоге24 объектов
проверяет обычный возврат и6 поздних откатов за0.059с; это не сравнение с оригиналом.
Компаратор574 случаев и адаптер двух ресурсов теперь написаны и компилируются;
восемь сравнительных проб позднего отката ещё не запускались. Индекс принимает
только корпус с полным исходным аудитом и точными хешами. Повторная нативная
проверка шести откатов прошла за0.061с; сравнение War с оригиналом остаётся открытым.
Шесть прежних сравнительных тестов соседних режимов прошли за185.895с:
373 Team setup,373 Tournament setup и252 selection, включая поздние откаты.
Снимок20 собственных файлов остаётся незавершённым;295 эталонов неизменны.
Дополнительная нативная проверка внешнего вызова прошла:13 новых ресурсов и
восемь повторных Random-выборов откатываются при четырёх поздних ошибках вывода
в каждой цепочке. Два теста прошли за0.113с. [Статический разбор подготовки War](research/LIB_WAR_PREPARATION_STATIC.md)
проверяет330 инструкций и1359 байтов; исполнения подготовки ещё нет.
Независимые проверки текущего этапа завершены; дальнейшее сравнение требует X5.
X5 исчез из системы во время сборки; завершённый исходный
захват нельзя перезапускать. Локальное состояние и архив работы находятся в
build/research/lib-war-storage-interruption, запись libWarSetup — в общем
application-catalog-work.json. После подключения диска требуется проверить
сохранённый корпус и продолжить сравнение. Принятый срез пока cd80865.
