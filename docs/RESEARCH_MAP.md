...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, полный enabled catalog, CRT/table, menu/presentation, preparation/replay, round control и собственный запуск через World physics/417f80/contacts | Собственная цепочка загрузка→меню→Naruto/Sasuke/District→Start→music/replay→новый игровой вход→control/physics/417f80/contacts до41eefb; новая основа ещё вне Practice | D полных записей/масок/вызовов, две собственные цепочки без подстановки ожидаемого состояния; ОС/CPU — явные границы | На собственном41eefb: type-separated hit/item passes, остальные связи/вывод/планировщик и полный такт; paused render, ранний playback, animated loading, остальные экраны/режимы, CRT lifetime/Windows |
...
| R06 — реестр объектов, создание и удаление | Общий каталог/400-slot pool, constructor и часть opoint; physics spawn998 | Реестр из DAT, общий physics lifecycle и создание998; прежние голоса/змея/иглы остаются срезами | D loaders, preparation и [World physics](research/WORLD_PHYSICS.md), включая reuse/alias/порядок слотов | Общие opoint/item spawn, владельцы/команды и все виды создания/удаления на собственном полном такте |
| R07 — сбор контактов | Весь419380 и417200/417400/4171c0; caller44d05c и fusion tail4064d0 | Общие raw ITR/BDY, кадры70/78/7c, команды/владельцы, приоритеты, signed arest/vrest, nearest/multiple buffers | D7925 [controlled cases](research/WORLD_CONTACTS.md) и обе собственные цепочки до41eefb; полный pool/маски/globals/RNG | Все естественные DAT-последовательности, последующие42e100/item passes, весь такт и Windows; доля достигнутых инструкций не доказывает эти области |
...
| R09 — захваты, оружие, связанные объекты | Целая417f80: глубина, удержание/использование/броски; остальные cpoint/link passes открыты | [OriginalWorldLinks](research/WORLD_LINKS.md) читает общие wpoint/центры/ссылки; собственный первый вызов до41eed8 | D3018 synthetic/полные pool masks/RNG и оба собственных запуска; survey исходных10041 Frame отдельно | Непрерывные DAT-связки,41 out-of-array weaponact/достижимость, cpoint/grab, остальные link helpers и второй417f80 в полном такте |
