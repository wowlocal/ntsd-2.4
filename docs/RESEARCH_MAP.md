...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, полный каталог, пролог/подготовка/replay init и продолжение меню, Frame/heap, частичный словарь/снимок | Каталог → World/Actor → пролог → подготовка → запись; продолжение с полным ret12; storage ещё вне игрового цикла | D загруженных записей, пролога, подготовки, буферов и продолжения против Swift; сырой снимок шести прежних тактов | Ранние menu/RNG inputs, остальные типы/владельцы и полная нормализация снимка |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [пролог](research/MATCH_PRELUDE.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md), [продолжение](research/MATCH_CONTINUATION.md): участки до ret12 перенесены; ранние menu/RNG inputs, оставшийся словарь и полный снимок открыты |
