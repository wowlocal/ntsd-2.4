...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, каталог, CRT seed/table, mouse/menu/network, tail/World1→2, подготовка/replay, Frame/heap,18 initial sounds и10 UI bitmap | Каталог → World/Actor → CRT/table → mouse/menu/tail/World2 → подготовка → запись; отдельно sounds и пул→UI→44d05c=0; storage вне игрового цикла | D записей/меню/CRT/presentation/lifecycle/replay/WAV/initial UI против Swift; сырой снимок шести прежних тактов | Enabled Object sound registry и полный первый loading path, остальные экраны/World2→429730, CRT lifetime, типы и нормализация снимка |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [CRT/table](research/RANDOM_INITIALIZATION.md), [меню/mouse](research/MAIN_MENU.md), [tail/World1→2](research/MENU_PRESENTATION.md), [WAV](research/WAVE_LOADING.md), [initial UI](research/INITIAL_INTERFACE.md), [пролог](research/MATCH_PRELUDE.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md), [продолжение](research/MATCH_CONTINUATION.md): эти участки перенесены; далее enabled Object sounds и полный loading path, остальные экраны/World2→429730/CRT lifetime, словарь и полный снимок |
