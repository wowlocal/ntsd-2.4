...
Далее — происхождение входов, продолжение после `42d704`, оставшиеся типы и широкий такт.
...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, пул, полный каталог, пролог/подготовка матча и replay init, Frame/heap, частичный словарь/снимок | Каталог → World/Actor → пролог → подготовка → запись начального состояния повтора; новый storage ещё вне игрового цикла | D загруженных записей, пролога, подготовки и полных буферов против Swift; сырой снимок шести прежних тактов | Menu/RNG provenance, продолжение после 42d704, остальные типы/владельцы и полная нормализация снимка |
...
| 2 | **R02.1 — состояние такта: в работе** | Карта R01.1 | [Карточка](research/R02.1.md), [словарь](research/STATE_LAYOUT.md), [пролог](research/MATCH_PRELUDE.md), [подготовка](research/MATCH_PREPARATION.md), [replay init](research/REPLAY_INITIALIZATION.md): цепочка до 42d704 перенесена; происхождение menu/RNG, продолжение запуска, оставшийся словарь и полный снимок открыты |
