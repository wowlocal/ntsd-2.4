...
| R02 — структуры, глобальное состояние, RNG | Конструкторы, полный enabled catalog, CRT/table, menu/presentation, preparation/replay, round control и собственный запуск через World physics | Собственная цепочка загрузка→меню→Naruto/Sasuke/District→Start→music/replay→новый игровой вход→control/physics до41eed1; новая основа ещё вне Practice | D полных записей/масок/вызовов, две собственные цепочки без подстановки ожидаемого состояния; ОС/CPU — явные границы | На собственном41eed1: Z/contact, предметы/связи/вывод/планировщик и полный такт; paused render, ранний playback, animated loading, остальные экраны/режимы, CRT lifetime/Windows |
