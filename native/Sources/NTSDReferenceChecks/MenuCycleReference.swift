...
public enum MenuCycleReference {
    ...
    private typealias Storage = MenuStartupReference.Storage
    private typealias Retained = MenuStartupReference.Retained
    private typealias Position = MenuStartupReference.Position
    ...
    public static func compare(cycle: Data,returning: Data,screen: Data,startup: Data,menu: Data,loading: Data,catalog: Data,sounds: Data) throws -> Result {
        ...
        var cache: [String:[UInt8]] = [:],records = 0,bytes = 0,events = 0,checkpoints = 0,cases = 0,returns = 0,callbacks = 0
        ...
        let parent = try MenuReturnReference.compare(returning: returning,screen: screen,startup: startup,menu: menu,loading: loading,catalog: catalog,sounds: sounds) { own,initialContext,crt,initialMusic,initialResources in
            callbacks += 1;var state = own,context = initialContext,musicMemory = initialMusic,loader = initialResources
            for (i,item) in c.cases.enumerated() {
                ...
                guard item.stimulus.count == (i == 1 || i == 3 ? 1 : 0) else { throw error("Acquired input sequence") }
                ...
                let outcome = try OriginalLoadedMatchCycle.run(state: &state,context: &context,controlBoundary: { request in
                    ...
                    guard !paused,item.prefix.paused == 0,item.prefix.phase == (i%2 == 0 ? 0 : 1),
                ...
                if try state.globals.integer(at: 0x44d020-0x44d000,as: Int32.self) == 10 {
                    ...
                    guard i == 3,item.mode == nil,item.returned == nil,item.end.pc == 0x429e5a,item.end.sp == 0x1000df08,
                          try state.globals.integer(at: 0x44d020-0x44d000,as: Int32.self) == 3 else { throw error("Own next selection entry") }
                }
        ...
        guard callbacks == 1,returns == 3 else { throw error("Own parent/return completion") }
        return .init(parent: parent,cases: cases,records: records,bytes: bytes,events: events,checkpoints: checkpoints,returns: returns)
    }
}
