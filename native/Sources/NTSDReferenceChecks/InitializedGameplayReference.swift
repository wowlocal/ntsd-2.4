...
public enum InitializedGameplayReference {
    ...
    public static func compare(_ data: Data,fixture: (String) throws -> Data,postDraw: Data? = nil,commands: Data? = nil,hud: Data? = nil,notices: Data? = nil,resultRecording: Data? = nil,resultLayout: Data? = nil,returned: Data? = nil,compareGameplayBody: Bool = false,continuous: Data? = nil,active: Data? = nil,paused: Data? = nil) throws -> Result {
        guard active == nil || paused == nil else { throw error("Active and paused continuations need separate retained neutral parents") }
        ...
        if let active {
            guard continuous != nil else { throw error("Active calls need the complete neutral parent") }
            let next = try JSONDecoder().decode(Corpus.self,from: MatchPreparationReference.unpack(active,maximumCount:256_000_000))
            guard next.exeSHA256 == c.exeSHA256,next.dllSHA256 == c.dllSHA256,next.control == c.control,
                  next.fpu.initialization == finalAudit.initialization,next.fpu.transitions == finalAudit.transitions,
                  next.fpu.watchedInstructions == finalAudit.watchedInstructions,next.fpu.checkpoints.starts(with:finalAudit.checkpoints),
                  next.fpu.checkpoints.count > finalAudit.checkpoints.count,
                  next.fpu.checkpoints.allSatisfy({ $0.fpcw == 0x23f }) else { throw error("Active initialized context") }
            finalAudit = next.fpu
        }
        if let paused {
