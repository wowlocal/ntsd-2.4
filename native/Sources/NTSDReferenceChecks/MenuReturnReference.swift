...
public enum MenuReturnReference {
    ...
    struct Input: Decodable { let presentation: OriginalMenuPresentationInput, milliseconds: UInt32, drawResults: [Int32] }
    struct Checkpoint: Decodable { let kind: String, pc: UInt32, sp: UInt32, saved: [UInt32], seh: UInt32, globals: String }
    ...
    public static func compare(returning: Data,screen: Data,startup: Data,menu: Data,loading: Data,catalog: Data,sounds: Data, arithmeticPrecision: OriginalArithmeticPrecision = .bits64,
        ...
        guard callbacks == 1,let portion = result else { throw error("Missing own first screen") }
        return .init(parent: parent,cases: portion.cases,events: portion.events,helpers: portion.helpers,records: portion.records,bytes: portion.bytes,draws: portion.draws,checkpoints: portion.checkpoints)
    }

    struct Portion {
        let state: OriginalMatchPreparation,context: OriginalInputControlContext
        var cases = 0, events = 0, helpers = 0, records = 0, bytes = 0, draws = 0, checkpoints = 0
    }
    ...
    static func compareCases(actorAddresses: [UInt32],objectAddresses: [UInt32],cases items: [Case],blobs sourceBlobs: [String:Blob],
        ...
        return .init(state: state,context: context,cases: cases,events: events,helpers: helpers,records: records,bytes: bytes,draws: draws,checkpoints: checkpoints)
    }
}
