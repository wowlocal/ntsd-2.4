...
public enum CharacterScreenReference {
    ...
    struct Point: Decodable { let pc: UInt32,sp: UInt32,seat: UInt32,state: Snapshot,locals: [String:Int32] }
    ...
    public static func compare(character: Data,cycle: Data,returning: Data,screen: Data,startup: Data,menu: Data,loading: Data,catalog: Data,sounds: Data,requireComplete: Bool = true) throws -> Result {
        ...
        var callbacks = 0
        let parent = try MenuCycleReference.compare(cycle: cycle,returning: returning,screen: screen,startup: startup,menu: menu,loading: loading,catalog: catalog,sounds: sounds) { own,initialContext,crt,initialMusic,initialResources in
            callbacks += 1;var state = own,context = initialContext,music = initialMusic,resources = initialResources
            ...
            for (i,item) in c.cases.enumerated() {
                ...
                if let cycle = item.cycle {
                    guard i > 0,cycle.stimulus.isEmpty,cycle.mode == nil,cycle.returned == nil else { throw error("Whole next caller") }
                ...
                } else { guard i == 0,item.acquired.isEmpty else { throw error("Own first429e5a") } }
                ...
                guard item.fills.count == 1,!item.input.drawResults.isEmpty else { throw error("Screen drawing input") }
                var eventIndex = 0,pointIndex = 0,blits = 0
                ...
                let end = try OriginalCharacterScreen.advance(state: &state,selectionAtEntry: selectionAtEntry,target: 0x28002020,input: item.input.body,fillBacking: blob(item.fills[0]),draw: { request,globals in
                    ...
                    try event(.init("draw",[address,UInt32(bitPattern: request.x),UInt32(bitPattern: request.y),UInt32.max,0,0,request.target]));draws += 1
                    let input = try OriginalBitmapDrawInput(x: request.x,y: request.y,frame: -1,colorKey: 0,mirrored: 0,sourceSurface: surface,targetSurface: request.target,
                ...
                },observe: event,checkpoint: { point,value in
                ...
                guard end == .returned,end == item.continuation,item.end.pc == 0x42e0d2,item.end.sp == 0x1000df08,
                      eventIndex == item.events.count,pointIndex == item.checkpoints.count,pointIndex == 12 else { throw error("Character completion") }
                for h in item.helpers {
                    let pop: UInt32 = h.entry == 0x401a30 ? 4 : h.entry == 0x43f010 ? 24 : 0
                    guard [0x415160,0x401290,0x401a30,0x43f010,0x43ef70,0x431c70].contains(h.entry),h.saved.count == 4,h.pop == pop,h.returnSP == h.entrySP+4+pop,
                          (0x42a1d0...0x42b218).contains(h.returnPC) || [0x43f0d5,0x43f212].contains(h.returnPC) else { throw error("Character helper ABI") }
                }
                ...
                let returning = c.cases[i].returned
                guard returning.inherited,returning.stimulus.isEmpty else { throw error("Own character return") }
        ...
        guard callbacks == 1 else { throw error("Own parent completion") }
        return .init(parent: parent,cases: cases,events: events,helpers: helpers,records: records,bytes: bytes,draws: draws,checkpoints: checkpoints,returns: returns)
    }
}
