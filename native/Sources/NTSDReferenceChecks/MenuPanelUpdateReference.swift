...
public enum MenuPanelUpdateReference {
    ...
    public static func compare(_ data: Data,
        onNatural: ((OriginalStateRecord,OriginalStateRecord,[UInt32:OriginalLoadedBitmap]) throws -> Void)? = nil) throws -> Result {
        ...
        for (i,item) in c.cases.enumerated() {
            ...
            try panelRecords(item.panelRecords,item.label)
            if i == 0 { try onNatural?(world,state,early) }
            result.cases += 1
        }
        return result
    }
