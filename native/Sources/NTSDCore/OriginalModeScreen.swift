...
public enum OriginalModeScreen {
    ...
    public static func advance(state: inout OriginalMatchPreparation, memory: inout OriginalMenuPresentationMemory,
        ...
        var candidate = state, owned = memory, scratch = local
        let base = OriginalMatchPreparation.globalBase
        ...
        func bitmap(_ address: Int,_ x: Int32,_ y: Int32,_ frame: Int32,_ key: UInt32 = 1) throws {
            ...
            try emit("draw",args);try draw(args,candidate.globals,owned)
        }
        func text(_ offset: Int,_ x: Int32,_ y: Int32,_ color: UInt32 = 0xd07750,_ background: UInt32 = 0x602010) throws {
            try OriginalSurfaceText.draw(string(offset),target: bits(word(0x455608)),background: background,color: color,x: x,y: y,
                dcResult: input.dcResult,dc: input.dc) { try emit($0.kind.rawValue,$0.arguments,$0.strings) }
        }
        func sound() throws {
            try OriginalMatchPrelude.confirmationSound(in: candidate.globals) { event in
                switch event {
        ...
        func finish(_ result: OriginalModeScreenExit) -> OriginalModeScreenExit { state = candidate;memory = owned;local = scratch;return result }
        ...
        try observe(filled)
        if try word(0x4511ac) == 0 { try background(&candidate.globals,&owned) }
        try update(&candidate.globals)
        ...
        try bitmap(0x451178,257,menuY &+ 160,candidate.globals.integer(at: 0x44f1af-base,as: Int8.self) > 0 ? 50 : 51)
        ...
        let selection = try OriginalModeSelection.advance(world: candidate.world,actors: candidate.actors,globals: &candidate.globals,memory: &owned) {
            try emit($0.kind.rawValue,$0.arguments)
        }
        if selection == .playback { return finish(.playback) }
        try emit("panel",[0x4513c4,target]);try panel(&candidate.globals)
