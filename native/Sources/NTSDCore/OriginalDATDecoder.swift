...
public enum OriginalFileTranslation: String, Codable, Sendable {
    ...
    fileprivate func read(_ bytes: [UInt8]) -> [UInt8] {
    ...
    fileprivate func write(_ bytes: [UInt8]) -> [UInt8] {
        guard self == .text else { return bytes }
...
public enum OriginalDATDecoder {
    ...
    public static func decode(_ source: [UInt8], fileName: String, translation: OriginalFileTranslation) throws -> String {
        ...
        let suffix = Array(fileName.unicodeScalars.suffix(3).map { UInt8($0.value) | 0x20 })
        let logical: [UInt8]
        if suffix == Array("dat".utf8) {
            ...
            let decoded = input.dropFirst(123).enumerated().map { $0.element &- key[($0.offset + 123) % key.count] }
