...
/// Arithmetic retains x87's64-bit intermediate significand and explicit stores.
public enum OriginalActorPhysics {
    public static func apply(actor: inout OriginalStateRecord,object: OriginalLoadedObject,globals: inout OriginalStateRecord,
                             sse2Conversion: Bool = false,observe: (OriginalActorPhysicsEvent) throws -> Void = { _ in }) throws {
        try apply(actor: &actor,header: object.header,globals: &globals,sse2Conversion: sse2Conversion,frame: { number in
    ...
    static func apply(actor: inout OriginalStateRecord,header: OriginalStateRecord,globals: inout OriginalStateRecord,
                      sse2Conversion: Bool = false,frame: (Int32) throws -> OriginalStateRecord,
        ...
        try withoutActuallyEscaping(frame) { frames in
            try withoutActuallyEscaping(observe) { observer in
                var body = Body(actor: actor,globals: globals,header: header,frame: frames,observe: observer)
    ...
    private struct Body {
        ...
        func v(_ offset: Int) throws -> OriginalExtended { try OriginalExtended(actor.binary64(at: offset)) }
        ...
        mutating func friction(_ offset: Int) throws {
            let epsilon = try OriginalExtended(0.0001),one = try OriginalExtended(1)
        ...
        mutating func run(sse2Conversion: Bool) throws {
            ...
            let zero = try OriginalExtended(0),epsilon = try OriginalExtended(0.0001)
            ...
            if type == 4 || id == 120 { try store(0x58,vx * OriginalExtended(0.2) + v(0x58)) }
            if id == 101 { try store(0x58,v(0x58)-OriginalExtended(0.2)*vx) }
            ...
            if type == 3,try f(0x2c) > 0 { try store(0x68,v(0x68)+OriginalExtended(Double(f(0x2c) &- 50))) }
            if try i(0x14) >= 0 { try friction(0x40);try friction(0x50) }
            if [4,6].contains(type),try f(8) == 1000 {
                let speed = try v(0x40)
                if try speed > OriginalExtended(9) || speed < OriginalExtended(-9) { try put(0x70,40) }
            }
            ...
            if y < -epsilon {
                if type != 3 {
                    ...
                    try store(0x48,v(0x48)+OriginalExtended(gravity))
                }
        ...
        mutating func airborne() throws {
            let speed = try v(0x48),one = try OriginalExtended(1)
            if try f(8) == 12 {
                let number = try i(0x70)
                if number < 185 {
                    let phase = try speed < OriginalExtended(-8) ? 0 : speed < one ? 1 : speed < OriginalExtended(8) ? 2 : 3
                    try put(0x70,Int32(180+phase))
                    if try i(0x320) < 0 {
                        let falling = try speed < OriginalExtended(12) && globals.integer(at: 0x450bd0-0x44d000,as: Int32.self) >= 6
        ...
        mutating func fighterLanding(y: OriginalExtended) throws {
            let zero = try OriginalExtended(0),epsilon = try OriginalExtended(0.0001),three = try OriginalExtended(3)
            if y > epsilon,try v(0x48) > epsilon,try f(8) == 13 {
                if try v(0x48) > OriginalExtended(17) || v(0x40) > OriginalExtended(9) || v(0x40) < OriginalExtended(-9) {
                    let divisor = try i(0x340)
                    try put(0x2fc,i(0x2fc) &+ (divisor == 0 ? -10 : -1000/divisor))
                    try number(0x60,0);try number(0x48,-3.5)
                    try store(0x40,min(OriginalExtended(7),max(OriginalExtended(-7),v(0x40))))
                    try put(0x70,185)
            ...
            if try jumpLanding || (hitGround && f(8) != 12 && f(8) != 18) {
                ...
                if try v(0x48) <= OriginalExtended(11) && v(0x40) <= OriginalExtended(9) && v(0x40) >= OriginalExtended(-9) && f(8) != 18 {
                    try number(0x60,0);try number(0x48,0);try store(0x40,v(0x40)/three)
        ...
        mutating func objectLanding(type: Int32,id: Int32,y: OriginalExtended) throws {
            let epsilon = try OriginalExtended(0.0001)
            ...
            let state = try f(8)
            if type == 1 {
                if try v(0x48) > OriginalExtended(9.9),state == 1002 {
                    try number(0x48,-8);try put(0x70,7);try reverse()
                    try store(0x40,v(0x40)*OriginalExtended(0.5));try weaponSound();return
                }
                try put(0x70,state == 1002 ? 70 : 60)
            } else if type == 2 {
                if try v(0x48) > OriginalExtended(9) {
                    try weaponSound();try number(0x48,-5);try reverse();try store(0x40,v(0x40)*OriginalExtended(0.5));return
                }
                try put(0x31c,max(0,i(0x31c) &- h(0x94)));try put(0x70,20)
            } else {
                let fast = try v(0x48) > OriginalExtended(8.5) || v(0x40) < OriginalExtended(-10) || v(0x40) > OriginalExtended(10)
                if fast && [1000,1002].contains(state) {
                    try put(0x70,0)
                    let rebound = try v(0x48)*OriginalExtended(-0.7);try store(0x48,max(OriginalExtended(-10),rebound))
                    try store(0x40,v(0x40)*OriginalExtended(0.7));try weaponSound();return
                }
                try put(0x70,state == 1002 ? 70 : 60)
            }
            try number(0x48,0);try put(0x88,0)
            try store(0x40,v(0x40)*OriginalExtended(type == 4 || type == 6 ? 0.7 : 0.5))
        }
    }
}
