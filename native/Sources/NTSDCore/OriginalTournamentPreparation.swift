/// Tournament434349..4347c5: seats, date/round filename, Actors, arena and
/// whole recording initialization. Callers stage device/allocator effects until
/// the enclosing432ab0/422ab8 return commits. All references are owned tokens.
public enum OriginalTournamentPreparation {
    public static func prepare(state: inout OriginalMatchPreparation,
        ...
        func error(_ s: String) -> OriginalStateError { .invalidStorage("Tournament preparation: "+s) }
        ...
        for position in 0..<2 {
            let index=try word(0x4513c8+position*4),participant=try seat(word(0x44d0e0+seat(index)*4))
            if try word(0x451364+participant*4)==0 {
                for slot in 0..<8 where try scene.world.integer(at:4+slot,as:UInt8.self)==0 {
                    try OriginalMatchPrelude.playSound(in:scene.globals,slot:0x45560c) { event in
                        switch event {
                    ...
                    try write(0x4512f8+slot*4,index);try scene.world.write(UInt8(1),at:0xe+slot)
                    let object=try word(0x44d0c0+participant*4)
                    guard scene.loadedObjects.indices.contains(Int(object)) else { throw error("CPU Object binding") }
                    try scene.actors[actor(slot+10)].write(UInt32(object),at:0x368);break
                }
            }
        }
        try mark(0x4343c3)
        ...
        name += Array("_1on1_".utf8)
        switch try word(0x44d318) {
        case 0:name += Array("Prelminar".utf8)
        ...
        if try word(0x44d028)==1 {
            var selected=try scene.drawMenuRandom(stream:0xfd,range:count &- 2,observe:observe)
        ...
        for slot in 0..<20 where try scene.world.integer(at:4+slot,as:UInt8.self) != 0 {
            ...
            try scene.actors[a].write(Int32(participant+10),at:0x364);try scene.actors[a].write(Int32(75),at:8)
            let arena=scene.backgrounds[Int(arenaIndex)],width=try arena.integer(at:0,as:Int32.self)
            let x=try scene.drawMenuRandom(stream:0xfe,range:width/2,observe:observe) &+ width/4
            ...
            let z=try scene.drawMenuRandom(stream:0xff,range:upper &- lower,observe:observe) &+ lower
        ...
        try mark(0x434765)
        try write(0x450bbc,0);try write(0x44d034,1)
        ...
        try observe(.init("resetInput"));try scene.resetOriginalInput();try mark(0x4347a6)
