...
public struct OriginalApplicationMenuInputs: Equatable {
    ...
    public static func bundledDirectory(in appBundle: Bundle = .main) throws -> URL {
        if let resources = appBundle.resourceURL {
            let directory = resources.appendingPathComponent("OriginalCharacterMenu",isDirectory:true)
            if appBundle.bundleURL.pathExtension == "app" || FileManager.default.fileExists(atPath:directory.path) { return directory }
        }
        guard let directory = Bundle.module.url(forResource:"OriginalCharacterMenu",withExtension:nil) else {
            throw Boundary.missing("OriginalCharacterMenu")
        }
        return directory
    }
    ...
    public static func load(directory: URL) throws -> Self {
        ...
        guard manifest.version == 1,
              ...
              manifest.entries.map(\.name) == OriginalMenuResourceLoading.paths else { throw Boundary.invalid("Manifest contract") }
