...
public enum OriginalBitmapSurfaceLoading {
    ...
    public static func load<Context>(path: [UInt8],device: UInt32,flags: UInt32,
        ...
        _ = try copy(surface:surface,bitmap:handle,context:&candidate,perform:perform)
        _ = try request(.init("deleteObject",[handle]))
        context = candidate;return surface
    }
    @discardableResult
    public static func copy<Context>(surface: UInt32,bitmap: UInt32,x: UInt32 = 0,y: UInt32 = 0,
        ...
        try apply(request(.init("description",[surface],structure:description)),&description)
        ...
        if acquired.result == 0 {
            ...
            let dw = try field(description,12,"surface width"),dh = try field(description,8,"surface height")
        ...
        context = candidate;return acquired.result
    }
}
...
extension OriginalBitmapConstructor {
    ...
    public static func constructWithSurfaceLoading<Context>(path: String,optional: Bool,
        ...
        var candidate = context,record = try OriginalStateRecord(bytes:backing,defined:[Bool](repeating:false,count:backing.count))
        var width: Int32?,height: Int32?
        let surface = try OriginalBitmapSurfaceLoading.load(path:Array(path.utf8),device:device,flags:flags,context:&candidate,dimensions:{ index,value,_ in
