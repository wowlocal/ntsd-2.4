...
final class OriginalApplicationMenuInputsTests: XCTestCase {
    ...
    struct Golden {
        ...
        init() throws {
            reference = try JSONDecoder().decode(Reference.self,from:Data(contentsOf:XCTUnwrap(Bundle.module.url(
                forResource:"menu-dib-pixels",withExtension:"json",subdirectory:"Fixtures"))))
            payload = try Data(contentsOf:XCTUnwrap(Bundle.module.url(forResource:"menu-dib-pixels",withExtension:"bin",subdirectory:"Fixtures")))
    ...
    static let inputs: Result<OriginalApplicationMenuInputs,Error> = Result { try .bundled() }
    static let golden: Result<Golden,Error> = Result { try Golden() }
    func testElevenOriginalDIBsFullPixelsMasksAndRows() throws {
        ...
        XCTAssertEqual(written,582632);XCTAssertEqual(unknown,0);XCTAssertEqual(rows,1388)
    }
    func testOrdinaryAppPackageAndOwnedSnapshot() throws {
        ...
        let resources = root.appendingPathComponent("CharacterMenu.app/Contents/Resources"),package = resources.appendingPathComponent("OriginalCharacterMenu")
        try fm.createDirectory(at:resources,withIntermediateDirectories:true)
        try fm.copyItem(at:OriginalApplicationMenuInputs.bundledDirectory(),to:package)
        let app = resources.deletingLastPathComponent().deletingLastPathComponent()
        try Data("<?xml version=\"1.0\"?><plist version=\"1.0\"><dict><key>CFBundleIdentifier</key><string>test.characterMenu</string><key>CFBundlePackageType</key><string>APPL</string></dict></plist>".utf8)
            .write(to:app.appendingPathComponent("Contents/Info.plist"))
        let bundle = try XCTUnwrap(Bundle(url:app)),owned = try OriginalApplicationMenuInputs.bundled(in:bundle)
        XCTAssertEqual(owned,try Self.inputs.get())
        try fm.removeItem(at:package)
        XCTAssertThrowsError(try OriginalApplicationMenuInputs.bundled(in:bundle))
        XCTAssertEqual(owned,try Self.inputs.get())
    }
    func testMissingCorruptExtraAndSymlinkPackagesAreRejected() throws {
        ...
        for kind in ["missing","corrupt","manifest","extra","symlink"] {
            ...
            try fm.copyItem(at:OriginalApplicationMenuInputs.bundledDirectory(),to:p)
            let dib = p.appendingPathComponent("SPARK.dib")
            switch kind {
            ...
            default:try fm.removeItem(at:dib);try fm.createSymbolicLink(at:dib,withDestinationURL:OriginalApplicationMenuInputs.bundledDirectory().appendingPathComponent("SPARK.dib"))
            }
            XCTAssertThrowsError(try OriginalApplicationMenuInputs.load(directory:p),kind)
        }
    }
}
