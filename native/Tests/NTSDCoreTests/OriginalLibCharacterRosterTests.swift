...
final class OriginalLibCharacterRosterTests: XCTestCase {
    ...
    final class Resources {
        ...
        init() throws {
            let url = try ProcessInfo.processInfo.environment["NTSD_LIB_CHARACTER_ROSTER"].map { URL(fileURLWithPath:$0) }
                ?? XCTUnwrap(Bundle.module.url(forResource:"original-lib-character-roster",withExtension:"json",subdirectory:"Fixtures"))
            ...
            let temporary = FileManager.default.temporaryDirectory.appendingPathComponent("ntsd-lib-roster-"+UUID().uuidString+".json")
            try JSONSerialization.data(withJSONObject:projected).write(to:temporary)
            defer { try? FileManager.default.removeItem(at:temporary) }
            startup = try Base.Resources(url:temporary,expectedCases:70)
            ...
            XCTAssertEqual(corpus.cases.count,70)
        ...
        do {
            let end=try OriginalCharacterMenuContinuation.advance(state:&prepared,memory:&memory,music:&music,resources:&resources,libraryText:&library,environment:&committed,
                ...
                musicRequest:{ _,_ in XCTFail("Music child must be skipped for menu1/3");throw Stop.unexpected
                    ...
                    let address: UInt32,owned: OriginalLoadedBitmap,surface: UInt32
                    switch request.bitmap {
                    case .menu(let token):address=token;owned=try XCTUnwrap(images.bitmaps[token]);surface=try XCTUnwrap(buffer.graphics.surfaceForWrapper[token])
                    case .catalog(let index):address=try XCTUnwrap(r.portraitAddresses[index]);owned=r.catalog.bitmaps[index];surface=0x24000000
                    }
                    ...
                    if failure == "portrait",case .catalog = request.bitmap { throw Stop.injected }
    ...
    func testRetainedRosterTeamsAndReadyWithLibraryText() throws {
    ...
    func testLateSelectedPortraitAndPresentRollBackCurrentCall() throws {
        let r=try Resources()
        for failure in ["portrait","present"] {
            var retained: Retained?
            for item in r.corpus.cases.prefix(7) { _ = try run(item,r,&retained) }
            _ = try run(r.corpus.cases[7],r,&retained,failure:failure)
        }
    }
}
