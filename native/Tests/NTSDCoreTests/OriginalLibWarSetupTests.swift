...
final class OriginalLibWarSetupTests: XCTestCase {
    ...
    func run(_ item: Case,_ r: Resources,_ retained: inout Retained?,failure: String? = nil) throws -> Int {
        ...
        var eventIndex=0,characterPoint=0,musicIndex=0,warPoint=0
        let warAdapter=item.warGraphics.map { OriginalWarSetupSurfaceAdapter($0,r,control:item.spec.control) }
        ...
        func compareState(_ value: OriginalMatchPreparation,_ records: [Record]) throws {
            ...
            let bitmapPointers=c.catalogDependency.bitmapAddresses
            for i in 0..<101 {
        ...
        do {
            let end=try OriginalCharacterMenuContinuation.advanceWithWar(state:&prepared,memory:&memory,music:&music,resources:&resources,war:&war,libraryText:&library,environment:&committed,
                ...
                characterCheckpoint:{ point,state,buffer in
                    ...
                    let points=item.points.filter { $0.kind.hasPrefix("war-") }
            ...
            XCTAssertNil(failure);XCTAssertEqual(warPoint,item.points.filter { $0.kind.hasPrefix("war-") }.count);XCTAssertEqual(end.rawValue,item.end);XCTAssertEqual(eventIndex,item.events.count)
        ...
        for number in index.c.cases.indices {
            try autoreleasepool {
                ...
                for failure in failures {
                    var trial=retained
                    _ = try run(item,r,&trial,failure:failure)
