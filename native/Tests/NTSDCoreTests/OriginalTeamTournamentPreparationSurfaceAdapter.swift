...
final class OriginalTeamTournamentPreparationSurfaceAdapter {
    typealias Test = OriginalLibTeamTournamentPreparationTests
    ...
    init(_ c: Test.PreparationGraphics,_ r: Test.Resources) { self.c=c;self.r=r;self.allocation=c.allocationStart }
    ...
    func release(_ index: Int,_ bitmap: OriginalLoadedBitmap,_ context: inout Base.Context,
        ...
        XCTAssertGreaterThanOrEqual(ordinal,0);XCTAssertLessThan(ordinal,c.allocationStart)
    ...
    func compare(_ state: OriginalMatchPreparation,_ context: Base.Context) throws {
        ...
        for (i,record) in c.records.enumerated() {
            ...
            let constructorIndex=try XCTUnwrap(c.constructionHistory.firstIndex { $0.kind=="constructor" && $0.wrapper==record.address })
            let h=c.constructionHistory[constructorIndex]
            ...
            let loader=try XCTUnwrap(c.constructionHistory[..<constructorIndex].last { $0.kind=="loader" && $0.eventStart>=h.eventStart && $0.eventEnd<=h.eventEnd })
