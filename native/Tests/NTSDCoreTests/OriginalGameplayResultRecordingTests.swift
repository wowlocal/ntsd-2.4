...
final class OriginalGameplayResultRecordingTests: XCTestCase {
    private func compare(_ control: Bool) throws {
        ...
        let data: Data
        if let directory = ProcessInfo.processInfo.environment["NTSD_GAMEPLAY_RESULT_RECORDING_DIRECTORY"] {
            data = try Data(contentsOf: URL(fileURLWithPath: directory).appendingPathComponent("gameplay-result-recording"+suffix+".json"))
        } else { data = try fixture("original-gameplay-result-recording"+suffix+".json") }
        let r = try InitializedGameplayReference.compare(fixture("original-initialized-gameplay"+suffix+".json"),fixture: fixture,postDraw: fixture("original-gameplay-lifecycle"+suffix+".json"),commands: fixture("original-gameplay-commands"+suffix+".json"),hud: fixture("original-gameplay-hud"+suffix+".json"),notices: fixture("original-gameplay-notices"+suffix+".json"),resultRecording: data)
        let g = r.gameplay
        ...
        XCTAssertEqual(g.resultRecordingPasses,1);XCTAssertEqual(r.fpuCheckpoints,1605)
        print("GAMEPLAY RESULT RECORDING",suffix,"records",g.records,"bytes",g.bytes,"helpers",g.helpers,"checkpoints",g.checkpoints,
              "FPU checkpoints",r.fpuCheckpoints,"transitions",r.fpuTransitions)
    }
    func testResultRecordingOnOwnInitializedMatch() throws { try compare(false) }
    func testResultRecordingOnOwnInitializedMatchWithControlBacking() throws { try compare(true) }
}
