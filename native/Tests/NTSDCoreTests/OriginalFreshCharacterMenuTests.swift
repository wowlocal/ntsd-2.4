...
final class OriginalFreshCharacterMenuTests: XCTestCase {
    ...
    final class Resources {
        ...
        init() throws {
            let url = try ProcessInfo.processInfo.environment["NTSD_FRESH_CHARACTER_MENU"].map { URL(fileURLWithPath:$0) }
                ?? XCTUnwrap(Bundle.module.url(forResource:"original-fresh-character-menu",withExtension:"json",subdirectory:"Fixtures"))
            ...
            let temporary = FileManager.default.temporaryDirectory.appendingPathComponent("ntsd-fresh-character-"+UUID().uuidString+".json")
            try JSONSerialization.data(withJSONObject:projected).write(to:temporary)
            defer { try? FileManager.default.removeItem(at:temporary) }
            startup = try Base.Resources(url:temporary,expectedCases:30)
            ...
            XCTAssertEqual(corpus.cases.count,30)
            ...
            // Rebuild an accepted catalog normally. Random-only controlled cases
            // never dereference it; no Object/private storage comes from this source.
            let urlCatalog = try XCTUnwrap(Bundle.module.url(forResource:"original-loaded-catalog-interleaved",withExtension:"json",subdirectory:"Fixtures"))
    ...
    func run(_ item: Case,_ r: Resources,_ library: inout OriginalLibSurfaceText,failure: String? = nil) throws -> Int {
        ...
        var eventIndex=0,characterPoint=0,committed=Environment()
        ...
        func compareState(_ value: OriginalMatchPreparation,_ records: [Record]) throws {
            ...
            for (i,p) in item.actorAddresses.enumerated() { try compare(value.actors[i],r.record(XCTUnwrap(records[p])),item.spec.label+" Actor"+String(i)) }
        }
        do {
            let end=try OriginalCharacterMenuContinuation.advance(state:&prepared,memory:&memory,music:&music,resources:&resources,libraryText:&library,environment:&committed,
                ...
                musicRequest:{ e,env in
                    ...
                    guard case let .menu(address)=request.bitmap else { throw Stop.unexpected }
                    let owned=try XCTUnwrap(images.bitmaps[address])
                    ...
                    let surface=try XCTUnwrap(buffer.graphics.surfaceForWrapper[address])
                    ...
                    _ = try OriginalBitmapDrawing.draw(input,bitmap:owned.storage,observeRead:{ read in
                ...
                characterCheckpoint:{ point,state,buffer in
                    ...
                    XCTAssertEqual(point.locals,try XCTUnwrap(expected.locals).reduce(into:[:]) { $0[Int($1.key)!]=Int32(bitPattern:$1.value) })
            ...
            XCTAssertEqual(item.endSP,c.entrySP+8)
            ...
            XCTAssertEqual(library,initialLibrary);XCTAssertEqual(committed,Environment())
    ...
    func testFreshResourcesHumanCharacterBodyAndActualReturns() throws {
    ...
    func testLateFreshMenuFailuresRollBackEveryOwner() throws {
        let r=try Resources()
        for (label,failure) in [("join-all","afterMusic"),("join-all","afterStartup"),("join-all","screen"),("join-all","text"),("join-all","draw"),("join-all","present"),("join-all","beforeReturn")] {
