...
final class OriginalLibTeamTournamentPreparationTests: XCTestCase {
    ...
    final class Resources {
        ...
        init() throws {
            let url = try ProcessInfo.processInfo.environment["NTSD_LIB_TEAM_TOURNAMENT_PREPARATION"].map { URL(fileURLWithPath:$0) }
                ?? XCTUnwrap(Bundle.module.url(forResource:"original-lib-team-tournament-preparation",withExtension:"json",subdirectory:"Fixtures"))
            ...
            if let parts=try? JSONDecoder().decode(Transport.self,from:envelope) {
                ...
                for name in parts.deflateParts {
                    guard !name.contains("/"),name.hasPrefix("original-lib-team-tournament-preparation-part") else { throw Stop.unexpected }
                ...
                data=try MatchPreparationReference.unpack(joined,maximumCount:4_000_000_000)
            } else { data=try MatchPreparationReference.unpack(envelope,maximumCount:4_000_000_000) }
            ...
            let temporary = FileManager.default.temporaryDirectory.appendingPathComponent("ntsd-lib-team-tournament-preparation-"+UUID().uuidString+".json")
            try JSONSerialization.data(withJSONObject:projected).write(to:temporary)
            defer { try? FileManager.default.removeItem(at:temporary) }
            startup = try Base.Resources(url:temporary,expectedCases:210)
            ...
            XCTAssertEqual(corpus.cases.count,210)
    ...
    func run(_ item: Case,_ r: Resources,_ retained: inout Retained?,failure: String? = nil) throws -> Int {
        ...
        var preparationAdapter: OriginalTeamTournamentPreparationSurfaceAdapter?
        ...
        do {
            let end=try OriginalCharacterMenuContinuation.advance(state:&prepared,memory:&memory,music:&music,resources:&resources,libraryText:&library,environment:&committed,includeTeamTournamentBracket:true,
                ...
                teamTournamentPreparation:{ scene,owned,env in
                    let pg=try XCTUnwrap(item.preparationGraphics)
                    let adapter=OriginalTeamTournamentPreparationSurfaceAdapter(pg,r);preparationAdapter=adapter
                    if item.spec.generation==0 { env.preparationGraphics=env.graphics }
                    try OriginalTeamTournamentPreparation.prepare(state:&scene,memory:&owned,localTime:{
                        ...
                        if pc==0x436a9f { try adapter.compareGlobals(value.globals) }
                        if pc==0x436afa {
    ...
    func testWholeTeamTournamentPreparationRetainsLibraryMenu() throws {
    ...
    func testLateTeamTournamentPreparationRollsBackWholeCall() throws {
        let r=try Resources()
        let start=try XCTUnwrap(r.corpus.cases.firstIndex { $0.spec.label=="controlled-team-preparation-0" })
        var first: Retained?;_ = try run(r.corpus.cases[start],r,&first)
        for failure in ["release","free","allocateReplay","recording","beforeReturn"] {
            var retained=first;_ = try run(r.corpus.cases[start+1],r,&retained,failure:failure)
        }
        var released=first;_ = try run(r.corpus.cases[start+1],r,&released)
        _ = try run(r.corpus.cases[start+2],r,&released,failure:"bitmap")
    }
    func testUnconnectedExistingArenaReleaseRejectsAndRollsBack() throws {
        let r=try Resources(),item=try XCTUnwrap(r.corpus.cases.first { $0.spec.label=="controlled-team-preparation-0" })
        ...
        do {
            ...
            try OriginalTeamTournamentPreparation.prepare(state:&scene,memory:&memory,
