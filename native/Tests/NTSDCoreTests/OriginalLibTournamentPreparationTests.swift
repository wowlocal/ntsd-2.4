...
final class OriginalLibTournamentPreparationTests: XCTestCase {
    ...
    struct Spec: Decodable { let assigned: [[Int]]?, localTime: [UInt16]?; let buttons: [[Int]]?; let label: String, control: Bool, music: MusicSpec?, chain: Bool?, dcResult: Int32?, dc: UInt32?, methodResult: Int32?, milliseconds: UInt32? }
    ...
    final class Resources {
        ...
        init() throws {
            let url = try ProcessInfo.processInfo.environment["NTSD_LIB_TOURNAMENT_PREPARATION"].map { URL(fileURLWithPath:$0) }
                ?? XCTUnwrap(Bundle.module.url(forResource:"original-lib-tournament-preparation",withExtension:"json",subdirectory:"Fixtures"))
            ...
            if let parts=try? JSONDecoder().decode(Transport.self,from:envelope) {
                ...
                for name in parts.deflateParts {
                    guard !name.contains("/"),name.hasPrefix("original-lib-tournament-preparation-part") else { throw Stop.unexpected }
            ...
            let temporary = FileManager.default.temporaryDirectory.appendingPathComponent("ntsd-lib-tournament-preparation-"+UUID().uuidString+".json")
            try JSONSerialization.data(withJSONObject:projected).write(to:temporary)
            defer { try? FileManager.default.removeItem(at:temporary) }
            startup = try Base.Resources(url:temporary,expectedCases:98)
            ...
            XCTAssertEqual(corpus.cases.count,98)
    ...
    func run(_ item: Case,_ r: Resources,_ retained: inout Retained?,failure: String? = nil) throws -> Int {
        ...
        var preparationAdapter: OriginalTournamentPreparationSurfaceAdapter?
        ...
        do {
            let end=try OriginalCharacterMenuContinuation.advance(state:&prepared,memory:&memory,music:&music,resources:&resources,libraryText:&library,environment:&committed,includeTournamentBracket:true,
                ...
                tournamentPreparation:{ scene,owned,env in
                    ...
                    let adapter=OriginalTournamentPreparationSurfaceAdapter(pg,r);preparationAdapter=adapter
                    env.preparationGraphics=env.graphics
                    try OriginalTournamentPreparation.prepare(state:&scene,memory:&owned,localTime:{
                        ...
                        if pc==0x4347ba {
                            ...
                            let recorded=try r.record(XCTUnwrap(expected.records.first { $0.address==0x75000020 }))
            ...
            if let adapter=preparationAdapter {
                ...
                let actual=try XCTUnwrap(memory.allocations[0x75000020]);let expected=try XCTUnwrap(after[0x75000020])
    ...
    func testLateTournamentPreparationRollsBackWholeCall() throws {
        let r=try Resources(),item=try XCTUnwrap(r.corpus.cases.first { $0.spec.label=="controlled-preparation-0" })
        for failure in ["bitmap","allocateReplay","recording","beforeReturn"] {
            var retained: Retained?;_ = try run(item,r,&retained,failure:failure)
        }
    }
