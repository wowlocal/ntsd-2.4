...
final class OriginalPostDrawCommandsTests: XCTestCase {
    ...
    func testEntireCommandsRecoveryAndCleanupAgainstOriginalInstructions() throws {
        let url: URL
        if let directory = ProcessInfo.processInfo.environment["NTSD_POSTDRAW_COMMANDS_DIRECTORY"] {
            url = URL(fileURLWithPath: directory).appendingPathComponent("postdraw-commands.json")
        } else {
            url = try XCTUnwrap(Bundle.module.url(forResource: "original-postdraw-commands", withExtension: "json", subdirectory: "Fixtures"))
        }
        ...
        XCTAssertEqual(c.cases.count, 3898)
        XCTAssertEqual(c.instructions.count, 545)
        ...
        for item in c.cases {
            ...
            do {
                try OriginalPostDrawCommands.apply(world: &world, actors: &actors, globals: &globals, retainedSpawnSlot: &retained,
                    sse2: item.sse2 == 1, objectCount: item.count ?? 4, header: { ownHeaders[$0] },
            ...
            guard pool == item.poolSHA256, masks == item.maskSHA256, globalSHA == item.globalsSHA256,
                ...
                if let directory = ProcessInfo.processInfo.environment["NTSD_POSTDRAW_COMMANDS_DIRECTORY"] {
                    try Data(records.flatMap(\.bytes)).write(to: URL(fileURLWithPath: directory).appendingPathComponent("postdraw-commands-native-mismatch.bin"))
                    try Data(item.label.utf8).write(to: URL(fileURLWithPath: directory).appendingPathComponent("postdraw-commands-native-mismatch-label.txt"))
                }
                return
            }
            totalEvents += events.count
        }
        XCTAssertEqual(totalEvents, 3252)
        print("POSTDRAW COMMANDS", c.cases.count, "full pools and", totalEvents, "ordered events compared")
    }
    ...
    func testSecondConstructionFailureRollsBackPoolRandomAndRetainedSlot() throws {
        ...
        XCTAssertThrowsError(try OriginalPostDrawCommands.apply(world: &world, actors: &actors, globals: &globals,
            retainedSpawnSlot: &retained, sse2: false, objectCount: 2, header: { _ in header }, frame: { _, _ in frame },
