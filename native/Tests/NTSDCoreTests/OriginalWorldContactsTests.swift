...
final class OriginalWorldContactsTests: XCTestCase {
    ...
    func testWholeCollectionAndRealHelpers() throws {
        let url: URL
        if let path = ProcessInfo.processInfo.environment["NTSD_WORLD_CONTACTS_CORPUS"] { url = URL(fileURLWithPath: path) }
        else { url = try XCTUnwrap(Bundle.module.url(forResource: "original-world-contacts",withExtension: "json",subdirectory: "Fixtures")) }
        ...
        XCTAssertEqual(c.ids,[2,7,8,51]);XCTAssertEqual(c.cases.count,7925)
        ...
        for item in c.cases {
            ...
            var pass = OriginalContactPass(world: world,actors: actors,globals: globals,objectCount: item.count ?? 4,
                header: { ownHeaders[$0] },frame: { ownFrames[$0][Int($1)] },heapWord: { try memory.word($0) })
    ...
    func testRawBoxReferencesPreserveInteriorOffsetsAndDefinedMasks() throws {
        ...
        try raw.write(Int32(-17),at: 4)
        let memory = OriginalContactFrameMemory([.init(address: 0x51000000,kind: .interactions,storage: raw)])
        XCTAssertEqual(try memory.word(0x51000004),-17)
        XCTAssertThrowsError(try memory.word(0x51000000))
        XCTAssertThrowsError(try memory.word(0x5100004f))
        XCTAssertThrowsError(try memory.word(0))
    }
}
