...
final class OriginalLibTournamentSetupTests: XCTestCase {
    ...
    final class Resources {
        ...
        init() throws {
            let url = try ProcessInfo.processInfo.environment["NTSD_LIB_TOURNAMENT_SETUP"].map { URL(fileURLWithPath:$0) }
                ?? XCTUnwrap(Bundle.module.url(forResource:"original-lib-tournament-setup",withExtension:"json",subdirectory:"Fixtures"))
            ...
            if let parts=try? JSONDecoder().decode(Transport.self,from:envelope) {
                ...
                for name in parts.deflateParts {
                    guard !name.contains("/"),name.hasPrefix("original-lib-tournament-setup-part") else { throw Stop.unexpected }
            ...
            let temporary = FileManager.default.temporaryDirectory.appendingPathComponent("ntsd-lib-tournament-setup-"+UUID().uuidString+".json")
    ...
    func run(_ item: Case,_ r: Resources,_ retained: inout Retained?,failure: String? = nil) throws -> Int {
        ...
        do {
            let end=try OriginalCharacterMenuContinuation.advance(state:&prepared,memory:&memory,music:&music,resources:&resources,libraryText:&library,environment:&committed,
                ...
                musicRequest:{ _,_ in XCTFail("Music child must be skipped for Tournament menu20..25");throw Stop.unexpected
                },allocate:{ i,env in
            ...
            XCTAssertEqual(item.endSP,item.end == "tournamentPrelude" ? try XCTUnwrap(item.characterSP)-0xa14 : c.entrySP+8)
    ...
    func testTournamentSetupRetainsWholeLibraryMenu() throws {
    ...
    func testLateTournamentShuffleAndLabelRollBackCurrentCall() throws {
        let r=try Resources()
