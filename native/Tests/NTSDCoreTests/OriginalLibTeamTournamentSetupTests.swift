...
final class OriginalLibTeamTournamentSetupTests: XCTestCase {
    ...
    final class Resources {
        ...
        init() throws {
            let url = try ProcessInfo.processInfo.environment["NTSD_LIB_TEAM_TOURNAMENT_SETUP"].map { URL(fileURLWithPath:$0) }
                ?? XCTUnwrap(Bundle.module.url(forResource:"original-lib-team-tournament-setup",withExtension:"json",subdirectory:"Fixtures"))
            ...
            if let parts=try? JSONDecoder().decode(Transport.self,from:envelope) {
                ...
                for name in parts.deflateParts {
                    guard !name.contains("/"),name.hasPrefix("original-lib-team-tournament-setup-part") else { throw Stop.unexpected }
            ...
            let temporary = FileManager.default.temporaryDirectory.appendingPathComponent("ntsd-lib-team-tournament-setup-"+UUID().uuidString+".json")
            try JSONSerialization.data(withJSONObject:projected).write(to:temporary)
            defer { try? FileManager.default.removeItem(at:temporary) }
            startup = try Base.Resources(url:temporary,expectedCases:373)
            ...
            XCTAssertEqual(corpus.cases.count,373)
    ...
    func run(_ item: Case,_ r: Resources,_ retained: inout Retained?,failure: String? = nil) throws -> Int {
        ...
        do {
            let end=try OriginalCharacterMenuContinuation.advance(state:&prepared,memory:&memory,music:&music,resources:&resources,libraryText:&library,environment:&committed,
                ...
                musicRequest:{ _,_ in XCTFail("Music child must be skipped for Team Tournament menu120..125");throw Stop.unexpected
                },allocate:{ i,env in
            ...
            XCTAssertEqual(item.endSP,item.end == "teamTournamentPrelude" ? try XCTUnwrap(item.characterSP)-0xa18 : c.entrySP+8)
    ...
    func testTeamTournamentSetupRetainsWholeLibraryMenu() throws {
    ...
    func testLateTeamTournamentShuffleAndLabelRollBackCurrentCall() throws {
        let r=try Resources()
        for (label,failure) in [("shuffle-press","shuffle"),("controller-5-press","controllerLabel")] {
