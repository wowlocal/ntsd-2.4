...
final class OriginalLibTeamTournamentBracketTests: XCTestCase {
    ...
    struct Point: Decodable { let kind: String,records: [Record],eventCount: Int,storeCount: Int,seat: UInt32?,locals: [String:UInt32]? }
    ...
    final class Resources {
        ...
        init() throws {
            let url = try ProcessInfo.processInfo.environment["NTSD_LIB_TEAM_TOURNAMENT_BRACKET"].map { URL(fileURLWithPath:$0) }
                ?? XCTUnwrap(Bundle.module.url(forResource:"original-lib-team-tournament-bracket",withExtension:"json",subdirectory:"Fixtures"))
            ...
            if let parts=try? JSONDecoder().decode(Transport.self,from:envelope) {
                ...
                for name in parts.deflateParts {
                    guard !name.contains("/"),name.hasPrefix("original-lib-team-tournament-bracket-part") else { throw Stop.unexpected }
            ...
            let temporary = FileManager.default.temporaryDirectory.appendingPathComponent("ntsd-lib-team-tournament-bracket-"+UUID().uuidString+".json")
            try JSONSerialization.data(withJSONObject:projected).write(to:temporary)
            defer { try? FileManager.default.removeItem(at:temporary) }
            startup = try Base.Resources(url:temporary,expectedCases:377)
            ...
            XCTAssertEqual(corpus.cases.count,377)
    ...
    func initialize(_ item: Case,_ r: Resources) throws -> Retained {
        ...
        let layoutStart=0x44d120-base,layoutEnd=0x44d318-base
        XCTAssertEqual(Array(produced.bytes[layoutStart..<layoutEnd]),Array(globals.bytes[layoutStart..<layoutEnd]))
        XCTAssertEqual(Array(produced.defined[layoutStart..<layoutEnd]),Array(globals.defined[layoutStart..<layoutEnd]))
        XCTAssertEqual(MatchPreparationReference.digest(Data(produced.bytes[layoutStart..<layoutEnd])),OriginalTournamentLayout.sourceSHA256)
    ...
    func run(_ item: Case,_ r: Resources,_ retained: inout Retained?,failure: String? = nil) throws -> Int {
        ...
        var eventIndex=0,characterPoint=0,musicIndex=0
        ...
        func event(_ e: OriginalFrontScreenEvent,_ environment: inout Environment) throws {
            ...
            if failure == "secondHealth",e.kind == "random",e.arguments[0]==0x108 { throw Stop.injected }
            if failure == "shuffle",e.kind == "random",[UInt32(0x101),0x102].contains(e.arguments[0]),
               currentEvents.filter({ $0.kind == "random" && [UInt32(0x101),0x102].contains($0.arguments[0]) }).count == 3 { throw Stop.injected }
        ...
        do {
            let end=try OriginalCharacterMenuContinuation.advance(state:&prepared,memory:&memory,music:&music,resources:&resources,libraryText:&library,environment:&committed,includeTeamTournamentBracket:true,
                ...
                characterCheckpoint:{ point,state,buffer in
                    let points=item.points.filter { $0.kind.hasPrefix("character-") || $0.kind.hasPrefix("team-tournament-") }
                    // The extended source observer also records the four
                    // bracket guards for menu120..125. The accepted setup API
                    // emits only their entry/exit. Verify every intermediate
                    // state and prove that no original store/event intervened.
                    if point.pc==0x436f9d,try state.globals.integer(at:0x44d020-base,as:Int32.self)<126,
                       points[characterPoint].kind=="team-tournament-0x435d59" {
                        let prior=points[characterPoint-1]
                        XCTAssertEqual(prior.kind,"team-tournament-0x435c7c")
                        for kind in ["team-tournament-0x435d59","team-tournament-0x435dbc","team-tournament-0x436afd","team-tournament-0x436e5e"] {
                    ...
                    XCTAssertEqual(expected.kind,(point.pc>=0x434ab0 ? "team-tournament-0x" : "character-0x")+String(point.pc,radix:16));XCTAssertEqual(eventIndex,expected.eventCount)
                    ...
                    if failure == "assignment" && point.pc == 0x436afd { throw Stop.injected }
                    if failure == "cpuPlacement" && point.pc == 0x436747 { throw Stop.injected }
            ...
            XCTAssertEqual(item.endSP,item.end == "teamTournamentMatchPreparation" ? try XCTUnwrap(item.characterSP)-0xa18 : c.entrySP+8)
            XCTAssertEqual(characterPoint,item.points.filter { $0.kind.hasPrefix("character-") || $0.kind.hasPrefix("team-tournament-") }.count)
    ...
    func testTeamTournamentBracketRetainsWholeLibraryMenu() throws {
    ...
    func testLateTeamTournamentStagesRollBackCurrentCall() throws {
        let r=try Resources()
        for (label,failure) in [("start-press-human","startMusic"),("claim-0-press-human","assignment"),("controlled-cpu-mask0-d2","secondHealth"),("prepare-match-mixed","cpuPlacement")] {
            var retained: Retained?
            let end=try XCTUnwrap(r.corpus.cases.firstIndex { $0.spec.label==label })
            let start=r.corpus.cases[...end].lastIndex { $0.spec.chain != true }!
            for item in r.corpus.cases[start..<end] { _ = try run(item,r,&retained) }
            if r.corpus.cases[end].spec.chain != true { retained=nil }
