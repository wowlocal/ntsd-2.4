...
final class OriginalWorldCPointsTests: XCTestCase {
    ...
    private struct Case: Decodable {
        ...
        let stage: String,retainedPartnerSlot: Int32?
    ...
    func testWholeActionsPlacementAndCaller() throws {
        ...
        else { url = try XCTUnwrap(Bundle.module.url(forResource: "original-world-cpoints",withExtension: "json",subdirectory: "Fixtures")) }
        ...
        XCTAssertEqual(c.ids,[2,122,123,10]);XCTAssertEqual(c.cases.count,2681)
        ...
        for item in c.cases {
            ...
            do {
                ...
                if item.stage == "placement" || item.stage == "caller" { try pass.placement() }
                if item.stage == "cleanup" || item.stage == "caller" { try pass.cleanup() }
                if item.stage == "caller" {
                    try OriginalWorldLinks.apply(world: world,actors: &pass.actors,globals: &pass.globals,sse2Conversion: item.sse2 == 1,
            ...
            actors = pass.actors;globals = pass.globals
        ...
        print("WORLD CPOINTS",c.cases.count,"whole pools compared")
    }
}
