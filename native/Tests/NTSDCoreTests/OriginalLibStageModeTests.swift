...
final class OriginalLibStageModeTests: XCTestCase {
    ...
    final class Resources {
        ...
        init() throws {
            let url = try ProcessInfo.processInfo.environment["NTSD_LIB_STAGE_MODE"].map { URL(fileURLWithPath:$0) }
                ?? XCTUnwrap(Bundle.module.url(forResource:"original-lib-stage-mode",withExtension:"json",subdirectory:"Fixtures"))
            ...
            if let parts=try? JSONDecoder().decode(Transport.self,from:envelope) {
                ...
                for name in parts.deflateParts {
                    guard !name.contains("/"),name.hasPrefix("original-lib-stage-mode-part") else { throw Stop.unexpected }
            ...
            let temporary = FileManager.default.temporaryDirectory.appendingPathComponent("ntsd-lib-stage-mode-"+UUID().uuidString+".json")
            try JSONSerialization.data(withJSONObject:projected).write(to:temporary)
            defer { try? FileManager.default.removeItem(at:temporary) }
            startup = try Base.Resources(url:temporary,expectedCases:279)
            ...
            XCTAssertEqual(corpus.cases.count,279)
    ...
    func run(_ item: Case,_ r: Resources,_ retained: inout Retained?,failure: String? = nil) throws -> Int {
        ...
        func event(_ e: OriginalFrontScreenEvent,_ environment: inout Environment) throws {
            ...
            if failure == "stageRandom",e.kind == "random",e.arguments[0] == 0xd8,
               environment.front.suffix(environment.front.count-firstFrontCount).filter({ $0.kind == "random" && $0.arguments[0] == 0xd8 }).count == 3 { throw Stop.injected }
            if failure == "stageText",e.kind == "textOut",e.strings == [Array("Survival".utf8)] { throw Stop.injected }
    ...
    func testStageModeRetainsWholeLibraryMenu() throws {
    ...
    func testLateStageRandomAndTextRollBackCurrentCall() throws {
        let r=try Resources()
        for (label,failure) in [("cpu2-character-press","stageRandom"),("stage-next-4-release","stageText")] {
