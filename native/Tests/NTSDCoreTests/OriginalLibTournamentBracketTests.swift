...
final class OriginalLibTournamentBracketTests: XCTestCase {
    ...
    final class Resources {
        ...
        init() throws {
            let url = try ProcessInfo.processInfo.environment["NTSD_LIB_TOURNAMENT_BRACKET"].map { URL(fileURLWithPath:$0) }
                ?? XCTUnwrap(Bundle.module.url(forResource:"original-lib-tournament-bracket",withExtension:"json",subdirectory:"Fixtures"))
            ...
            if let parts=try? JSONDecoder().decode(Transport.self,from:envelope) {
                ...
                for name in parts.deflateParts {
                    guard !name.contains("/"),name.hasPrefix("original-lib-tournament-bracket-part") else { throw Stop.unexpected }
            ...
            let temporary = FileManager.default.temporaryDirectory.appendingPathComponent("ntsd-lib-tournament-bracket-"+UUID().uuidString+".json")
            try JSONSerialization.data(withJSONObject:projected).write(to:temporary)
            defer { try? FileManager.default.removeItem(at:temporary) }
            startup = try Base.Resources(url:temporary,expectedCases:250)
            ...
            XCTAssertEqual(corpus.cases.count,250)
    ...
    func testTournamentBracketRetainsWholeLibraryMenu() throws {
    ...
    func testLateTournamentMusicAndAssignmentRollBackCurrentCall() throws {
        let r=try Resources()
        for (label,failure) in [("start-press-human","startMusic"),("claim-first-press-human","assignment")] {
