#!/usr/bin/env python3
"""Independently verify complete initialized-return raw/packed bytes and pins.
...
def main():
    old=json.loads((ROOT/'build/research/gameplay-result-layout-fixture-pins.json').read_bytes())
    current=json.loads((ROOT/'build/research/gameplay-return-fixture-pins.json').read_bytes())
    actual={p.name:digest(p.read_bytes()) for p in FIXTURES.glob('*.json')}
    assert len(old)==186 and all(current[k]==v and actual[k]==v for k,v in old.items())
    assert all(actual[k]==v for k,v in current.items())
    checks=[]
    for suffix in ('','-control'):
        name='gameplay-return'+suffix
    ...
    (ROOT/'build/research/gameplay-return-artifact-verification.json').write_text(json.dumps(result,indent=2)+'\n')
