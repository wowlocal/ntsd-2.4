...
def main(task):
    assert task.name == 'application-host-loading-correction3-20260922'
    ...
    try:
        ...
        assert len(selected)==42 and selected[:35]==[r['method'] for r in queue['completed']]
        assert len(queue['completed'])==35 and [r['passed'] for r in queue['completed']]==[True]*34+[False]
        assert all(not (task/f'test-{i:02d}.job.json').exists() for i in range(36,43))
        tests = []
        for record in queue['completed']:
            ...
            if record['passed']:
                ...
                assert child['guardReason']=='resident bound' and child['peakTreeRSS']>child['residentGuardBytes']==8*2**30
        ...
        assert changes == [context['changedPath']] and context['correctionRound'] == 3
        proof = read(task/'patch-roundtrip1.json')
        assert proof['allChangedBytesVerified'] and proof['changedFiles'] == changes
        root_patch = ROOT/'docs/evidence/application-host-loading-correction3.patch'
        ...
        write(task/'verification1.json', dict(UTC=now(), protectedPins=len(context['protected']), rootNativeFiles=1034,
            ...
            all42Passed=False, passedMethods=34, guardedMethods=1, unstartedMethods=selected[35:], tests=tests,
        ...
        tools = ['prepare_application_host_loading_validation4.py', 'run_application_host_loading_validation4.py',
                 'verify_application_host_loading_package4.py', 'run_application_host_loading_tests4.py', Path(__file__).name]
        for name in tools:
        ...
        publication = dict(schema='ntsd-host-loading-validation-v1', UTC=now(), status='34-passed-resident-guard-7-unstarted',
            ...
            corrections=[dict(round=3, changedFiles=changes, patch=dict(path=str(task/'correction1.patch'), **pin(task/'correction1.patch')), roundtrip=pin(task/'patch-roundtrip1.json'), previousFailure='docs/evidence/application-host-loading-completion.json', correction='Bool presence assertion; full semantic comparisons retained', expectedSourceUnchanged=True, independentReview=False)],
        ...
        (ROOT/'docs/evidence/application-host-loading-correction3.json').write_bytes((task/'publication1.json').read_bytes())
        ...
        (ROOT/'docs/evidence/application-host-loading-correction3-close.json').write_bytes((task/'external-close1.json').read_bytes())
        job.update(status='terminal', exitCode=0)
        print(json.dumps(dict(all42Passed=False, passed=34, guarded=1, unstarted=7, testSeconds=publication['tests']['totalTestProcessSeconds'],
