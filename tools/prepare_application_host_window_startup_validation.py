#!/usr/bin/env python3
"""Prepare exact Host window-startup validation with the existing guarded host tools."""
...
NAME='application-host-window-startup-validation-20260926'
TASK=Path('/Volumes/X5/ntsd-2.4-research/01a0dc49-738f-7972-8fb0-e98fb2f34408')/NAME
BASE=(ROOT/'build/research/application-host-window-startup-20260926').resolve()
OLD=(ROOT/'build/research/application-window-exchange-validation-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_HOST_WINDOW_STARTUP_VALIDATION_PLAN.md'
...
def generate(name):
 ...
 if name==NAMES[0]:
  new=once(new,"r'build1|test-(0[1-9]|10)'","r'build1|test-(0[1-9]|1[0-9]|2[0-8])'")
  marker='    def identity(pid):';assert old[old.index(marker):]==new[new.index(marker):]
 elif name==NAMES[1]:
  new=once(new,'10-method','28-method');new=once(new,'== 10','== 28');new=once(new,'all10Passed','all28Passed')
  begin,end='            done = re.findall(',"            job['completed'].append("
  assert old[old.index(begin):old.index(end)]==new[new.index(begin):new.index(end)]
 else:
  new=once(new,"'NTSDCore':193","'NTSDCore':194")
  new=once(new,"'NTSDCoreTests':273","'NTSDCoreTests':274")
...
def main():
 ...
 try:
  ...
  rows=read(BASE/'candidate1-inputs.json');assert len(rows)==2246
  ...
  assert len(methods)==len(set(methods))==28 and len(new)==5 and len(old)==23 and methods==new+old
  ...
  save('method-limits1.json',dict(methods=limits,queueSeconds=14400,inheritedMethods=23,priorLimits=record(OLD/'method-limits1.json'),priorContextLimits=record(priorContext/'method-limits1.json'),priorSelection=record(OLD/'selected-methods1.json'),precedingComparisonsAreSeparate=True))
  ...
  protected=[PLAN,Path(__file__),BASE/'publication1.json',BASE/'external-close1.json',BASE/'candidate1-inputs.json',BASE/'selected-methods1.json',OLD/'publication1.json',OLD/'external-close1.json',OLD/'method-limits1.json',ROOT/'AGENTS.md',ROOT/'AGENTS_HISTORY_2026-09-12.md',ROOT/'docs/research/WORKFLOW.md',ROOT/'docs/research/TASK_TEMPLATE.md',ROOT/'docs/research/APPLICATION_HOST_WINDOW_STARTUP.md',ROOT/'docs/research/APPLICATION_HOST_WINDOW_STARTUP_PLAN.md',ROOT/'docs/research/WINDOW_INITIALIZATION.md',ROOT/'docs/research/WINMAIN_STARTUP.md',ROOT/'tools/prepare_application_window_exchange_validation.py',ROOT/'docs/evidence/codex-safety-incidents-2026-09-12.json',ROOT/'build/research/application-host-gameplay-completion-20260926/reader-recovery1.json',TASK/'adaptation1.json',TASK/'method-limits1.json',source/'capture1/source.job.json']+[TASK/'tools'/n for n in NAMES]
  context=dict(schema='ntsd-host-window-startup-validation-v1',UTC=now(),head=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),protected=[record(p) for p in protected],candidateSource=str(BASE/'candidate1'),rootManifest=basecontext['rootManifest'],priorManifest=basecontext['baselineManifest'],priorCandidate=str(OLD/'candidate1'),sourceTask=str(source),sourceCodePins=basecontext['sourceCodePins'],startFree=free,physicalAllowanceBytes=28*2**30,stopObservedDecrease=26*2**30,originalExternalReserve=40*2**30,originalInternalReserve=6*2**30,sourceCommitment=17*2**30,fullCloneVerified=True,cloneFiles=2246,distinctRegularInodes=True,swiftVersion=version,sdk=sdk,environment=env,gitStatus=subprocess.check_output(['git','status','--porcelain'],text=True))
  ...
  pattern=r'build1|test-(0[1-9]|1[0-9]|2[0-8])'
  assert all(re.fullmatch(pattern,p) for p in ['build1']+[f'test-{i:02d}' for i in range(1,29)])
  invalid=['build0','build2','test-00','test-29','test-99','test-1','test-01x'];assert not any(re.fullmatch(pattern,p) for p in invalid)
  save('runner-controls1.json',dict(UTC=now(),monitorBodyUnchanged=True,resultPredicatesUnchanged=True,cwdPositive=1,cwdNegative=4,compilerPositive=1,compilerNegative=2,transientPositive=1,transientNegative=1,phasePositive=29,phaseNegative=7,exactMethods=28,oldMethodLimitsUnchanged=True,nativeExecuted=False,originalExecuted=False,independentReview=False))
  job.update(status='terminal',exitCode=0,files=2246,preparedMethods=28,buildConfig=record(TASK/'build1-config.json'))
