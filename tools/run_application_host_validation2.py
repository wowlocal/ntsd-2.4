...
def main(config_path,config_sha):
    ...
    assert P.name=='application-host-transaction-validation-20260922'
    checked(config_path,{'sha256':config_sha})
    config=read(config_path);context=read(P/'context1.json');phase=config['phase']
    assert re.fullmatch(r'build[123]|test-(0[1-9]|1[0-9]|2[0-5])',phase)
    ...
    for item in read(P/'runner-inputs2.json'):checked(Path(item['path']),item)
    ...
    j=dict(config,status='prepared',UTC=now(),config=pin(config_path),launcher=pin(Path(__file__)),runnerInputs=pin(P/'runner-inputs2.json'),
