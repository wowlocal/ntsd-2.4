#!/usr/bin/env python3
"""Read-only full trace audit of retained Team Tournament roster/controller/pair shuffle/settings and ret4 or prelude boundaries.
...
See docs/research/LIB_TEAM_TOURNAMENT_SETUP_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-team-tournament-setup-finite-inputs1.json').read_bytes())
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  ...
  previous=c
  assert c['cw']==0x23f and c['end'] in ('returned','teamTournamentPrelude')
  ...
  if c['end']=='returned':
   ...
   assert c['end']=='teamTournamentPrelude' and c['endPC']==0x435a58 and c['endSP']==c['characterSP']-0xa18
   assert [h['entry'] for h in c['pending']]==[0x429730,0x434ab0] and all(h['pop']==12 for h in c['pending'])
   assert c['points'][-1]['kind']=='team-tournament-0x435a58' and '0x435a58' not in c['instructions']
  if True:
 ...
 assert ends=={'returned':372,'teamTournamentPrelude':1}
 ...
 for c in d['cases']:
  ...
  if not c['spec'].get('chain'):
   assert before_g[0x44d320-0x44d000:0x44d322-0x44d000]==exe_bytes(0x44d320,2)==b'x\0'
  rng=[h for h in c['helpers'] if h['entry']==0x417170]
  if any(h['stream'] in (0x101,0x102) for h in rng):
   assert [h['stream'] for h in rng]==[0x101,0x102]*50;shuffle_calls+=1
   order=list(struct.unpack_from('<8i',before_g,0x44d0e0-0x44d000));teams=list(struct.unpack_from('<8i',before_g,0x44d100-0x44d000))
   for a,b in zip(rng[::2],rng[1::2]):
    first,second=a['result']*2,b['result']*2
    for values in (order,teams):
     for member in (0,1):values[first+member],values[second+member]=values[second+member],values[first+member]
    stats['pairSwaps']+=1
   assert order==list(struct.unpack_from('<8i',g,0x44d0e0-0x44d000)) and sorted(order)==list(range(8))
   assert teams==list(struct.unpack_from('<8i',g,0x44d100-0x44d000))
   assert all(order[i]%2==0 and order[i+1]==order[i]+1 and teams[i]==teams[i+1]==order[i]//2+1 for i in range(0,8,2))
  for h in c['helpers']:
   if h['entry']==0x417170:
    assert h['stream'] in (1,0x101,0x102,0x103);streams[h['stream']]+=1
    ...
    else:assert h['range']==(8 if h['stream']==1 else 4)
   ...
   if h['entry']==0x401290 and h['textAddress']==0x44d320:
    ...
    assert g[0x44d321-0x44d000]==0;controller_labels[chr(h['text'][0])]+=1
  if label=='shuffle-wait-7'+suffix:assert word(0x44d020)==122 and word(0x451404)==11 and word(0x451408)==0
  if label=='reselect-press'+suffix:assert word(0x44d020)==120
  if not c['spec'].get('chain') or label=='dialog-back-press'+suffix or label=='reselect-held'+suffix:
   assert [word(0x44d100+i*4) for i in range(8)]==[1,1,2,2,3,3,4,4]
  if label=='reselect-held'+suffix:assert word(0x44d020)==121 and word(0x451414)==0 and word(0x451410)==0
  ...
  if label=='start':assert word(0x44d020)==125 and word(0x44d024)==21%(d['arenas']['count']+2) and word(0x44d028)==0 and word(0x451340)==2 and [word(0x44d0c0+i*4) for i in range(2)]==[17,21]
  if label=='quit-control':assert word(0x44d020)==10 and word(0x451340)==0
 assert shuffle_calls==22 and streams[0x101]==streams[0x102]==1100
 assert streams[0x103]==stats['liveCandidateLists'] and streams[0x103]>0
