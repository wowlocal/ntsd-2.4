#!/usr/bin/env python3
"""Read-only full trace audit of retained Stage/mode1 count/CPU/settings and ret4 or prelude boundaries.
...
See docs/research/LIB_STAGE_MODE_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 previous=None
 assert len(d['cases'])==len(d['installations'])==279
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-stage-mode-finite-inputs.json').read_bytes())
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  ...
  assert len(state)==746+len(d['arenas']['entries'])+1 and len(alive)==20
  if c['spec'].get('chain'):
  ...
  previous=c
  assert c['cw']==0x23f and c['end'] in ('returned','matchPrelude')
  ...
  for step in range(len(c['writes'])+1):
   for point in points.pop(step,[]):
    assert len(point['records'])==746+len(d['arenas']['entries'])+1
  ...
  if c['end']=='returned':
   ...
   assert c['end']=='matchPrelude' and c['endPC']==0x42cf8a and c['endSP']==c['characterSP']
   assert len(c['pending'])==1 and c['pending'][0]['entry']==0x429730 and c['pending'][0]['pop']==12
   assert c['points'][-1]['kind']=='character-0x42cf8a' and '0x42cf8a' not in c['instructions']
  if True:
   ...
   for point in points:
    assert point['sp']==c['characterSP'];stack=decode_record(next(x for x in point['records'] if x['address']==d['stackAddress']))
 ...
 for c in d['cases']:
  ...
  if c['end']=='matchPrelude':
   g=decode_record(next(x for x in c['after'] if x['address']==0x44d000))[0];word=lambda p:struct.unpack_from('<i',g,p-0x44d000)[0]
   assert [word(0x451248+i*4) for i in range(2)]==[17,21] and [word(0x451288+i*4) for i in range(4)]==([3,3,0,0] if c['spec']['control'] else [3,3,13,13])
   assert word(0x4512c8)==3 and word(0x450b94)==(50 if c['spec']['control'] else 0) and word(0x44d070)==(0 if c['spec']['control'] else 2)
 assert ends=={'returned':277,'matchPrelude':2}
 stage_labels=set();streams=collections.Counter();text_origins=collections.Counter()
 for c in d['cases']:
  ...
  assert word(0x451160)==1
  for actor in c['actorAddresses'][:20]:
   record=decode_record(next(x for x in c['after'] if x['address']==actor))[0]
   assert struct.unpack_from('<i',record,0x364)[0]==1
  if label=='count-left-wrap-press'+suffix:assert word(0x44d070)==6
  if label=='count-right-wrap-press'+suffix:assert word(0x44d070)==0
  if label in ('count-confirm-press-control','recount-confirm-press-control'):
   assert word(0x4512c8)==3 and word(0x44d070)==0;stats['zeroCPUConfirmations']+=1
  if label=='cpu1-character-press':assert word(0x451288+2*4)==13 and word(0x4511fc)==1
  if label=='cpu2-back-previous-press':assert word(0x451288+2*4)==11 and word(0x4511fc)==0
  if label=='cpu1-cancel-count-press':assert word(0x4512c8)==1
  if label.startswith('reselect-press'):
   assert [word(0x451288+i*4) for i in range(8)]==[0]*8 and [word(0x451268+i*4) for i in range(8)]==[1]*8
   assert word(0x44d078)==150 and word(0x4512c8)==0;stats['reselectCommands']+=1
  draws=[e for e in c['events'] if e['kind']=='random']
  for e in draws:streams[e['arguments'][0]]+=1
  if 'held' in label:assert not draws,(label,'held input repeated RNG')
  # Reconstruct each actual candidate list from live selected globals immediately
  # before the original RNG helper, not from after-state or producer's event.
  before_g=decode_record(next(x for x in c['before'] if x['address']==0x44d000))[0]
  for h in c['helpers']:
   if h['entry']==0x417170:
    assert h['stream'] in (0xd7,0xd8,0xea)
    ...
    selected=struct.unpack_from('<8i',live,0x451248-0x44d000)
    expected=[x['ordinal'] for x in d['roster']['entries'] if x['ordinal']>=1 and x['type']==0 and x['id']<30 and x['ordinal'] not in selected]
    event=c['events'][h['eventStart'] if h['stream']==0xd7 else h['eventStart']-1];assert event['kind']=='candidates' and event['arguments'][1:]==expected
    assert h['range']==len(expected) and 0<=h['result']<len(expected)
    stats['liveCandidateLists']+=1
   if h['entry']==0x401290 and h['arguments'][4]==91 and h['text'] in [list(bytes([x])) for x in range(49,54)]+[list(b'Survival')]:
    value=bytes(h['text']);stage_labels.add(value.decode())
    assert h['textAddress']==c['characterSP']+0x83c
    assert h['arguments'][3]==(174 if value==b'Survival' else 194)
    # All requested local string bytes are defined by actual writes this call.
    start=h['textAddress'];produced={}
    for w in c['writes'][:h['firstStore']]:
     for i,v in enumerate(bytes.fromhex(w['bytes'])):
      if start<=w['address']+i<start+len(value)+1:produced[w['address']+i]=v
    assert bytes(produced[start+i] for i in range(len(value)+1))==value+b'\0'
    text_origins[value.decode()]+=1
  if label.startswith('music-off-') and '-press' in label:assert word(0x44f18c)==-1 and word(0x44d010)==0
  if label.startswith('music-on-') and '-press' in label:assert word(0x44f18c)==0 and word(0x44d010)==1
 assert stage_labels=={'1','2','3','4','5','Survival'}
 assert streams=={0xd7:12,0xd8:10,0xea:22} and stats['liveCandidateLists']==44
 assert stats['zeroCPUConfirmations']==2 and stats['reselectCommands']==2
 stats['stageLocalStrings']=sum(text_origins.values())


 return dict(scope=__doc__,stageLabels=dict(text_origins),randomStreams={hex(k):v for k,v in streams.items()},undefinedBitmapReads=[dict(pc=hex(pc),bytes=n,reads=count) for (pc,n),count in bitmap_unknown.items()],rawBytes=len(input_raw),rawSHA256=sha(input_raw),cases=len(d['cases']),ends=dict(ends),**stats,events=dict(events),instructionStarts=len(pcs),exeInstructionStarts=sum(int(p,16)<0x36000000 for p in pcs),libraryInstructionStarts=sum(0x36000000<=int(p,16)<0x36005000 for p in pcs),crtInstructionStarts=sum(int(p,16)>=0x78130000 for p in pcs),blobCount=len(blobs),decodedBlobBytes=sum(map(len,blobs.values())),writeMaskFalseReadsWithPinnedEXEProvenance=[dict(pc=hex(pc),address=hex(p),bytes=n,reads=count) for (pc,p,n),count in unknown.items()],nativeCompared=False,windowsVerified=False)
