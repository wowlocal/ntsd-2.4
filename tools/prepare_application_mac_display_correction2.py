#!/usr/bin/env python3
"""Prepare exact display observation correction validation with the existing guarded host tools."""
...
NAME='application-mac-display-correction2-20260926'
TASK=Path('/Volumes/X5/ntsd-2.4-research/01a0dc49-738f-7972-8fb0-e98fb2f34408')/NAME
BASE=(ROOT/'build/research/application-mac-display-correction1-20260926').resolve()
OLD=BASE
CONTROL=(ROOT/'build/research/application-mac-view-capture-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_MAC_DISPLAY_CORRECTION2_PLAN.md'
...
def generate(name):
 ...
 new=new.replace('3[0-4]','3[0-5]').replace('34-method','35-method').replace('== 34','== 35').replace('all34Passed','all35Passed').replace("'NTSDCoreTests':279","'NTSDCoreTests':280").replace("'NTSDMacPlatform':5","'NTSDMacPlatform':6")
 if name==NAMES[0]:
...
def main():
 ...
 for key in ['candidateManifest','context','verification','metadataArchive']:checked(Path(pub[key]['path']),pub[key])
 checked(Path(pub['artifactArchive']['manifest']['path']),pub['artifactArchive']['manifest'])
 basecontext=read(BASE/'context1.json')
 for parent,names in [(BASE,['prepare1.job.json','build1.job.json','finalize1.job.json']),(CONTROL,['prepare1.job.json','build1.job.json','build2.job.json','finalize1.job.json'])]:
  for n in names:
   j=read(parent/n);assert j['status']=='terminal' and j['exitCode']==0 and absent(j['pid'])
 for parent,n,code in [(BASE,'tests1-queue.job.json',1),(CONTROL,'probe1.job.json',-6),(CONTROL,'probe2.job.json',1)]:
  j=read(parent/n);assert j['status']=='terminal' and j['exitCode']==code and absent(j['pid'])
 assert read(CONTROL/'external-close1.json')['taskFrozen']
 ...
 try:
  ...
  rows=read(BASE/'candidate1-inputs.json');assert len(rows)==2261
  ...
  changed='native/Sources/NTSDMacPlatform/OriginalMacWindowBackend.swift'
  destination=TASK/'candidate1'/changed;before=destination.read_text()
  replacements=[
   ('    public func captureView(_ token: UInt32) throws -> NSBitmapImageRep {','    public func captureView(_ token: UInt32) throws -> OriginalMacViewCapture {'),
   ('        view.cacheDisplay(in:view.bounds,to:bitmap); return bitmap','        view.cacheDisplay(in:view.bounds,to:bitmap); return try OriginalMacViewCapture(bitmap)'),
   ('            window.isReleasedWhenClosed = false; window.title', '            window.colorSpace = .sRGB // Match the explicitly sRGB native image/backing policy.\n            window.isReleasedWhenClosed = false; window.title')]
  after=before
  for a,b in replacements:after=once(after,a,b)
  destination.write_text(after)
  helper='native/Sources/NTSDMacPlatform/OriginalMacViewCapture.swift'
  test='native/Tests/NTSDCoreTests/OriginalMacDisplayColorTests.swift'
  shutil.copy2(CONTROL/'candidate2/OriginalMacViewCapture.swift',TASK/'candidate1'/helper)
  shutil.copy2(ROOT/'tools/OriginalMacDisplayColorTests.swift',TASK/'candidate1'/test)
  patch=''.join(difflib.unified_diff(before.splitlines(True),after.splitlines(True),fromfile='a/'+changed,tofile='b/'+changed))
  for n in [helper,test]:patch+=''.join(difflib.unified_diff([], (TASK/'candidate1'/n).read_text().splitlines(True),fromfile='/dev/null',tofile='b/'+n))
  (TASK/'correction1.patch').write_text(patch)
  save('correction1.json',dict(path=changed,before=record(BASE/'candidate1'/changed),after=record(destination),replacements=replacements,newFiles=[helper,test],helperSource=record(CONTROL/'candidate2/OriginalMacViewCapture.swift'),testSource=record(ROOT/'tools/OriginalMacDisplayColorTests.swift'),captureContractChanged=True,nativeWindowColorPolicy='sRGB',allOldTestsUnchanged=True))
  names=sorted([r['path'] for r in rows]+[helper,test]);assert len(names)==2263
  current=[dict(path=n,**pin(TASK/'candidate1'/n)) for n in names]
  mapped={r['path']:r for r in current};assert [r['path'] for r in rows if mapped[r['path']]!=r]==[changed]
  save('candidate1-inputs.json',current)
  for src,name in [(PLAN,'plan1.md'),(Path(__file__),'prepare1.py')]:shutil.copy2(src,TASK/name)
  selection=read(BASE/'selected-methods1.json');newmethod='OriginalMacDisplayColorTests/testWholeWindowKeepsSRGBPatternsAndOwnedCapture'
  selection['methods']=[newmethod]+selection['methods'];selection['newMethods']=[newmethod]+selection['newMethods']
  save('selected-methods1.json',selection)
  methods=selection['methods'];new=selection['newMethods'];old=selection['retainedMethods']
  assert len(methods)==len(set(methods))==35 and len(new)==5 and len(old)==30 and methods==new+old
  previous=read(OLD/'method-limits1.json')['methods'];assert set(previous)==set(methods[1:])
  previous[newmethod]=dict(seconds=600,residentBytes=8*2**30,priorOrdinal=None)
  for m in methods:
  ...
  save('method-limits1.json',dict(methods=previous,queueSeconds=14400,inheritedMethods=34,priorLimits=record(OLD/'method-limits1.json'),priorSelection=record(OLD/'selected-methods1.json'),precedingComparisonsAreSeparate=True))
  ...
  protected=[TASK/'correction1.json',TASK/'correction1.patch',TASK/'baseline-inputs1.json',OLD/'failure-diagnosis1.json',OLD/'build1.job.json',PLAN,Path(__file__),BASE/'publication1.json',BASE/'external-close1.json',BASE/'candidate1-inputs.json',BASE/'selected-methods1.json',OLD/'publication1.json',OLD/'external-close1.json',OLD/'method-limits1.json',ROOT/'AGENTS.md',ROOT/'AGENTS_HISTORY_2026-09-12.md',ROOT/'docs/research/WORKFLOW.md',ROOT/'docs/research/TASK_TEMPLATE.md',ROOT/'docs/research/APPLICATION_MAC_DISPLAY.md',ROOT/'docs/research/APPLICATION_MAC_DISPLAY_PLAN.md',ROOT/'docs/research/WINDOW_INITIALIZATION.md',ROOT/'docs/research/WINMAIN_STARTUP.md',ROOT/'tools/prepare_application_host_window_inspection_validation.py',ROOT/'docs/evidence/codex-safety-incidents-2026-09-12.json',ROOT/'build/research/application-host-gameplay-completion-20260926/reader-recovery1.json',TASK/'adaptation1.json',TASK/'method-limits1.json',source/'capture1/source.job.json']+[TASK/'tools'/n for n in NAMES]+[CONTROL/'publication1.json',CONTROL/'external-close1.json',CONTROL/'failure-diagnosis1.json',CONTROL/'failure-diagnosis2.json',CONTROL/'candidate2/OriginalMacViewCapture.swift',ROOT/'docs/research/APPLICATION_MAC_VIEW_CAPTURE.md',ROOT/'tools/OriginalMacDisplayColorTests.swift']
  context=dict(schema='ntsd-mac-display-validation-v1',UTC=now(),head=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),protected=[record(p) for p in protected],candidateSource=str(BASE/'candidate1'),rootManifest=basecontext['rootManifest'],priorManifest=basecontext['priorManifest'],priorCandidate=basecontext['priorCandidate'],failedCandidate=str(OLD/'candidate1'),baselineManifest=record(TASK/'baseline-inputs1.json'),sourceTask=str(source),sourceCodePins=basecontext['sourceCodePins'],startFree=free,physicalAllowanceBytes=28*2**30,stopObservedDecrease=26*2**30,originalExternalReserve=40*2**30,originalInternalReserve=6*2**30,sourceCommitment=17*2**30,fullCloneVerified=True,cloneFiles=2261,distinctRegularInodes=True,swiftVersion=version,sdk=sdk,environment=env,gitStatus=subprocess.check_output(['git','status','--porcelain'],text=True))
  ...
  pattern=r'build1|test-(0[1-9]|[12][0-9]|3[0-5])'
  assert all(re.fullmatch(pattern,p) for p in ['build1']+[f'test-{i:02d}' for i in range(1,36)])
  invalid=['build0','build2','test-00','test-36','test-99','test-1','test-01x'];assert not any(re.fullmatch(pattern,p) for p in invalid)
  save('runner-controls1.json',dict(UTC=now(),monitorBodyUnchanged=True,resultPredicatesUnchanged=True,cwdPositive=1,cwdNegative=4,compilerPositive=1,compilerNegative=2,transientPositive=1,transientNegative=1,phasePositive=36,phaseNegative=7,exactMethods=35,oldMethodLimitsUnchanged=True,nativeExecuted=False,originalExecuted=False,independentReview=False))
  job.update(status='terminal',exitCode=0,files=2263,preparedMethods=len(methods),buildConfig=record(TASK/'build1-config.json'))
