#!/usr/bin/env python3
"""Read-only audit of retained character catalog/menu selection and actual returns.
...
See docs/research/LIB_CHARACTER_ROSTER_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 previous=None
 assert len(d['cases'])==len(d['installations'])==70
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-character-roster-finite-inputs.json').read_bytes())
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  ...
  assert len(state)==746 and len(alive)==20
  if c['spec'].get('chain'):
 ...
 probe=json.loads((path.parent/'lib-character-roster-probe1.json').read_bytes());assert probe['cases']==d['cases'][:2]
 ...
 for c in (d['cases'][34],d['cases'][69]):
  states={x['address']:decode_record(x)[0] for x in c['after']};g=states[0x44d000];word=lambda p:struct.unpack_from('<i',g,p-0x44d000)[0]
  assert [word(0x451248+i*4) for i in range(2)]==[17,21] and [word(0x451288+i*4) for i in range(2)]==[3,3]
  assert word(0x4512c8)==0 and word(0x44d078)==115
