...
NAME='application-mac-display-correction1-20260926'
TASK=Path('/Volumes/X5/ntsd-2.4-research/01a0dc49-738f-7972-8fb0-e98fb2f34408')/NAME
BASE=(ROOT/'build/research/application-mac-display-20260926').resolve()
OLD=(ROOT/'build/research/application-mac-display-validation-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_MAC_DISPLAY_CORRECTION1_PLAN.md'
...
def main():
 ...
 for parent,names in [(BASE,['prepare1.job.json','parse2.job.json','finalize1.job.json']),(OLD,['prepare1.job.json','finalize-failure1.job.json'])]:
  for n in names:
 ...
 try:
  ...
  old_expression='        let known = s?.storage.map { Array(UnsafeBufferPointer(start:$0.known,count:$0.count)).filter { $0 != 0 }.count } ?? 0\n'
  new_expression='        let known: Int\n        if let storage = s?.storage {\n            known = Array(UnsafeBufferPointer(start:storage.known,count:storage.count)).filter { $0 != 0 }.count\n        } else { known = 0 }\n'
  assert before.count(old_expression)==1
  after=before.replace(old_expression,new_expression);destination.write_text(after)
  assert after.replace(new_expression,old_expression)==before
  ...
  save('correction1.json',dict(path=changed,before=record(BASE/'candidate1'/changed),after=record(destination),oldExpression=old_expression,newExpression=new_expression,onlyObservationOptionalCountChanged=True))
  ...
  assert len(methods)==len(set(methods))==34 and len(new)==4 and len(old)==30 and methods==new+old
  previous=read(OLD/'method-limits1.json')['methods'];assert set(previous)==set(methods)
  for m in methods:
  ...
  pattern=r'build1|test-(0[1-9]|[12][0-9]|3[0-4])'
  assert all(re.fullmatch(pattern,p) for p in ['build1']+[f'test-{i:02d}' for i in range(1,35)])
  invalid=['build0','build2','test-00','test-35','test-99','test-1','test-01x'];assert not any(re.fullmatch(pattern,p) for p in invalid)
  save('runner-controls1.json',dict(UTC=now(),monitorBodyUnchanged=True,resultPredicatesUnchanged=True,cwdPositive=1,cwdNegative=4,compilerPositive=1,compilerNegative=2,transientPositive=1,transientNegative=1,phasePositive=35,phaseNegative=7,exactMethods=34,oldMethodLimitsUnchanged=True,nativeExecuted=False,originalExecuted=False,independentReview=False))
  job.update(status='terminal',exitCode=0,files=2261,preparedMethods=len(methods),buildConfig=record(TASK/'build1-config.json'))
