...
def main(config_path,config_sha):
    ...
    assert P.name=='application-host-match-completion-20260922'
    checked(config_path,{'sha256':config_sha})
    config=read(config_path);context=read(P/'context1.json');phase=config['phase']
    A=Path(context['artifactRoot']).resolve()
    assert A.name=='application-host-match-correction1-20260922' and A.parent==P.parent
    assert config['artifactRoot']==str(A) and read(A/'external-close1.json')['taskFrozen']
    assert re.fullmatch(r'test-(2[6-9]|[34][0-9]|5[0-3])',phase)
    assert config_path.name==phase+'-config.json'
    checked(P/'context1.json',{'sha256':config['contextSHA256']})
    checked(A/'candidate1-inputs.json',{'sha256':config['candidateManifestSHA256']})
    ...
    for item in read(A/'candidate1-inputs.json'):checked(A/'candidate1'/item['path'],item)
    assert config['environment']==context['environment']
    assert set(config['environment'])<={'HOME','USER','LOGNAME','LANG','LC_ALL','PATH','TMPDIR'}
    assert config['cwd']==str(A/'candidate1') and config['environment']['TMPDIR']==str(P/'tmp')
    assert config['kind']=='test'
    package=read(A/'package-check1.json');assert package['allInputBytesVerified']
    checked(Path(package['binary']['path']),package['binary'])
    for item in package['resourceFiles']:checked(Path(item['path']),item)
    assert read(A/'build1.job.json')['exitCode']==0 and package['buildPhase']=='build1'
    selection=read(A/'selected-methods1.json')['methods'];index=int(phase[-2:])-1
    ...
    ordinal=index+1
    assert config['residentGuardBytes']==(16 if ordinal>=51 else 12 if ordinal in (26,28,35,36,37) or 43<=ordinal<=50 else 8)*2**30
    assert config['timeoutSeconds']==(900 if ordinal>=51 else 600)
    ...
    try:
        with (P/(phase+'.log')).open('xb')as log,(P/(phase+'-storage.jsonl')).open('x')as storage:
            ...
            if p.poll()is None:
                ...
                assert owned_cwd(where,p.pid,A),where
                        ...
                        if a[0]in known:
                            ...
                            if current!=before:
                                ...
                                if pid==p.pid and transient_xctest_exit(before,current,where_now):
                                    ...
                                    live.append(a)
                                    continue
                                assert compiler_transition(before,current,known[a[0]]['cwd'],where_now,A), (before,current,where_now)
                                j.setdefault('identityTransitions',[]).append(dict(pid=pid,before=before,after=current))
                        else:
                            assert a[1]in selected or pid==p.pid
                        assert owned_cwd(where_now,pid,A),where_now
