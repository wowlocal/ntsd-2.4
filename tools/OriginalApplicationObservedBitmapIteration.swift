...
/// One whole Host iteration on its existing owner. Only bitmap requests are
/// suspended here; all other inputs remain explicitly prepared observations.
public final class OriginalApplicationObservedBitmapIteration<Platform: OriginalApplicationObservedBitmapPlatform> {
    public typealias Host = OriginalApplicationHostSession<Platform>
    public typealias Exchange = OriginalBitmapRequestExchange
    public typealias Boundary = OriginalApplicationObservedBitmapBoundary
    public enum Outcome {
        ...
        try attempt {
            ...
            do {
                let result = try host.step(prepare:{ p,state in
                    p.bitmapDelivery.begin(cursor);return try prepare(p,state)
                ...
                bitmap:{ _,q,p in try p.bitmapDelivery.response(for:q) },
                beforePublication:{ p in
                    try beforePublication(p)
                    guard let consumed = p.bitmapDelivery.cursor else { throw Boundary.missingCursor }
