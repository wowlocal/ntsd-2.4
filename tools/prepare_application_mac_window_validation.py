#!/usr/bin/env python3
"""Prepare exact physical AppKit window validation with the existing guarded host tools."""
...
NAME='application-mac-window-validation-20260926'
TASK=Path('/Volumes/X5/ntsd-2.4-research/01a0dc49-738f-7972-8fb0-e98fb2f34408')/NAME
BASE=(ROOT/'build/research/application-mac-window-20260926').resolve()
OLD=(ROOT/'build/research/application-observed-startup-correction1-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_MAC_WINDOW_VALIDATION_PLAN.md'
...
def generate(name):
 ...
 if name==NAMES[0]:
  new=once(new,"r'build1|test-(0[1-9]|1[0-9]|2[0-5])'","r'build1|test-(0[1-9]|1[0-9]|2[0-9])'")
  marker='    def identity(pid):';assert old[old.index(marker):]==new[new.index(marker):]
 elif name==NAMES[1]:
  new=once(new,'25-method','29-method');new=once(new,'== 25','== 29');new=once(new,'all25Passed','all29Passed')
  begin,end='            done = re.findall(',"            job['completed'].append("
  assert old[old.index(begin):old.index(end)]==new[new.index(begin):new.index(end)]
 else:
  new=once(new,"('NTSDReferenceChecks', 'native/Sources/NTSDReferenceChecks'),","('NTSDReferenceChecks', 'native/Sources/NTSDReferenceChecks'),\n                             ('NTSDMacPlatform', 'native/Sources/NTSDMacPlatform'),")
  new=once(new,"if name != 'NTSDReferenceChecks' else set()","if name in ('NTSDCore', 'NTSDCoreTests') else set()")
  new=once(new,"'NTSDCoreTests':277","'NTSDCoreTests':278,'NTSDMacPlatform':2")
...
def main():
 ...
 try:
  ...
  rows=read(BASE/'candidate1-inputs.json');assert len(rows)==2257
  ...
  assert len(methods)==len(set(methods))==29 and len(new)==4 and len(old)==25 and methods==new+old
  previous=read(OLD/'method-limits1.json')['methods'];inherited={m:previous[m] for m in old}
  assert len(inherited)==25
  limits={}
  for m in methods:
  ...
  save('method-limits1.json',dict(methods=limits,queueSeconds=14400,inheritedMethods=25,priorLimits=record(OLD/'method-limits1.json'),priorSelection=record(OLD/'selected-methods1.json'),precedingComparisonsAreSeparate=True))
  ...
  pattern=r'build1|test-(0[1-9]|1[0-9]|2[0-9])'
  assert all(re.fullmatch(pattern,p) for p in ['build1']+[f'test-{i:02d}' for i in range(1,30)])
  invalid=['build0','build2','test-00','test-30','test-99','test-1','test-01x'];assert not any(re.fullmatch(pattern,p) for p in invalid)
  save('runner-controls1.json',dict(UTC=now(),monitorBodyUnchanged=True,resultPredicatesUnchanged=True,cwdPositive=1,cwdNegative=4,compilerPositive=1,compilerNegative=2,transientPositive=1,transientNegative=1,phasePositive=30,phaseNegative=7,exactMethods=29,oldMethodLimitsUnchanged=True,nativeExecuted=False,originalExecuted=False,independentReview=False))
  job.update(status='terminal',exitCode=0,files=2257,preparedMethods=len(methods),buildConfig=record(TASK/'build1-config.json'))
