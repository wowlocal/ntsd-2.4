#!/usr/bin/env python3
"""Pin existing typed-host products and prior passes for28 whole methods."""
import ast
import datetime
import json
import os
...
TASK = Path('/Volumes/X5/ntsd-2.4-research/01a0c7b2-a956-7760-950e-ecf7dceaaa20/application-host-match-completion-20260922')
ARTIFACT = TASK.with_name('application-host-match-correction1-20260922')
PLAN = ROOT/'docs/research/APPLICATION_HOST_MATCH_COMPLETION_PLAN.md'
...
def main():
    ...
    assert start_free > 66*2**30 and shutil.disk_usage(ROOT).free > 7*2**30
    ...
    try:
        ...
        paths = [PLAN,Path(__file__),ARTIFACT/'context1.json',ARTIFACT/'publication1.json',ARTIFACT/'external-close1.json',
            ...
            ARTIFACT/'selected-methods1.json',ARTIFACT/'failure-diagnosis1.json',ARTIFACT/'test-26.job.json',ARTIFACT/'test-26.log',
            ARTIFACT/'artifact-archive1.json',ARTIFACT/'metadata1.tar',
            ROOT/'tools/run_application_host_match_validation2.py',ROOT/'tools/run_application_host_match_tests2.py',
            ROOT/'tools/run_application_host_match_validation3.py',ROOT/'tools/run_application_host_match_tests3.py']
        ...
        for index,r in enumerate(queue['completed'][:25]):
        ...
        save(TASK/'prior-passes1.json',dict(methods=prior,count=25,remaining=methods[25:],required=methods))
        environment=dict(old['environment'],TMPDIR=str(TASK/'tmp'))
        context=dict(schema='ntsd-host-match-completion-context-v1',UTC=now(),head=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
            artifactRoot=str(ARTIFACT),rootManifest=old['rootManifest'],sourceTask=old['sourceTask'],sourceCodePins=old['sourceCodePins'],
            protected=protected,startFree=start_free,physicalAllowanceBytes=4*2**30,stopObservedDecrease=3*2**30,
            ...
            vmStat=subprocess.check_output(['vm_stat'],text=True),candidateFiles=1039,rootNativeFiles=1034,packageResourceFiles=487,
            allInputBytesVerified=True,retainedPasses=25,remainingMethods=28,sourceExecuted=False,independentReview=False,
            gitStatus=subprocess.check_output(['git','status','--porcelain=v1'],text=True))
        assert context['physicalMemoryBytes']==24*2**30
        save(TASK/'resource-contract1.json',dict(revisedAfterFirstTypedHostGuard=True,diagnosis=record(ARTIFACT/'failure-diagnosis1.json'),retainedPasses=25,remainingMethods=methods[25:],residentBytes={f'test-{i:02d}':(16 if i>=51 else 12 if i in (26,28,35,36,37) or 43<=i<=50 else 8)*2**30 for i in range(26,54)},candidateOrExpectedChanged=False,timeLimitSeconds={f'test-{i:02d}':900 if i>=51 else 600 for i in range(26,54)},queueSeconds=7200,independentReview=False))
        save(TASK/'context1.json',context)
        config=dict(read(ARTIFACT/'test-26-config.json'),artifactRoot=str(ARTIFACT),environment=environment,
            contextSHA256=pin(TASK/'context1.json')['sha256'],stopObservedDecrease=3*2**30,externalGuardBytes=44*2**30,
        ...
        previous=(ROOT/'tools/run_application_host_match_validation2.py').read_text();current=(ROOT/'tools/run_application_host_match_validation3.py').read_text()
        ...
        assert a==b
        from run_application_host_match_validation3 import owned_cwd
        ...
        valid=[f'test-{i:02d}' for i in range(26,54)];invalid=['test-25','test-54','test-00','build1','other']
        for phase in valid:assert re.fullmatch(r'test-(2[6-9]|[34][0-9]|5[0-3])',phase)
        for phase in invalid:assert not re.fullmatch(r'test-(2[6-9]|[34][0-9]|5[0-3])',phase)
        old_queue=(ROOT/'tools/run_application_host_match_tests2.py').read_text();new_queue=(ROOT/'tools/run_application_host_match_tests3.py').read_text()
        def parser(body):
            begin=body.index('            passed = ');return body[begin:body.index("            job['completed']",begin)]
        assert parser(old_queue)==parser(new_queue)
        save(TASK/'runner-preflight1.json',dict(completePassParserByteIdentical=True,identicalPredicates=names,monitorLoopIdenticalExceptVerifiedArtifactRootArgument=True,
        ...
        inputs=[ROOT/'tools/run_application_host_match_validation3.py',ROOT/'tools/run_application_host_match_tests3.py',
        ...
        assert start_free-shutil.disk_usage(TASK).free<3*2**30
        job.update(status='terminal',exitCode=0,allInputsVerified=True,retainedPasses=25,remaining=28,context=record(TASK/'context1.json'))
