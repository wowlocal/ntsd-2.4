#!/usr/bin/env python3
"""Prepare one isolated window-exchange candidate using existing pin/APFS clone procedure."""
...
B=(R/'build/research/application-host-delivery-context-validation-20260926').resolve()
P=B.parent/'application-window-exchange-20260926'
PLAN=R/'docs/research/APPLICATION_WINDOW_EXCHANGE_PLAN.md'
...
def main():
 ...
 pub=read(B/'publication1.json');assert pub['status']=='all82-passed' and read(B/'external-close1.json')['taskFrozen']
 ...
 inputs=read(B/'candidate1-inputs.json');assert len(inputs)==2242
 ...
 try:
  ...
  job.update(status='terminal',exitCode=0,files=2242,bytes=sum(r['bytes'] for r in inputs),fullCloneVerified=True,distinctRegularInodes=True)
