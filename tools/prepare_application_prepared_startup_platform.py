#!/usr/bin/env python3
"""Prepare one isolated production prepared-startup candidate using existing pin/APFS clone procedure."""
...
B=(R/'build/research/application-host-window-inspection-validation-20260926').resolve()
P=B.parent/'application-prepared-startup-platform-20260926'
PLAN=R/'docs/research/APPLICATION_PREPARED_STARTUP_PLATFORM_PLAN.md'
...
def main():
 ...
 pub=read(B/'publication1.json');assert pub['status']=='all8-passed' and read(B/'external-close1.json')['taskFrozen']
 for key in ['candidateManifest','context','verification','packageCheck']:checked(Path(pub[key]['path']),pub[key])
 for n in ['tests1-queue.job.json','finalize2.job.json']:
 ...
 inputs=read(B/'candidate1-inputs.json');assert len(inputs)==2247
 ...
 try:
  ...
  job.update(status='terminal',exitCode=0,files=2247,bytes=sum(r['bytes'] for r in inputs),fullCloneVerified=True,distinctRegularInodes=True)
