#!/usr/bin/env python3
"""Clone the validated host candidate for the bounded typed Start/launch continuation."""
import ctypes
import datetime
import json
import os
...
BASE = (ROOT/'build/research/application-host-loading-correction3-20260922').resolve()
TASK = Path('/Volumes/X5/ntsd-2.4-research/01a0c7b2-a956-7760-950e-ecf7dceaaa20/application-host-match-candidate-20260922')
PLAN = ROOT/'docs/research/APPLICATION_HOST_MATCH_CANDIDATE_PLAN.md'
...
def main():
    ...
    assert free > 58*2**30 and shutil.disk_usage(ROOT).free > 7*2**30
    assert not TASK.exists()
    inputs = json.loads((BASE/'candidate1-inputs.json').read_text())
    assert len(inputs) == 1037
    ...
    validation = ROOT/'docs/evidence/application-host-loading-regressions.json'
    assert json.loads(validation.read_text())['all42Passed']
    ...
    job = dict(schema='ntsd-host-match-prepare-v1', pid=os.getpid(), status='running', startedUTC=now(),
    ...
    try:
        protected = [PLAN, Path(__file__), ROOT/'tools/prepare_application_host_candidate.py',
            ...
            ROOT/'docs/evidence/application-host-match-preflight-observations.json', validation, ROOT/'docs/evidence/application-host-match-preflight.json',
            ROOT/'docs/research/APPLICATION_HOST_MATCH_PREFLIGHT.md', root_manifest, source_manifest,
            ROOT/'AGENTS_HISTORY_2026-09-12.md', ROOT/'docs/research/WORKFLOW.md']
        context = dict(schema='ntsd-host-match-candidate-context-v1', UTC=now(),
            ...
            sourceIdentity=identity, sourceCwd=cwd, initialExternalFree=free, allowance=2**30, stopThreshold=896*2**20,
        ...
        for item in inputs:
            assert free-shutil.disk_usage(TASK).free < 896*2**20
