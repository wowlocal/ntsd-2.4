#!/usr/bin/env python3
"""Read-only full trace audit of retained VS reselection/reroll commands and ret4 or prelude boundaries.
...
See docs/research/LIB_SELECTION_COMMANDS_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 previous=None
 parent_parts=path.parent/'lib-selection-stage-capture3.parts'
 parent_index={}
 # Parent sequence labels/indices come from its immutable manifest; compare
 # each whole shared case against its retained atomic source file.
 for i,spec in enumerate(json.loads((path.parent/'lib-selection-stage-finite-inputs.json').read_bytes())):
  parent_index[spec['label']]=i
 prefix_done={False:False,True:False}
 assert len(d['cases'])==len(d['installations'])==338
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-selection-commands-finite-inputs.json').read_bytes())
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  label=c['spec']['label']
  control=c['spec']['control']
  if not prefix_done[control]:
   old=json.loads((parent_parts/f"{parent_index[label]:04d}.json").read_bytes())
   assert old['case']==c,(label,'complete unchanged parent prefix')
   assert all(d['blobs'][k]==v for k,v in old['blobs'].items())
   stats['identicalParentCalls']+=1
   if label=='cpu2-ready-release'+('-control' if control else ''):prefix_done[control]=True
 ...
 for c in d['cases']:
  ...
  if c['end']=='matchPrelude':
   ...
   assert [word(0x451248+i*4) for i in range(2)]==[17,21] and [word(0x451288+i*4) for i in range(4)]==[3,3,13,13]
   assert word(0x4512c8)==3 and word(0x44d024)==0 and word(0x44d028)==0 and word(0x44d070)==2
 assert ends=={'returned':336,'matchPrelude':2}
 assert all(prefix_done.values())
 for c in d['cases']:
  label=c['spec']['label'];g=decode_record(next(x for x in c['after'] if x['address']==0x44d000))[0]
  word=lambda p:struct.unpack_from('<i',g,p-0x44d000)[0]
  ea=[e for e in c['events'] if e['kind']=='random' and e['arguments'][0]==0xea]
  if label.startswith(('reroll-first-press','reroll-second-press','reroll-after-reselect-press')):
   assert len(ea)==5 and [e['arguments'][0] for e in c['events'] if e['kind']=='candidates']==[3,4,5,6,7]
   assert [word(0x451248+i*4) for i in range(2)]==[17,21]
   assert [word(0x451288+i*4) for i in range(4)]==[3,3,13,13]
   tail=next(p for p in c['points'] if p['kind']=='character-0x42e0b6')
   assert tail['locals']['32']==d['worldAddress']+0x1b4 and tail['locals']['52']==32
   stats['rerollCommands']+=1;stats['rerollAssignments']+=len(ea)
  else:assert not ea,(label,'no repeated reroll while held/released')
  if label in ('reselect-press','reselect-press-control'):
   assert [word(0x451288+i*4) for i in range(8)]==[0]*8
   assert [word(0x451268+i*4) for i in range(8)]==[1]*8
   assert [word(0x451248+i*4) for i in range(8)][3:]==[-1]*5
   assert [word(0x451248+i*4) for i in range(2)]==[17,21]
   assert word(0x44d078)==150 and word(0x4512c8)==0
   stats['reselectionCommands']+=1
 assert stats['rerollCommands']==6 and stats['rerollAssignments']==30 and stats['reselectionCommands']==2

 return dict(scope=__doc__,undefinedBitmapReads=[dict(pc=hex(pc),bytes=n,reads=count) for (pc,n),count in bitmap_unknown.items()],rawBytes=len(input_raw),rawSHA256=sha(input_raw),cases=len(d['cases']),ends=dict(ends),**stats,events=dict(events),instructionStarts=len(pcs),exeInstructionStarts=sum(int(p,16)<0x36000000 for p in pcs),libraryInstructionStarts=sum(0x36000000<=int(p,16)<0x36005000 for p in pcs),crtInstructionStarts=sum(int(p,16)>=0x78130000 for p in pcs),blobCount=len(blobs),decodedBlobBytes=sum(map(len,blobs.values())),writeMaskFalseReadsWithPinnedEXEProvenance=[dict(pc=hex(pc),address=hex(p),bytes=n,reads=count) for (pc,p,n),count in unknown.items()],nativeCompared=False,windowsVerified=False)
