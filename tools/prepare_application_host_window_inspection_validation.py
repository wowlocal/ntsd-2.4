#!/usr/bin/env python3
"""Prepare exact Host window inspection validation with the existing guarded host tools."""
...
NAME='application-host-window-inspection-validation-20260926'
TASK=Path('/Volumes/X5/ntsd-2.4-research/01a0dc49-738f-7972-8fb0-e98fb2f34408')/NAME
BASE=(ROOT/'build/research/application-host-window-inspection-20260926').resolve()
OLD=(ROOT/'build/research/application-host-window-startup-validation-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_HOST_WINDOW_INSPECTION_VALIDATION_PLAN.md'
...
def generate(name):
 ...
 if name==NAMES[0]:
  new=once(new,"r'build1|test-(0[1-9]|1[0-9]|2[0-8])'","r'build1|test-0[1-8]'")
  marker='    def identity(pid):';assert old[old.index(marker):]==new[new.index(marker):]
 elif name==NAMES[1]:
  new=once(new,'28-method','8-method');new=once(new,'== 28','== 8');new=once(new,'all28Passed','all8Passed')
  begin,end='            done = re.findall(',"            job['completed'].append("
  assert old[old.index(begin):old.index(end)]==new[new.index(begin):new.index(end)]
 else:
  new=once(new,"'NTSDCoreTests':274","'NTSDCoreTests':275")
...
def main():
 ...
 try:
  ...
  rows=read(BASE/'candidate1-inputs.json');assert len(rows)==2247
  ...
  assert len(methods)==len(set(methods))==8 and len(new)==3 and len(old)==5 and methods==new+old
  ...
  save('method-limits1.json',dict(methods=limits,queueSeconds=14400,inheritedMethods=5,priorLimits=record(OLD/'method-limits1.json'),priorContextLimits=record(priorContext/'method-limits1.json'),priorSelection=record(OLD/'selected-methods1.json'),precedingComparisonsAreSeparate=True))
  ...
  pattern=r'build1|test-0[1-8]'
  assert all(re.fullmatch(pattern,p) for p in ['build1']+[f'test-{i:02d}' for i in range(1,9)])
  invalid=['build0','build2','test-00','test-09','test-99','test-1','test-01x'];assert not any(re.fullmatch(pattern,p) for p in invalid)
  save('runner-controls1.json',dict(UTC=now(),monitorBodyUnchanged=True,resultPredicatesUnchanged=True,cwdPositive=1,cwdNegative=4,compilerPositive=1,compilerNegative=2,transientPositive=1,transientNegative=1,phasePositive=9,phaseNegative=7,exactMethods=8,oldMethodLimitsUnchanged=True,nativeExecuted=False,originalExecuted=False,independentReview=False))
  job.update(status='terminal',exitCode=0,files=2247,preparedMethods=8,buildConfig=record(TASK/'build1-config.json'))
