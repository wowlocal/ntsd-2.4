...
BASE = ROOT/'build/research/application-host-loading-correction1-20260922'
TASK = Path('/Volumes/X5/ntsd-2.4-research/01a0c7b2-a956-7760-950e-ecf7dceaaa20/application-host-loading-correction3-20260922')
PLAN = ROOT/'docs/research/APPLICATION_HOST_LOADING_CORRECTION3_PLAN.md'
...
def main():
    ...
    for key in ['context', 'candidateManifest', 'packageCheck']:
        same(Path(publication[key]['path']), publication[key])
    same(BASE/'artifact-archive1.json', publication['artifactArchive']['manifest'])
    same(BASE/'metadata1.tar', publication['metadataArchive'])
    ...
    job = dict(schema='ntsd-host-loading-validation-prepare-job-v1', status='running', pid=os.getpid(),
    ...
    try:
        ...
        for name in ['tmp', 'cache', 'config', 'security']:
            (TASK/name).mkdir()
        shutil.copy2(BASE/'candidate1-inputs.json', TASK/'candidate-before1-inputs.json')
        shutil.copy2(BASE/'selected-methods1.json', TASK/'selected-methods1.json')
        changed = 'native/Tests/NTSDCoreTests/OriginalApplicationHostLoadingTests.swift'
        f = TASK/'candidate1'/changed
        before = f.read_text()
        old = '        XCTAssertNotNil(alternate);XCTAssertNil(host.preparedLoadedMenu)'
        new = '        XCTAssertTrue(alternate != nil);XCTAssertNil(host.preparedLoadedMenu)'
        ...
        corrected = []
        for item in candidate:
            actual_pin = pin(TASK/'candidate1'/item['path'])
            if item['path'] != changed: same(TASK/'candidate1'/item['path'], item)
            corrected.append(dict(path=item['path'], **{k:actual_pin[k] for k in ('bytes','sha256','mode')}))
        assert sum(a['sha256'] != b['sha256'] for a,b in zip(candidate,corrected)) == 1
        (TASK/'candidate1-inputs.json').write_text(json.dumps(corrected, indent=2)+'\n')
        ...
        protected_paths = [PLAN, Path(__file__), ROOT/'tools/prepare_application_host_candidate.py',
            ROOT/'tools/run_application_host_validation2.py', BASE/'publication1.json', BASE/'external-close1.json',
            ...
            ROOT/'docs/evidence/application-host-loading-validation.json',
            ROOT/'docs/evidence/application-host-loading-validation-close.json',
            ROOT/'build/research/application-host-loading-validation-20260922/failure-diagnosis1.json',
            ROOT/'build/research/application-host-loading-validation-20260922/test-26.log',
            ROOT/'docs/evidence/application-host-loading-completion.json',
            ROOT/'docs/evidence/application-host-loading-completion-close.json',
            ROOT/'build/research/application-host-loading-completion-20260922/formatting-diagnosis1.json',
            ROOT/'build/research/application-host-loading-completion-20260922/test-28-stack-sample1.txt',
            ROOT/'build/research/application-host-loading-completion-20260922/test-28.job.json']
        context = dict(schema='ntsd-host-loading-validation-context-v1', UTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),
            ...
            correctionRound=3, changedPath=changed, beforeManifest=pin(TASK/'candidate-before1-inputs.json'),
            correctionPatch=pin(TASK/'correction1.patch'), coreAndResourcesUnchanged=True,
        ...
        config = dict(schema='ntsd-host-loading-validation-run-config-v1', phase='build1', buildPhase='build1', kind='build', command=command,
