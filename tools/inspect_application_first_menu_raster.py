...
def main():
 ...
 try:
  ...
  input_records=[];(TASK/'inputs').mkdir()
  ...
  shutil.copy2(PLAN,TASK/'plan1.md');shutil.copy2(Path(__file__),TASK/'inspect1.py')
  ...
  job.update(endedUTC=now(),elapsedSeconds=time.monotonic()-began);(TASK/'inspect1.job.json').write_text(json.dumps(job,indent=2)+'\n');print(json.dumps(job),flush=True)
