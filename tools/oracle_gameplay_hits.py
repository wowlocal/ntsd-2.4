...
# ///
"""Continue own41eefb through both complete42e100 passes and item spawn.
Fresh original menu/loading/launch/control/physics/links/contacts on the SAME
CPU/stack, with no new gameplay stimuli. Stops41f2ac before link helpers.
Source capture; native comparison of this complete continuation remains open.
"""
import argparse,json
from oracle_gameplay_contacts import GameplayContacts
...
HELPERS={0x42e100:(1,4),0x417170:(2,0),0x4061d0:(0,0),0x4450d0:(0,0),0x416fb0:(2,0),0x417090:(2,0)}
class GameplayHits(GameplayContacts):
 def gameplay_code(self,uc,pc,size,data):
  if not self.gameplay_running or self.gameplay_label!='hits-items':return super().gameplay_code(uc,pc,size,data)
  ...
  if pc in (0x41ef47,0x41f29e):
   slot=uc.reg_read(REGISTERS[2]);assert 0<=slot<400
   # Only actual42e100 return PCs are checkpoints, not ordinary loop skips.
   if self.gameplay_helpers and self.gameplay_helpers[-1]['returnPC']==pc and self.gameplay_helpers[-1]['arguments']==[slot]:
    self.gameplay_checkpoints.append(dict(label='hit-return',pc=pc,slot=slot,actor=self.record(self.pool[slot])))
  assert any(a<=pc<=b for a,b in [(0x41eefb,0x41f2ac),(0x42e100,0x431b64),(0x4061d0,0x4064cf),(0x416fb0,0x4171bc),(0x4450d0,0x44517a)]),hex(pc)

 def capture_character(self,parent):
  old=super().capture_character(parent);suffix='-control' if self.control else ''
  report=json.loads((ROOT/'docs/evidence'/f'gameplay-contacts{suffix}.json').read_bytes())
  ...
  print('Entire pinned GAMEPLAY_CONTACTS reproduced; continuing hits/items',flush=True)
  ...
  before_heap=heap();section=self.gameplay_step('hits-items',0x41eefb,0x41f2ac)
...
def main():
 ...
 doc=capture_startup(a.control,vm_type=GameplayHits,after=lambda vm,parent:vm.capture_screen(parent,after_first=lambda vm,first:vm.capture_return(first,after_cycle=lambda vm,cycle:vm.capture_character(cycle))))
 suffix='-control' if a.control else '';raw=(json.dumps(doc,separators=(',',':'))+'\n').encode();path=ROOT/'build/original'/f'gameplay-hits{suffix}.json';path.write_bytes(raw)
