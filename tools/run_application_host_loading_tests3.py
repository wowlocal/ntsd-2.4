#!/usr/bin/env python3
"""Run the fifteen remaining whole methods through the existing guarded runner."""
import datetime
import json
import os
...
from run_application_host_loading_validation3 import main as run_one
...
def main(task):
    ...
    cases = []
    for index, method in enumerate(methods, 28):
    ...
    selection = dict(schema='ntsd-host-loading-test-commands-v1', cases=cases, packageCheck=pin(artifact/'package-check1.json'),
        selectedMethods=pin(artifact/'selected-methods1.json'), queueProducer=pin(Path(__file__)),
        runner=pin(ROOT/'tools/run_application_host_loading_validation3.py'), completeResultParser='same exact named XCTest regex and one-test zero-failure summary as Catalog53 completion')
    ...
    try:
        for case in cases:
            ...
            job.update(status='terminal', exitCode=0, all15RemainingPassed=True)
