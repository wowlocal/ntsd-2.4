#!/usr/bin/env python3
"""Prepare exact retained-context validation with the existing catalog host tools."""
...
NAME='application-host-delivery-context-validation-20260926'
TASK=Path('/Volumes/X5/ntsd-2.4-research/01a0dc49-738f-7972-8fb0-e98fb2f34408')/NAME
BASE=(ROOT/'build/research/application-host-delivery-context-20260926').resolve()
OLD=(ROOT/'build/research/application-catalog-package-validation-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_HOST_DELIVERY_CONTEXT_VALIDATION_PLAN.md'
...
def generate(name):
 ...
 if name==NAMES[0]:
  new=once(new,"r'build1|test-(0[1-9]|[1-6][0-9]|7[0-6])'","r'build1|test-(0[1-9]|[1-7][0-9]|8[0-2])'")
  marker='    def identity(pid):';assert old[old.index(marker):]==new[new.index(marker):]
 elif name==NAMES[1]:
  new=once(new,'76-method','82-method');new=once(new,'== 76','== 82');new=once(new,'all76Passed','all82Passed')
  begin,end='            done = re.findall(',"            job['completed'].append("
  assert old[old.index(begin):old.index(end)]==new[new.index(begin):new.index(end)]
 else:new=once(new,"'NTSDCoreTests':271","'NTSDCoreTests':272")
...
def main():
 ...
 try:
  ...
  rows=read(BASE/'candidate1-inputs.json');assert len(rows)==2242
  ...
  assert len(methods)==len(set(methods))==82 and len(new)==6 and len(old)==76 and methods==new+old
  previous=read(OLD/'method-limits1.json')['methods'];assert set(previous)==set(old)
  limits={}
  for i,m in enumerate(methods,1):
   ...
   limits[m]=dict(seconds=600 if i<=3 else 1800,residentBytes=(8 if i<=3 else 16)*2**30,priorOrdinal=None) if i<=6 else previous[m]
  assert {m:limits[m] for m in old}==previous
  save('method-limits1.json',dict(methods=limits,queueSeconds=14400,inheritedMethods=76,priorLimits=record(OLD/'method-limits1.json'),priorSelection=record(OLD/'selected-methods1.json')))
  ...
  pattern=r'build1|test-(0[1-9]|[1-7][0-9]|8[0-2])'
  assert all(re.fullmatch(pattern,p) for p in ['build1']+[f'test-{i:02d}' for i in range(1,83)])
  invalid=['build0','build2','test-00','test-83','test-99','test-1','test-01x'];assert not any(re.fullmatch(pattern,p) for p in invalid)
  save('runner-controls1.json',dict(UTC=now(),monitorBodyUnchanged=True,resultPredicatesUnchanged=True,cwdPositive=1,cwdNegative=4,compilerPositive=1,compilerNegative=2,transientPositive=1,transientNegative=1,phasePositive=83,phaseNegative=7,exactMethods=82,old76LimitsUnchanged=True,nativeExecuted=False,originalExecuted=False,independentReview=False))
  job.update(status='terminal',exitCode=0,files=2242,preparedMethods=82,buildConfig=record(TASK/'build1-config.json'))
