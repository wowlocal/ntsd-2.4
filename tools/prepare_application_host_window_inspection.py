#!/usr/bin/env python3
"""Prepare one isolated post-handoff inspection candidate using existing pin/APFS clone procedure."""
...
B=(R/'build/research/application-host-window-startup-validation-20260926').resolve()
P=B.parent/'application-host-window-inspection-20260926'
PLAN=R/'docs/research/APPLICATION_HOST_WINDOW_INSPECTION_PLAN.md'
...
def main():
 ...
 pub=read(B/'publication1.json');assert pub['status']=='all28-passed' and read(B/'external-close1.json')['taskFrozen']
 ...
 inputs=read(B/'candidate1-inputs.json');assert len(inputs)==2246
 ...
 try:
  ...
  job.update(status='terminal',exitCode=0,files=2246,bytes=sum(r['bytes'] for r in inputs),fullCloneVerified=True,distinctRegularInodes=True)
