#!/usr/bin/env python3
"""Pin existing host-loading products and prior passes for fifteen whole methods."""
import ast
import datetime
import json
import os
...
TASK = Path('/Volumes/X5/ntsd-2.4-research/01a0c7b2-a956-7760-950e-ecf7dceaaa20/application-host-loading-completion-20260922')
ARTIFACT = TASK.with_name('application-host-loading-correction1-20260922')
PLAN = ROOT/'docs/research/APPLICATION_HOST_LOADING_COMPLETION_PLAN.md'
...
def main():
    ...
    try:
        ...
        paths = [PLAN,Path(__file__),ARTIFACT/'context1.json',ARTIFACT/'publication1.json',ARTIFACT/'external-close1.json',
            ...
            ARTIFACT/'selected-methods1.json',ARTIFACT/'guard-diagnosis1.json',ARTIFACT/'test-28.job.json',ARTIFACT/'test-28.log',
            ARTIFACT/'artifact-archive1.json',ARTIFACT/'metadata1.tar',
            ROOT/'tools/run_application_host_loading_validation2.py',ROOT/'tools/run_application_host_loading_tests2.py']
        ...
        for index,r in enumerate(queue['completed'][:27]):
        ...
        save(TASK/'prior-passes1.json',dict(methods=prior,count=27,remaining=methods[27:],required=methods))
        ...
        context=dict(schema='ntsd-host-loading-completion-context-v1',UTC=now(),head=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
            ...
            allInputBytesVerified=True,retainedPasses=27,remainingMethods=15,sourceExecuted=False,independentReview=False,
        ...
        config=dict(read(ARTIFACT/'test-28-config.json'),artifactRoot=str(ARTIFACT),environment=environment,
        ...
        previous=(ROOT/'tools/run_application_host_loading_validation2.py').read_text();current=(ROOT/'tools/run_application_host_loading_validation3.py').read_text()
        ...
        assert a==b
        from run_application_host_loading_validation3 import owned_cwd
        ...
        valid=[f'test-{i:02d}' for i in range(28,43)];invalid=['test-27','test-43','test-00','build1','other']
        for phase in valid:assert re.fullmatch(r'test-(2[89]|3[0-9]|4[0-2])',phase)
        for phase in invalid:assert not re.fullmatch(r'test-(2[89]|3[0-9]|4[0-2])',phase)
        ...
        inputs=[ROOT/'tools/run_application_host_loading_validation3.py',ROOT/'tools/run_application_host_loading_tests3.py',
            ROOT/'tools/archive_catalog53_storage.py',TASK/'runner-preflight1.json',TASK/'prior-passes1.json',TASK/'test-template1.json']
        ...
        job.update(status='terminal',exitCode=0,allInputsVerified=True,retainedPasses=27,remaining=15,context=record(TASK/'context1.json'))
