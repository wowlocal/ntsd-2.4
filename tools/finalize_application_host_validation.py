...
def main(task):
    assert task.name == 'application-host-transaction-validation-20260922'
    ...
    job = dict(schema='ntsd-host-validation-finalizer-v1', pid=os.getpid(), status='running', startedUTC=now(),
    ...
    try:
        ...
        assert queue['status'] == 'terminal' and queue['exitCode'] == 0 and queue['all25Passed'] and absent(queue['pid'])
        selected = read(task/'selected-methods1.json')['methods']
        assert selected == [r['method'] for r in queue['completed']] and len(selected) == 25
        tests = []
        for record in queue['completed']:
            ...
            assert child['status'] == 'terminal' and child['exitCode'] == 0 and absent(child['pid'])
            ...
            tests.append(dict(method=record['method'], pid=child['pid'], exitCode=0,
        ...
        builds = [read(task/(name+'.job.json')) for name in ('build1', 'build2')]
        assert [b['exitCode'] for b in builds] == [1, 0]
        for b in builds:
        ...
        for target in package['targets'].values():
            for item in target['outputsFromThisFreshTask']:
                checked(Path(item['path']), item)
        retained = read(task/'build1-preservation.json')
        for item in retained['files']:
            checked(task/'build1-retained'/item['path'], item)
        actual = {str(f.relative_to(task/'build1-retained')) for f in (task/'build1-retained').rglob('*') if f.is_file() and not f.is_symlink()}
        assert actual == {r['path'] for r in retained['files']}
        ...
        artifact_report = dict(schema='ntsd-host-validation-artifact-archive-v1', UTC=now(), files=files,
        ...
        write(task/'verification1.json', dict(UTC=now(), protectedPins=len(context['protected']), rootNativeFiles=1034,
            candidateFiles=1036, sourceCodePins=55, allPreserved=True, resourceFiles=487,
            preservedFailedBuildFiles=len(retained['files']), all25Passed=True, tests=tests,
        ...
        tools = ['prepare_application_host_validation.py', 'run_application_host_validation.py',
                 'run_application_host_validation2.py', 'verify_application_host_validation_package.py',
                 'verify_application_host_validation_package2.py', 'run_application_host_tests.py', Path(__file__).name]
        for name in tools:
        ...
        publication = dict(schema='ntsd-host-transaction-validation-v1', UTC=now(), status='all25-methods-verified-review-open',
            ...
            retainedIdenticalFirstBuildObjects=len(package['targets']['NTSDCore']['retainedIdenticalBuild1Objects']),
            ...
            corrections=['build1 lacked release testability; failed tree retained, compiler flag added with unchanged candidate',
                         'first package checker assumed every object timestamp changes;40 exact first-build Core objects verified against preserved outputs'],
            gates=dict(nativeComparison=True, packageBytes=True, archive=True, independentReview=False,
            ...
            limitations=['Short test processes can finish before any RSS sample; recorded0 is not known zero RSS.',
                         ...
                         'Driver retains pending loading but does not yet finish the child chain or deliver real IO.'])
        write(task/'publication1.json', publication)
        (ROOT/'docs/evidence/application-host-transaction-validation.json').write_bytes((task/'publication1.json').read_bytes())
        ...
        closure = dict(schema='ntsd-host-validation-close-v1', UTC=now(), largePhaseClosed=True,
        ...
        (ROOT/'docs/evidence/application-host-transaction-validation-close.json').write_bytes((task/'external-close1.json').read_bytes())
        job.update(status='terminal', exitCode=0)
        print(json.dumps(dict(all25Passed=True, testSeconds=publication['tests']['totalTestProcessSeconds'],
