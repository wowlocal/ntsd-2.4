#!/usr/bin/env python3
"""Independently verify complete active-key-call raw/packed bytes and pins.

The controls omit bundled lib.dll patches and do not establish its gameplay.
...
def main():
    ...
    prior=json.loads((ROOT/'build/research/active-gameplay-prior-pins.json').read_bytes())
    ...
    current=json.loads((ROOT/'build/research/active-gameplay-fixture-pins.json').read_bytes())
    library=json.loads((ROOT/'build/research/lib-runtime-fixture-pins.json').read_bytes())
    actual={p.name:digest(p.read_bytes()) for p in FIXTURES.glob('*.json')}
    assert len(old)==190 and all(current[k]==v and actual[k]==v for k,v in old.items())
    assert len(latest)==195 and all(prior[k]==v for k,v in latest.items())
    assert len(dispatch)==196 and all(prior[k]==v for k,v in dispatch.items())
    assert all(current[k]==v and actual[k]==v for k,v in prior.items())
    assert all(actual[k]==v for k,v in current.items())
    assert len(library)==200 and all(library[k]==v for k,v in current.items())
    assert all(actual[k]==v for k,v in library.items())
    checks=[]
    for suffix in ('','-control'):
        name='active-gameplay'+suffix
        ...
        assert report['bundledLibLoaded'] is False
    ...
    result=dict(acceptedOldPinsUnchanged=len(old),allPrepublicationPinsUnchanged=len(prior),currentMilestonePins=len(current),subsequentLibraryPinsUnchanged=len(library),actualPins=len(actual),vendorHashesVerified=10,
        completeArtifactChecks=checks,windowsVerified=False)
    (ROOT/'build/research/active-gameplay-artifact-verification.json').write_text(json.dumps(result,indent=2)+'\n')
