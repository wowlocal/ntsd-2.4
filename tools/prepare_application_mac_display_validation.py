#!/usr/bin/env python3
"""Prepare exact native display resource validation with the existing guarded host tools."""
...
NAME='application-mac-display-validation-20260926'
...
OLD=(ROOT/'build/research/application-mac-window-validation-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_MAC_DISPLAY_VALIDATION_PLAN.md'
...
def generate(name):
 ...
 if name==NAMES[0]:
  new=once(new,"r'build1|test-(0[1-9]|1[0-9]|2[0-9])'","r'build1|test-(0[1-9]|[12][0-9]|3[0-4])'")
  marker='    def identity(pid):';assert old[old.index(marker):]==new[new.index(marker):]
 elif name==NAMES[1]:
  new=once(new,'29-method','34-method');new=once(new,'== 29','== 34');new=once(new,'all29Passed','all34Passed')
  begin,end='            done = re.findall(',"            job['completed'].append("
  assert old[old.index(begin):old.index(end)]==new[new.index(begin):new.index(end)]
 else:
  new=once(new,"'NTSDCoreTests':278,'NTSDMacPlatform':2","'NTSDCoreTests':279,'NTSDMacPlatform':5")
...
def main():
 ...
 for parent,names in [(BASE,['prepare1.job.json','parse2.job.json','finalize1.job.json']),(OLD,['tests1-queue.job.json','finalize1.job.json'])]:
  for n in names:
 ...
 try:
  ...
  previous=read(OLD/'method-limits1.json')['methods'];inherited={m:previous[m] for m in old if m in previous}
  assert len(inherited)==29 and old[:-1]==read(OLD/'selected-methods1.json')['methods']
  assert old[-1]=='OriginalApplicationDispatchPrefixTests/testWholeSurfaceClearRequestsAndReturns'
  limits={}
  for m in methods:
  ...
  save('method-limits1.json',dict(methods=limits,queueSeconds=14400,inheritedMethods=29,priorLimits=record(OLD/'method-limits1.json'),priorSelection=record(OLD/'selected-methods1.json'),precedingComparisonsAreSeparate=True))
