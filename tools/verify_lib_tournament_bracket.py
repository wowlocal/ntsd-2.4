#!/usr/bin/env python3
"""Read-only full trace audit of Tournament setup/bracket/CPU decisions/seat assignment/result/winner output and ret4 or preparation boundaries.
...
See docs/research/LIB_TOURNAMENT_BRACKET_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 previous=None
 assert len(d['cases'])==len(d['installations'])==250
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-tournament-bracket-finite-inputs3.json').read_bytes())
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  ...
  previous=c
  assert c['cw']==0x23f and c['end'] in ('returned','tournamentMatchPreparation')
  ...
  for event in c['events']:
   if event['kind']=='fill':
    ...
    assert f['effects'][:4]==[100,0,0,0] and f['effects'][0x50:0x54] in ([0x65,0x25,0x12,0],[255,255,255,0],[0,0,0,0],[0x9a,0x4d,0x32,0],[0x60,0x28,0x10,0],[0xa0,0xa0,0xff,0])
  for x in c['after']:
  ...
  if c['end']=='returned':
   ...
   assert c['end']=='tournamentMatchPreparation' and c['endPC']==0x434349 and c['endSP']==c['characterSP']-0xa14
   assert [h['entry'] for h in c['pending']]==[0x429730,0x432ab0] and all(h['pop']==12 for h in c['pending'])
   assert c['points'][-1]['kind']=='tournament-0x434349' and '0x434349' not in c['instructions']
  if True:
 ...
 assert ends=={'returned':249,'tournamentMatchPreparation':1}
 ...
 for c in d['cases']:
  ...
  for h in c['helpers']:
   if h['entry']==0x417170:
    assert h['stream'] in (1,0xf7,0xf8,0xf9,0xfa,0xfb,0xfc,0x100);streams[h['stream']]+=1
    ...
    if h['stream'] in (0xfa,0xfb,0x100):assert h['range']==2
    if h['stream']==0xfc:
     difficulty=struct.unpack_from('<i',live,0x450c30-0x44d000)[0];assert h['range']==15+12*(0 if difficulty < -1 else difficulty)
   ...
   if h['entry']==0x401290 and h['textAddress']==0x44d31c:
    ...
    assert writes and bytes.fromhex(writes[-1]['bytes'])==bytes(h['text']) and g[0x44d31d-0x44d000]==0
  ...
  if c['spec']['label']=='prepare-match-human':assert word(0x44d020)==27 and word(0x4513d8)==3
  if c['spec']['label']=='winner-dismiss-cpu':assert word(0x44d020)==10
  if c['spec']['label'].startswith('controlled-result'):
   assert [word(0x44d080+i*4) for i in range(2)]==[321,198]
   assert [word(0x44d0a0+i*4) for i in range(2)]==[4,5]
   assert all(word(0x4512d0+i*4)==-1 for i in range(20))
  if c['spec']['label']=='controlled-held-attack':
   held=[x for x in c['reads'] if x['pc']==0x433d8b];assert len(held)==1 and held[0]['bytes']=='01' and held[0]['known']==[1]
   assert word(0x44d020)==27 and word(0x4513d8)==0 and c['end']=='returned';stats['heldAttackReads']+=1
 stats['controllerLabelCalls']=sum(controller_labels.values());stats['bodyMusicEvents']=sum(body_music.values())
 assert streams[0xfa]>0 and streams[0xfb]>0 and streams[0xfc]>0 and streams[0x100]>0
 assert body_music['allocate']==body_music['convert']==2 and stats['heldAttackReads']==1
 # Complete reproduction of all249 earlier successful source calls.
 parent_raw=(path.parent/'lib-tournament-bracket-capture2.json').read_bytes();assert sha(parent_raw)=='5faec79b2b036699df94a328461088bb3b631ae472a497f39cecf6ab3f48bf34'
 parent=json.loads(parent_raw);assert d['cases'][:249]==parent['cases'] and d['installations'][:249]==parent['installations']
 assert all(d['blobs'][k]==v for k,v in parent['blobs'].items()) and d['assets']==parent['assets'] and d['roster']==parent['roster'] and d['arenas']==parent['arenas']
 stats['earlierCallsReproducedUnchanged']=249
