#!/usr/bin/env python3
"""Read-only full trace audit of retained VS computer/team/arena selection and ret4 or prelude boundaries.
...
See docs/research/LIB_SELECTION_STAGE_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 previous=None
 assert len(d['cases'])==len(d['installations'])==252
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-selection-stage-finite-inputs.json').read_bytes())
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  label=c['spec']['label'];state={x['address']:decode_record(x) for x in c['before']};alive={x['address']:x['live'] for x in c['before'] if x['live'] is not None}
 ...
 assert ends=={'returned':250,'matchPrelude':2}
