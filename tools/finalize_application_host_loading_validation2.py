...
def main(task):
    assert task.name == 'application-host-loading-correction1-20260922'
    ...
    try:
        ...
        assert len(selected) == 42 and selected[:28] == [r['method'] for r in queue['completed']]
        assert len(queue['completed']) == 28 and [r['passed'] for r in queue['completed']] == [True]*27+[False]
        assert all(not (task/f'test-{i:02d}.job.json').exists() for i in range(29,43))
        tests = []
        for record in queue['completed']:
            ...
            assert child['status'] == 'terminal' and child['exitCode'] == record['exitCode'] == (0 if record['passed'] else -15) and absent(child['pid'])
            ...
            if record['passed']:
                ...
                assert child['guardReason'] == 'resident bound'
                assert len(child['signals']) == 1 and child['signals'][0]['signal'] == 'SIGTERM'
                assert child['peakTreeRSS'] > child['residentGuardBytes'] == 8*2**30
            ...
            if record['passed']:
                assert len(done) == 1 and '/'.join(done[0][:2]) == record['method'] and done[0][2] == 'passed'
                assert text.count('Executed 1 test, with 0 failures (0 unexpected)') == 3
                assert "Test Suite 'Selected tests' passed" in text
            else:
                assert not done and ': error:' not in text and 'Executed 1 test' not in text
                assert record['method'].split('/')[1] in text
            tests.append(dict(method=record['method'], pid=child['pid'], exitCode=child['exitCode'], passed=record['passed'], outcome='pass' if record['passed'] else 'resident-guard-incomplete',
        ...
        assert len(previous) == len(candidate) == 1037
        changes = []
        ...
        assert proof['allChangedBytesVerified'] and proof['changedFiles'] == changes
        root_patch = ROOT/'docs/evidence/application-host-loading-correction1.patch'
        ...
        write(task/'verification1.json', dict(UTC=now(), protectedPins=len(context['protected']), rootNativeFiles=1034,
            ...
            all42Passed=False, passedMethods=27, guardedMethods=1, unstartedMethods=selected[28:], tests=tests,
        ...
        tools = ['prepare_application_host_loading_validation2.py', 'run_application_host_loading_validation2.py',
                 'verify_application_host_loading_package2.py', 'run_application_host_loading_tests2.py', Path(__file__).name]
        for name in tools:
        ...
        publication = dict(schema='ntsd-host-loading-validation-v1', UTC=now(), status='27-passed-resident-guard-14-unstarted',
            ...
            corrections=[dict(round=1, changedFiles=changes, patch=dict(path=str(task/'correction1.patch'), **pin(task/'correction1.patch')), roundtrip=pin(task/'patch-roundtrip1.json'), previousFailure='docs/evidence/application-host-loading-validation.json', expectedSourceUnchanged=True, independentReview=False)],
            guardDiagnosis=dict(path=str(task/'guard-diagnosis1.json'), **pin(task/'guard-diagnosis1.json')),
        ...
        (ROOT/'docs/evidence/application-host-loading-correction1.json').write_bytes((task/'publication1.json').read_bytes())
        ...
        (ROOT/'docs/evidence/application-host-loading-correction1-close.json').write_bytes((task/'external-close1.json').read_bytes())
        job.update(status='terminal', exitCode=0)
        print(json.dumps(dict(all42Passed=False, passed=27, guarded=1, unstarted=14, testSeconds=publication['tests']['totalTestProcessSeconds'],
