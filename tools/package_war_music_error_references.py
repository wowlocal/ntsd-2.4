#!/usr/bin/env python3
"""Losslessly package nine saved, returned NTSD War music/control calls.

Pinned EXE/lib/VC80 observations used controlled Unicorn2.1.4/CW023f/C locale
and declared music creation/query/allocation/render/conversion responses. Full
records, masks, event order, owners and blobs are retained to compare Native
preparation. The two normal controls remain explicit. s01 links to bound cases
11..20; the other eight scenarios link to 0..9, without changing local proof IDs.

This tool only reads retained reports, inventory, plans and atomic JSON,
verifies their pins and DEFLATE roundtrips, and publishes nine new fixtures.
It imports no source producer and executes no original, emulator or Native code.
Each selected scenario has exactly one normal return. Nine historical source
faults stay separate; no fault archive, retry, continuation, incident closure,
Windows/device or full-game acceptance is introduced by this packaging.
"""
import argparse
import base64
import hashlib
...
TASK = STUDY / "war-music-errors-native-20260912"
PLAN = TASK / "plan.json"
PLAN_SHA = "f6daf75f621fa473537202df01fc6e62bdb1fd486932ba21d49b8ea7a964f7cc"
...
def main():
    ...
    assert plan["sourceRuns"] == 0 and len(plan["inputs"]) == 57 and len(plan["protectedFixtures"]) == 350
    assert [(item["scenario"], item["ordinal"]) for item in plan["cases"]] == [(i, 0) for i in (0, 1, 20, 21, 23, 24, 25, 26, 27)]
    ...
    for item in plan["cases"]:
        ...
        failure_input = case["resourceFailureInput"]
        assert failure_input["declared"] == item["input"]
        assert failure_input["music"] == item["musicInput"] and failure_input["graphics"] == {}
        first_bound = 11 if scenario == 1 else 0
        assert item["prefixFirst"] == first_bound
        assert bool(item["input"]["control"]) == (scenario == 1)
        assert len(corpus["prefixProof"]) == 10
        prefix_links = []
        for index, proof in enumerate(corpus["prefixProof"]):
            bound_index = first_bound + index
            reference = bound["cases"][bound_index]
            # Local proof indices stay0..9 in the raw s01 file. The linked
            # accepted bound cases are11..20; never rewrite either sequence.
            assert proof["index"] == index and reference["index"] == bound_index
            assert proof["normalizedSHA256"] == reference["sha256"]
            prefix_links.append({"proofIndex": index, "boundIndex": bound_index,
        ...
        packed = envelope(raw)
        target = FIXTURES / f"original-lib-war-music-error-s{scenario:02}-call-00.json"
        ...
        artifacts.append({"scenario": scenario, "source": pin(path, raw), "fixture": pin(target, retained),
                          ...
                          "declaredScenario": item["input"], "declaredMusicInput": item["musicInput"],
                          "prefixFirst": first_bound, "declaredInputsMatchSavedCase": True,
    ...
    assert len(artifacts) == 9 and len(prefix_digests) == 20
    check_pins(plan["inputs"])
    check_pins(plan["protectedFixtures"])
    counts = {"returnedCalls": len(artifacts), "rawBytes": sum(x["source"]["bytes"] for x in artifacts),
              ...
              "distinctBoundPrefixes": len(prefix_digests), "normalControls": 2, "musicErrorCases": 7,
              "declaredMusicInputsCompared": len(artifacts), "selectedSourceFaults": 0,
              "historicalSourceFaultsExcluded": len(previous_faults),
              "sourceEvidencePinsUnchanged": 57, "oldFixturesUnchanged": 350}
