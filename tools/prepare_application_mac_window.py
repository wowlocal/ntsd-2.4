#!/usr/bin/env python3
"""Prepare one isolated resumable observed-startup candidate using existing pin/APFS clone procedure."""
...
B=(R/'build/research/application-observed-startup-correction1-20260926').resolve()
P=B.parent/'application-mac-window-20260926'
PLAN=R/'docs/research/APPLICATION_MAC_WINDOW_PLAN.md'
...
def main():
 ...
 pub=read(B/'publication1.json');assert pub['status']=='all25-passed' and read(B/'external-close1.json')['taskFrozen']
 ...
 inputs=read(B/'candidate1-inputs.json');assert len(inputs)==2254
 ...
 try:
  ...
  job.update(status='terminal',exitCode=0,files=2254,bytes=sum(r['bytes'] for r in inputs),fullCloneVerified=True,distinctRegularInodes=True)
