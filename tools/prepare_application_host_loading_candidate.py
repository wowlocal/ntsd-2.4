#!/usr/bin/env python3
"""Clone the validated host candidate for the bounded loading continuation."""
import ctypes
import datetime
import json
import os
...
BASE = (ROOT/'build/research/application-host-transaction-candidate-20260922').resolve()
TASK = Path('/Volumes/X5/ntsd-2.4-research/01a0c7b2-a956-7760-950e-ecf7dceaaa20/application-host-loading-candidate-20260922')
PLAN = ROOT/'docs/research/APPLICATION_HOST_LOADING_CANDIDATE_PLAN.md'
...
def main():
    ...
    validation = ROOT/'docs/evidence/application-host-transaction-validation.json'
    assert json.loads(validation.read_text())['tests']['all25Passed']
    ...
    job = dict(schema='ntsd-host-loading-prepare-v1', pid=os.getpid(), status='running', startedUTC=now(),
    ...
    try:
        protected = [PLAN, Path(__file__), ROOT/'tools/prepare_application_host_candidate.py',
            ...
            BASE/'selected-methods1.json', validation, ROOT/'docs/evidence/application-host-loading-preflight.json',
            ROOT/'docs/research/APPLICATION_HOST_LOADING_PREFLIGHT.md', root_manifest, source_manifest,
            ROOT/'AGENTS_HISTORY_2026-09-12.md', ROOT/'docs/research/WORKFLOW.md']
        context = dict(schema='ntsd-host-loading-candidate-context-v1', UTC=now(),
