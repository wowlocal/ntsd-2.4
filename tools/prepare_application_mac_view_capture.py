...
NAME = 'application-mac-view-capture-20260926'
...
def main():
    ...
    plan=ROOT/'docs/research/APPLICATION_MAC_VIEW_CAPTURE_PLAN.md'
    for src,dst in [(plan,'plan1.md'),(Path(__file__),'prepare1.py'),(ROOT/'tools/probe_application_mac_view_capture.swift','candidate1/probe.swift')]:
        shutil.copy2(src,P/dst)
    shutil.copy2(ROOT/'tools/OriginalMacViewCapture.swift',P/'candidate1/OriginalMacViewCapture.swift')
...
'''+adapted[end:]
    ...
    save('candidate1-inputs.json',[dict(path=n,**pin(P/'candidate1'/n)) for n in ['probe.swift','OriginalMacViewCapture.swift']])
    ...
    protected=[plan,Path(__file__),ROOT/'tools/probe_application_mac_view_capture.swift',P/'plan1.md',P/'prepare1.py',P/'adaptation1.json',OLD/'failure-diagnosis1.json',OLD/'publication1.json',OLD/'external-close1.json',OLD/'candidate1-inputs.json',ROOT/'AGENTS.md',ROOT/'AGENTS_HISTORY_2026-09-12.md',ROOT/'docs/research/WORKFLOW.md',ROOT/'docs/evidence/codex-safety-incidents-2026-09-12.json']
    protected += [ROOT/'tools/OriginalMacViewCapture.swift',ROOT/'tools/probe_application_mac_display_color.swift',ROOT/'docs/research/APPLICATION_MAC_DISPLAY_COLOR_DIAGNOSIS.md']
    diagnostic=OLD.parent/'application-mac-display-color-diagnosis-20260926'
    assert read(diagnostic/'external-close1.json')['taskFrozen']
    dj=read(diagnostic/'finalize1.job.json'); assert dj['exitCode']==0 and not subprocess.run(['ps','-p',str(dj['pid']),'-o','pid='],capture_output=True,text=True).stdout.strip()
    protected += [diagnostic/'publication1.json',diagnostic/'external-close1.json',diagnostic/'result2.json',diagnostic/'verification1.json']
    ...
    for phase,kind,command,seconds,rss in [
        ('build1','build',[swift,'-sdk',str(sdk),'-target','arm64-apple-macosx14.0','-parse-as-library',str(P/'candidate1/OriginalMacViewCapture.swift'),str(P/'candidate1/probe.swift'),'-o',str(P/'probe1'),'-module-cache-path',str(P/'modules')],300,4),
