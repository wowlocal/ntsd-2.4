#!/usr/bin/env python3
"""Read-only22-call War preparation with explicitly bound recording inputs.
...
LIB_WAR_PREPARATION_INPUTS.md defines the separate controlled input bridge.
Preserve the completed unbound capture3 and its failed finite acceptance.
No claim of exact reproduction of its ten later prefix calls is made.
"""
...
def verify(path):
 ...
 manifest=json.loads((path.parent/'war-preparation-bound-manifest1.json').read_text())
 ...
 for index,(spec,install) in enumerate(zip(manifest,d['installations'])):
  ...
  if spec.get('chain'):
   ...
   if 'preparationInputs' in spec:
    inputs=spec['preparationInputs'];assert index in (2,13)
   ...
   for change in bound+declared+c['stimulus']+c['callerABI']:
 ...
 assert stats['source2ParentReproductions']==4 and stats['preparationInputBridges']==2 and stats['preparationInputWrites']==14
 assert stats['definedPreparationNoticeReads']==2 and stats['recordingNoticeFormats']==1
 ...
 coverage=dict(preflightCalls=22,preparations=2,parentCallsReproduced=4,explicitControlledInputBridges=2,ownStartupProvenanceRecovered=False,recordingNoticeInputsBound=True,recordingInputsBound=not recording_unknown,complete=not recording_unknown)
