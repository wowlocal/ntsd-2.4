#!/usr/bin/env python3
"""Losslessly package the completed574/192/40-call NTSD War research corpora.

Reference: pinned NTSD EXE/lib/VC80 under controlled Unicorn2.1.4/CW023f,
ordinary ret1c/ret4 and one BEFORE43a21f preparation boundary. Separate
supplements cover44 cells and14 multi-human action masks. Require the
successful full read/store/REP/ownership audit and immutable atomic-case index.
Store each complete case separately within transport groups so Native tests
can retain game state without decoding the3.46GB document at once. Reconstruct
and hash every original raw byte, including metadata and the final newline.
Transport deflation is fixture storage only, never the game replay codec.
No original execution, expected edits, Native/Windows/device acceptance claim.
"""
...
def package(source,audit,index,output,profile='setup'):
