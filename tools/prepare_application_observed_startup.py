...
B=(R/'build/research/application-prepared-startup-platform-validation-20260926').resolve()
P=B.parent/'application-observed-startup-20260926'
PLAN=R/'docs/research/APPLICATION_OBSERVED_STARTUP_PLAN.md'
...
def main():
 ...
 pub=read(B/'publication1.json');assert pub['status']=='all11-passed' and read(B/'external-close1.json')['taskFrozen']
 ...
 inputs=read(B/'candidate1-inputs.json');assert len(inputs)==2249
 ...
 try:
  ...
  job.update(status='terminal',exitCode=0,files=2249,bytes=sum(r['bytes'] for r in inputs),fullCloneVerified=True,distinctRegularInodes=True)
