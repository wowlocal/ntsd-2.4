"""Close this fixed native display resource validation; reuse host finalizer checks and APFS archive procedure.
Build failure closure using the same archive/check procedures. No test results exist; no build, test or original executes.
"""
...
def main():
 assert P.name=='application-observed-bitmap-20260926'
 ...
 save('finalize-failure1.job.json',job)
 def guard():
 ...
 try:
  ...
  guard();checked(Path(__file__),read(P/'finalizer-failure-inputs1.json')['finalizer'])
  terminal(read(P/'prepare1.job.json'))
  build=read(P/'build1.job.json');terminal(build,False)
  assert build['exitCode']==1 and not build.get('guardReason') and not build.get('signals')
  assert not (P/'tests1-queue.job.json').exists() and not list(P.glob('test-*.job.json'))
  ...
  previous=native(Path(ctx['candidateSource']),P/'baseline-inputs1.json',2265)
  correction=read(P/'correction1.json');changes=correction['changes']
  assert len(changes)==9 and len(correction['newFiles'])==3
  modified={r['path'] for r in changes if r['before']};assert len(modified)==6
  ...
  save('patch-roundtrip1.json',dict(modified=sorted(modified),newFiles=correction['newFiles'],observedBitmapBridgeAdded=True,allOldTestBodiesUnchanged=True,roundtripBytesVerified=True))
  rootpatch=R/'docs/evidence/application-observed-bitmap.patch';assert not rootpatch.exists();rootpatch.write_bytes(patch.read_bytes())
  ...
  guard()
  pub=dict(schema='ntsd-observed-bitmap-validation-v1',UTC=now(),status='build-failed-before-tests',baseHead=ctx['head'],task=str(P),candidate=str(P/'candidate1'),candidateManifest=record(P/'candidate1-inputs.json'),context=record(P/'context1.json'),buildJob=record(P/'build1.job.json'),packageCheck=None,binary=None,testQueue=None,failureDiagnosis=record(P/'failure-diagnosis1.json'),verification=record(P/'verification1.json'),passedMethods=verification['passedMethods'],selectedMethods=69,artifactArchive=dict(path=str(archive),files=len(files),directories=len(dirs)+len(implicit),links=len(links),bytes=artifact['bytes'],manifest=record(P/'artifact-archive1.json')),metadataArchive=dict(members=len(members),**record(tarpath)),sourceObservation=record(P/'source-terminal1.json'),rootNativeUnchanged=1034,priorNativeUnchanged=2257,candidateUnchanged=2268,sourcePinsUnchanged=55,gates=dict(build=False,packageBytes=False,nativeComparison=all_passed,archive=True,independentReview=False,rootPromotion=False,actualWindowInputAudio=False,wholeCatalogSource=False,fullMatch=False,fullGame=False),EXEEnvelopeRecalculated=False,originalExecuted=False,limitations=['Controlled responses do not establish actual Windows or host device behavior.','Source59727 terminal at its previously recorded34-Object publication boundary; full137 and transport/provenance audits remain open.','The new observed-bitmap test has two helper-call overload errors; Core and MacPlatform compiled but the complete test package did not. All69 methods remain unstarted; prior all48 evidence is separate. Whole menu/physical bitmap comparison, independent review and application/Windows gates remain open.','Short processes can finish before RSS sample; recorded0 is not known zero.'])
  ...
  for local,suffix in [('publication1.json','.json'),('external-close1.json','-close.json')]:
   dest=R/('docs/evidence/application-observed-bitmap'+suffix);assert not dest.exists();dest.write_bytes((P/local).read_bytes())
  ...
  job.update(endedUTC=now(),elapsedSeconds=time.monotonic()-began);(P/'finalize-failure1.job.json').write_text(json.dumps(job,indent=2)+'\n');print(json.dumps(job),flush=True)
