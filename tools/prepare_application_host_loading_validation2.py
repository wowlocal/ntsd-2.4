...
BASE = ROOT/'build/research/application-host-loading-candidate-20260922'
TASK = Path('/Volumes/X5/ntsd-2.4-research/01a0c7b2-a956-7760-950e-ecf7dceaaa20/application-host-loading-correction1-20260922')
PLAN = ROOT/'docs/research/APPLICATION_HOST_LOADING_CORRECTION1_PLAN.md'
...
def main():
    ...
    try:
        ...
        before = f.read_text()
        old = '        XCTAssertEqual(Array(ready.operations.prefix(context.entry.stagedEffects.count)),context.entry.stagedEffects.map(M.S.Input.Operation.menu))'
        new = '\n'.join([
        ...
        context = dict(schema='ntsd-host-loading-validation-context-v1', UTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),
            ...
            correctionRound=1, changedPath=changed, beforeManifest=pin(TASK/'candidate-before1-inputs.json'),
