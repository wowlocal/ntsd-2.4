#!/usr/bin/env python3
"""Independently replay continuous menu music/bitmap source writes/read masks.
...
unknown; source-only operand cases are not native matches. See CHARACTER_MENU_MUSIC_SURFACE_PLAN.md and CHARACTER_MENU_MUSIC_LOG_PLAN.md. No corpus/expected/native input is modified.
"""
...
def verify(path):
 ...
 for c in d['cases']:
  ...
  seh=block(0,c['before']['seh'].to_bytes(4,'little'),b'\1'*4,'SEH')
  crt=block(d['crtDataAddress'],decoded[c['crtBefore']['data']],b'\1'*d['crtDataCount'],'CRT data')
  ptd=block(0x20000000,decoded[c['crtBefore']['ptd']],b'\1'*512,'CRT PTD')
  music={r['address']:block(r['address'],decoded[r['initial']],bytes(r['count']),'music-wide') for r in c['musicAllocations']}
  ...
  def snapshot(s,records):
   ...
   assert int.from_bytes(seh['bytes'],'little')==s['seh']
  ...
  origins=[];unknown=[]
  loader_lifetime=bytearray(stack['bytes'][c['bodySP']-0x118-d['fullStackAddress']:c['bodySP']-0x110-d['fullStackAddress']])
  loader_known=bytearray(8)
  assert len(loader_lifetime)==8
  for f in c['formats']:
   directory=decoded[c['before']['globals']][0x44ef38-0x44d000:].split(b'\0',1)[0]
   assert bytes(f['directory'])==directory
   assert bytes(f['format'])==b'%s\\graph.log' and bytes(f['bytes'])==directory+b'\\graph.log\0'
   assert f['result']==len(f['bytes'])-1
   counts['formats']+=1
   for i,byte in enumerate(f['bytes']):
    if 4<=i<12:loader_lifetime[i-4]=byte;loader_known[i-4]=1
  def music_snapshot(q):
   snapshot(q['snapshot'],[])
   equal(crt,q['crt']['data']);equal(ptd,q['crt']['ptd'])
   for r in q['allocations']:
    equal(music[r['address']],r['bytes'],r['mask']);assert r['guards']==['96'*16,'69'*16]
    counts['musicSnapshots']+=1;counts['musicSnapshotBytes']+=r['count']
  for index in range(len(writes)+1):
   while ri<len(reads) and reads[ri]['storeCount']==index:
    ...
    if q['pc'] in (0x40147e,0x40148a,0x43ed79,0x43ed94):
     last=r['last'][off:off+n];origin=dict(read=ri,pc=hex(q['pc']),address=hex(p),bytes=q['bytes'],writers=last)
     producers=[]
     for offset,wi in enumerate(last):
      if wi is None:
       assert not q['known'][offset];producers.append(dict(kind='unknown initial'));continue
      assert q['known'][offset];w=writes[wi]
      if w['pc'] is None:
       event=events[w['eventIndex']-1];kind=event['request']['kind']
       assert kind==('description' if q['pc'] in (0x40147e,0x40148a) else 'getObject'),('Dimension producer',w,event)
       assert len(bytes.fromhex(w['bytes']))==(108 if kind=='description' else 24)
       producers.append(dict(kind=kind,write=wi,event=w['eventIndex']-1,key=event['key']))
      else:
       assert q['pc'] in (0x43ed79,0x43ed94)
       h=next(h for h in c['helpers'] if h['kind']=='sprintf' and h['firstStore']<=wi<h['lastStore'])
       f=next(f for f in c['formats'] if f['sp']==h['sp'])
       pbyte=p+offset
       assert f['address']<=pbyte<f['address']+len(f['bytes'])
       assert r['bytes'][off+offset]==f['bytes'][pbyte-f['address']]
       assert hex(w['pc']) in c['instructions'] and w['pc']>=0x78000000
       producers.append(dict(kind='actual sprintf',write=wi,pc=hex(w['pc']),formatAddress=hex(f['address'])))
     origin['producers']=producers
   ...
   if c['musicBoundary']['storeCount']==index:
    assert c['musicBoundary']['readCount']==ri;music_snapshot(c['musicBoundary'])
   while ei<len(events) and events[ei]['storeCount']==index:
    ...
    if 'bytes' in q:
     if q['kind']=='getObject' and e['returnPC']==0x43ed6b:
      assert bytes(q['bytes'][4:12])==loader_lifetime,('Retained loader dimensions',c['spec']['label'],e['key'])
      p=c['bodySP']-0x118;off=p-stack['address']
      assert stack['mask'][off:off+8]==loader_known
      for w in e['response']['writes']:
       for i,byte in enumerate(w['bytes']):
        if 4<=w['offset']+i<12:loader_lifetime[w['offset']+i-4]=byte;loader_known[w['offset']+i-4]=1
      counts['loaderLifetimeEntries']+=1
  ...
  equal(crt,c['crtAfter']['data']);equal(ptd,c['crtAfter']['ptd'])
  for r in c['musicAllocations']:
   equal(music[r['address']],r['bytes'],r['mask']);counts['musicSnapshots']+=1;counts['musicSnapshotBytes']+=r['count']
  for h in c['musicCalls']:
   assert h['returnSP']==h['entrySP']+4;counts['musicHelperReturns']+=1
  for boundary in c['crtBoundaries']:
   assert boundary['name']=='_getptd' and hex(boundary['pc']) not in c['instructions'];counts['controlledCRTBoundaries']+=1
  for h in c['helpers']:
   assert h['returnSP']==h['sp']+4+h['pop']
   if h['kind']=='constructor':assert h['result']==h['wrapper']
   counts['helperReturns']+=1
  assert c['before']['cw']==c['after']['cw']==0x37f # Declared research CW, no hardware/device claim.
  for pc,code in c['instructions'].items():
   assert pc not in pcs or pcs[pc]==code;pcs[pc]=code
  if unknown:
   expected={'first-description-after-music':2,'object-after-create-negative':2,'object-after-cached':2,'object-after-disabled':2,'object-after-empty-directory':1}
   assert c['spec']['label'] in expected and len(unknown)==expected[c['spec']['label']]
   counts['undefinedReads']+=len(unknown);counts['undefinedReadBytes']+=sum(q['known'].count(0) for q in unknown)
