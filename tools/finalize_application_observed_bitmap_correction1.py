"""Close whole-Host observed-bitmap validation; reuse host finalizer checks and APFS archive procedure.
...
def main():
 assert P.name=='application-observed-bitmap-correction1-20260926'
 ...
 try:
  ...
  assert len(methods)==len(set(methods))==69 and len(selection['retainedMethods'])==64 and len(selection['newMethods'])==5 and 1<=count<=69
  assert methods[:count]==[r['method'] for r in queue['completed']]
  all_passed=count==69 and all(r['passed'] for r in queue['completed'])
  assert queue['exitCode']==(0 if all_passed else 1) and bool(queue.get('all69Passed'))==all_passed
  ...
  assert all(not (P/f'test-{i:02d}.job.json').exists() for i in range(count+1,70))
  ...
  candidate=native(P/'candidate1',P/'candidate1-inputs.json',2268)
  baseline=Path(ctx['baselineManifest']['path']);checked(baseline,ctx['baselineManifest'])
  previous=native(Path(ctx['candidateSource']),baseline,2268)
  correction=read(P/'correction1.json');changes=correction['changes']
  assert len(changes)==1 and not correction['newFiles']
  modified={r['path'] for r in changes if r['before']};assert len(modified)==1
  ...
  assert modified=={'native/Tests/NTSDCoreTests/OriginalApplicationObservedBitmapTests.swift'}
  changed=next(iter(modified));before=(Path(ctx['candidateSource'])/changed).read_text();after=(P/'candidate1'/changed).read_text()
  assert before.count(correction['oldExpression'])==2 and before.replace(correction['oldExpression'],correction['newExpression'])==after
  for row in changes:
  ...
  save('patch-roundtrip1.json',dict(modified=sorted(modified),newFiles=correction['newFiles'],observedBitmapBridgeAdded=True,allPriorTestAssertionsUnchanged=True,productionUnchanged=True,testInvocationCorrection=True,roundtripBytesVerified=True))
  rootpatch=R/'docs/evidence/application-observed-bitmap-correction1.patch';assert not rootpatch.exists();rootpatch.write_bytes(patch.read_bytes())
  package=read(P/'package-check1.json')
  assert package['allInputBytesVerified'] and package['sourceMembershipVerified'] and package['candidateFiles']==2268 and package['fixtureFiles']==385 and package['runtimeFiles']==1301
  assert {k:v['originalSources'] for k,v in package['targets'].items()}==dict(NTSDCore=201,NTSDReferenceChecks=61,NTSDCoreTests=282,NTSDMacPlatform=7)
  ...
  assert sum(r['path'].startswith('candidate1/') for r in files)==2268 and sum(r['path'].startswith('swift/') for r in files)==release_count and len(files)==2268+release_count and not links
  artifact=dict(schema='ntsd-observed-bitmap-validation-artifact-archive-v1',UTC=now(),files=files,directories=dirs,implicitParentDirectories=sorted(implicit),links=links,bytes=sum(r['bytes'] for r in files),fullBodyModeMtimeMembershipVerified=True,distinctRegularInodes=True)
  ...
  verification=dict(UTC=now(),all69Passed=all_passed,passedMethods=sum(t['passed'] for t in tests),selectedMethods=69,unstartedMethods=methods[count:],tests=tests,totalTestProcessSeconds=sum(t['seconds'] for t in tests),sampledTestPeakRSS=max(t['sampledTreeRSS'] for t in tests),zeroRSSMeansNoSample=True,taskProcessAbsenceVerified=True,rootNativeFiles=1034,priorNativeFiles=2257,candidateFiles=2268,sourceCodePins=55,allPreserved=True,resourceFiles=1686,independentReview=False)
  ...
  guard()
  pub=dict(schema='ntsd-observed-bitmap-validation-v1',UTC=now(),status='all69-passed' if all_passed else 'first-nonpass-retained',baseHead=ctx['head'],task=str(P),candidate=str(P/'candidate1'),candidateManifest=record(P/'candidate1-inputs.json'),context=record(P/'context1.json'),buildJob=record(P/'build1.job.json'),packageCheck=record(P/'package-check1.json'),binary=package['binary'],testQueue=record(P/'tests1-queue.job.json'),verification=record(P/'verification1.json'),passedMethods=verification['passedMethods'],selectedMethods=69,artifactArchive=dict(path=str(archive),files=len(files),directories=len(dirs)+len(implicit),links=len(links),bytes=artifact['bytes'],manifest=record(P/'artifact-archive1.json')),metadataArchive=dict(members=len(members),**record(tarpath)),sourceObservation=record(P/'source-terminal1.json'),rootNativeUnchanged=1034,priorNativeUnchanged=2257,candidateFilesVerified=2268,observedBitmapBridgeAdded=True,allPriorTestAssertionsUnchanged=True,productionUnchanged=True,testInvocationCorrection=True,sourcePinsUnchanged=55,gates=dict(build=True,packageBytes=True,nativeComparison=all_passed,archive=True,independentReview=False,rootPromotion=False,actualWindowInputAudio=False,wholeCatalogSource=False,fullMatch=False,fullGame=False),EXEEnvelopeRecalculated=False,originalExecuted=False,limitations=['Native bitmap/DC resources and one-to-one XRGB conversion are explicit host policy, not actual Windows GDI raster observations.','Source59727 terminal at its previously recorded34-Object publication boundary; full137 and transport/provenance audits remain open.','Unified startup is tested with declared responses plus a separate actual Mac clock whole-caller control if its exact method passes. Physical display allocation/clear/AppKit view readback methods are separately observed if passed. Native XRGB8888/logical-screen/refcount policies do not establish Windows equivalence. Whole first-menu bitmap Host integration is tested with physical bitmap replies and declared other inputs; a separate prepared replay of those API values checks whole Core state. General blit/presentation/palette/text/callbacks/other providers/input/audio remain open; NTSDApp still uses practice. Retained inspection controls are finite scheduler observations.','Short processes can finish before RSS sample; recorded0 is not known zero.'])
  save('publication1.json',pub)
  close=dict(schema='ntsd-observed-bitmap-validation-close-v1',UTC=now(),taskFrozen=True,largePhaseClosed=True,task=str(P),externalFree=shutil.disk_usage(P).free,internalFree=shutil.disk_usage(R).free,externalReserve=40*2**30,internalReserve=6*2**30,sourceCommitment=17*2**30,observedFreeDecrease=ctx['startFree']-shutil.disk_usage(P).free,physicalAllowance=ctx['physicalAllowanceBytes'],publication=pin(P/'publication1.json'),metadataArchive=pub['metadataArchive'],artifactArchive=pub['artifactArchive'])
  save('external-close1.json',close)
  for local,suffix in [('publication1.json','.json'),('external-close1.json','-close.json')]:
   dest=R/('docs/evidence/application-observed-bitmap-correction1'+suffix);assert not dest.exists();dest.write_bytes((P/local).read_bytes())
  job.update(status='terminal',exitCode=0,all69Passed=all_passed,passedMethods=verification['passedMethods'],archivedFiles=len(files),metadataMembers=len(members))
