#!/usr/bin/env python3
"""Read-only audit of whole Tournament preparation and retained menu calls.
...
returns. Verify Actor-constructor provenance, full replay reads and FD/FE/FF,
date/local string backing and actual4347ba rootSP10 with pending12arguments.
LIB_TOURNAMENT_ARENA_RELEASE_PLAN.md defines finite acceptance and open boundaries.
"""
...
def verify(path):
 ...
 assert len(d['cases'])==len(d['installations'])==51
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-tournament-arena-release-finite-inputs1.json').read_bytes())
 ...
 for c,install in zip(d['cases'],d['installations']):
  ...
  assert c['instructions']['0x422ab8']=='c20400' and c['instructions']['0x434aaa']=='c20c00'
  if c['spec'].get('chain'):
   prior=records(previous['after'])
   assert prior==records(c['bridge']['before']),(label,'whole retained bridge input')
   expected_bridge=[dict(address=int(p),bytes=struct.pack('<I',v).hex()) for p,v in c['spec']['bridgeGlobals'].items()]
   assert c['bridge']['writes']==expected_bridge and {x['address'] for x in expected_bridge}=={0x44d020,0x44d024,0x44d028}
   for change in c['bridge']['writes']+c['stimulus']+c['callerABI']:
  ...
  for step in range(len(c['writes'])+1):
   for point in points.pop(step,[]):
    ...
    if point['kind'].startswith('preparation-'):
     pc=int(point['kind'].split('-')[1],16);root=c['characterSP']-0xa14
     assert point['sp']==root-(12 if pc==0x4347ba else 0) and point['fpu']['cw']==0x23f
     if pc==0x4347ba:
      _,v,m,o=owner(point['sp']+0x1c,4);assert v[o:o+4]==struct.pack('<I',0x44d020) and m[o:o+4]==b'\1'*4
     if point['name'] is not None:
      b=bytes.fromhex(point['name'])+b'\0';_,v,m,o=owner(root+0x80c,len(b));assert v[o:o+len(b)]==b and m[o:o+len(b)]==b'\1'*len(b)
 ...
 assert stats['wholePreparations']==51 and stats['constructorReturns']==17*401 and all(streams[s]>0 for s in (0xfe,0xff))
 assert stats['releasePairs']==302 and stats['bitmapAllocations']==604 and stats['staleTailReloads']==17
