...
NAME='application-observed-startup-validation-20260926'
...
OLD=(ROOT/'build/research/application-prepared-startup-platform-validation-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_OBSERVED_STARTUP_VALIDATION_PLAN.md'
...
def generate(name):
 ...
 if name==NAMES[0]:
  new=once(new,"r'build1|test-(0[1-9]|1[01])'","r'build1|test-(0[1-9]|1[0-9]|2[0-5])'")
  marker='    def identity(pid):';assert old[old.index(marker):]==new[new.index(marker):]
 elif name==NAMES[1]:
  new=once(new,'11-method','25-method');new=once(new,'== 11','== 25');new=once(new,'all11Passed','all25Passed')
  begin,end='            done = re.findall(',"            job['completed'].append("
  assert old[old.index(begin):old.index(end)]==new[new.index(begin):new.index(end)]
 else:
  new=once(new,"'NTSDCoreTests':276","'NTSDCoreTests':277");new=once(new,"'NTSDCore':195","'NTSDCore':199")
