#!/usr/bin/env python3
"""Losslessly package eight saved, returned NTSD War graphics-error calls.

Pinned EXE/lib/VC80 observations used controlled Unicorn2.1.4/CW023f/C locale
and declared first-layer GetDC/Restore/CreateDC/SelectObject/StretchBlt/
ReleaseDC/DeleteDC/DeleteObject responses. Full records, masks, event order,
owners and blobs are retained to compare Native arena preparation.

This tool only reads retained reports, inventory, plans and atomic JSON,
verifies their pins and DEFLATE roundtrips, and publishes eight new fixtures.
...
TASK = STUDY / "war-graphics-errors-native-20260912"
PLAN = TASK / "plan.json"
PLAN_SHA = "db7e5a9b5b29a1100079b36278294837f50f4fd344d93fd5d5e327a82405fed8"
...
def main():
    ...
    assert plan["sourceRuns"] == 0 and len(plan["inputs"]) == 45 and len(plan["protectedFixtures"]) == 342
    assert [(item["scenario"], item["ordinal"]) for item in plan["cases"]] == [(i, 0) for i in range(12, 20)]
    ...
    for item in plan["cases"]:
        ...
        packed = envelope(raw)
        target = FIXTURES / f"original-lib-war-graphics-error-s{scenario:02}-call-00.json"
    ...
    counts = {"returnedCalls": len(artifacts), "rawBytes": sum(x["source"]["bytes"] for x in artifacts),
              ...
              "sourceEvidencePinsUnchanged": 45, "oldFixturesUnchanged": 342}
