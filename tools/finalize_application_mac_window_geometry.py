"""Close whole-Host Native window-geometry validation; reuse host finalizer checks and APFS archive procedure.
...
def main():
 assert P.name=='application-mac-window-geometry-20260926'
 ...
 try:
  ...
  assert len(methods)==len(set(methods))==76 and len(selection['retainedMethods'])==72 and len(selection['newMethods'])==4 and 1<=count<=76
  assert methods[:count]==[r['method'] for r in queue['completed']]
  all_passed=count==76 and all(r['passed'] for r in queue['completed'])
  assert queue['exitCode']==(0 if all_passed else 1) and bool(queue.get('all76Passed'))==all_passed
  ...
  assert all(not (P/f'test-{i:02d}.job.json').exists() for i in range(count+1,77))
  ...
  candidate=native(P/'candidate1',P/'candidate1-inputs.json',2272)
  ...
  assert len(changes)==9 and len(correction['newFiles'])==4
  modified={r['path'] for r in changes if r['before']};assert len(modified)==5
  ...
  save('patch-roundtrip1.json',dict(modified=sorted(modified),newFiles=correction['newFiles'],nativeWindowGeometryBridgeAdded=True,allOldTestBodiesUnchanged=True,roundtripBytesVerified=True))
  rootpatch=R/'docs/evidence/application-mac-window-geometry.patch';assert not rootpatch.exists();rootpatch.write_bytes(patch.read_bytes())
  package=read(P/'package-check1.json')
  assert package['allInputBytesVerified'] and package['sourceMembershipVerified'] and package['candidateFiles']==2272 and package['fixtureFiles']==385 and package['runtimeFiles']==1301
  assert {k:v['originalSources'] for k,v in package['targets'].items()}==dict(NTSDCore=203,NTSDReferenceChecks=61,NTSDCoreTests=283,NTSDMacPlatform=8)
  ...
  assert sum(r['path'].startswith('candidate1/') for r in files)==2272 and sum(r['path'].startswith('swift/') for r in files)==release_count and len(files)==2272+release_count and not links
  artifact=dict(schema='ntsd-mac-window-geometry-validation-artifact-archive-v1',UTC=now(),files=files,directories=dirs,implicitParentDirectories=sorted(implicit),links=links,bytes=sum(r['bytes'] for r in files),fullBodyModeMtimeMembershipVerified=True,distinctRegularInodes=True)
  ...
  verification=dict(UTC=now(),all76Passed=all_passed,passedMethods=sum(t['passed'] for t in tests),selectedMethods=76,unstartedMethods=methods[count:],tests=tests,totalTestProcessSeconds=sum(t['seconds'] for t in tests),sampledTestPeakRSS=max(t['sampledTreeRSS'] for t in tests),zeroRSSMeansNoSample=True,taskProcessAbsenceVerified=True,rootNativeFiles=1034,priorNativeFiles=2257,candidateFiles=2272,sourceCodePins=55,allPreserved=True,resourceFiles=1686,independentReview=False)
  ...
  guard()
  pub=dict(schema='ntsd-mac-window-geometry-validation-v1',UTC=now(),status='all76-passed' if all_passed else 'first-nonpass-retained',baseHead=ctx['head'],task=str(P),candidate=str(P/'candidate1'),candidateManifest=record(P/'candidate1-inputs.json'),context=record(P/'context1.json'),buildJob=record(P/'build1.job.json'),packageCheck=record(P/'package-check1.json'),binary=package['binary'],testQueue=record(P/'tests1-queue.job.json'),verification=record(P/'verification1.json'),passedMethods=verification['passedMethods'],selectedMethods=76,artifactArchive=dict(path=str(archive),files=len(files),directories=len(dirs)+len(implicit),links=len(links),bytes=artifact['bytes'],manifest=record(P/'artifact-archive1.json')),metadataArchive=dict(members=len(members),**record(tarpath)),sourceObservation=record(P/'source-terminal1.json'),rootNativeUnchanged=1034,priorNativeUnchanged=2257,candidateFilesVerified=2272,nativeWindowGeometryBridgeAdded=True,allOldTestBodiesUnchanged=True,sourcePinsUnchanged=55,gates=dict(build=True,packageBytes=True,nativeComparison=all_passed,archive=True,independentReview=False,rootPromotion=False,actualWindowInputAudio=False,wholeCatalogSource=False,fullMatch=False,fullGame=False),EXEEnvelopeRecalculated=False,originalExecuted=False,limitations=['Physical geometry is a Native integral logical-point/top-left-current-screen mapping, not a measured Windows/DPI/multi-screen contract.',
   'All76 compares whole WM_MOVE and following first-menu presentation projection, saved replies/late rollback, owners and numeric failure controls; DefWindowProc and other queue/API inputs are explicit controls. No original execution or automatic callback delivery.',
   'Source59727 terminal at34 Objects; full137 and transport/provenance audits remain open. Independent review/root promotion/general raster/other providers/loading/Windows/input/audio/clean-Mac/full match/game remain open; NTSDApp still Practice.',
   'Short processes can finish before RSS sample; recorded0 is not known zero.'])
  save('publication1.json',pub)
  close=dict(schema='ntsd-mac-window-geometry-validation-close-v1',UTC=now(),taskFrozen=True,largePhaseClosed=True,task=str(P),externalFree=shutil.disk_usage(P).free,internalFree=shutil.disk_usage(R).free,externalReserve=40*2**30,internalReserve=6*2**30,sourceCommitment=17*2**30,observedFreeDecrease=ctx['startFree']-shutil.disk_usage(P).free,physicalAllowance=ctx['physicalAllowanceBytes'],publication=pin(P/'publication1.json'),metadataArchive=pub['metadataArchive'],artifactArchive=pub['artifactArchive'])
  save('external-close1.json',close)
  for local,suffix in [('publication1.json','.json'),('external-close1.json','-close.json')]:
   dest=R/('docs/evidence/application-mac-window-geometry'+suffix);assert not dest.exists();dest.write_bytes((P/local).read_bytes())
  job.update(status='terminal',exitCode=0,all76Passed=all_passed,passedMethods=verification['passedMethods'],archivedFiles=len(files),metadataMembers=len(members))
