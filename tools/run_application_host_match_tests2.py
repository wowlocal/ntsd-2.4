#!/usr/bin/env python3
"""Run the frozen 53-method host selection through the existing guarded runner."""
import datetime
import json
import os
...
from run_application_host_match_validation2 import main as run_one
...
def main(task):
    ...
    for index, method in enumerate(methods, 1):
        ...
        config = dict(template, phase=phase, kind='test',
            ...
            timeoutSeconds=900 if index >= 51 else 600,
            residentGuardBytes=(16 if index >= 51 else 12 if index == 28 or 35 <= index <= 37 or 43 <= index <= 50 else 8)*2**30, selectionDeadlineUnix=deadline)
        path = task/(phase+'-config.json')
        assert not path.exists()
        path.write_text(json.dumps(config, indent=2)+'\n')
        cases.append(dict(method=method, phase=phase, config=str(path), pin=pin(path)))
    selection = dict(schema='ntsd-host-match-test-commands-v1', cases=cases, packageCheck=pin(task/'package-check1.json'),
        selectedMethods=pin(task/'selected-methods1.json'), queueProducer=pin(Path(__file__)),
        runner=pin(ROOT/'tools/run_application_host_match_validation2.py'), completeResultParser='same exact named XCTest regex and one-test zero-failure summary as Catalog53 completion')
    ...
    try:
        for case in cases:
            ...
            job.update(status='terminal', exitCode=0, all53Passed=True)
