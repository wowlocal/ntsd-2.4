#!/usr/bin/env python3
"""Prepare exact observed startup validation with the existing guarded host tools."""
...
NAME='application-observed-startup-correction1-20260926'
TASK=Path('/Volumes/X5/ntsd-2.4-research/01a0dc49-738f-7972-8fb0-e98fb2f34408')/NAME
BASE=(ROOT/'build/research/application-observed-startup-20260926').resolve()
OLD=(ROOT/'build/research/application-observed-startup-validation-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_OBSERVED_STARTUP_CORRECTION1_PLAN.md'
...
def main():
 ...
 for parent,names in [(BASE,['prepare1.job.json','parse1.job.json','finalize1.job.json']),(OLD,['prepare1.job.json','finalize-failure1.job.json'])]:
  for n in names:
   j=read(parent/n);assert j['status']=='terminal' and j['exitCode']==0 and absent(j['pid'])
 failed=read(OLD/'build1.job.json');assert failed['status']=='terminal' and failed['exitCode']==1 and absent(failed['pid'])
 assert read(OLD/'external-close1.json')['taskFrozen'] and not (OLD/'tests1-queue.job.json').exists()
 ...
 try:
  ...
  rows=read(BASE/'candidate1-inputs.json');assert len(rows)==2254
  ...
  changed='native/Tests/NTSDCoreTests/OriginalApplicationObservedStartupTests.swift'
  destination=TASK/'candidate1'/changed;before=destination.read_text()
  old_import='import NTSDReference\n';new_import='@testable import NTSDReferenceChecks\n'
  assert before.count(old_import)==1 and before.startswith('import XCTest\n')
  after='import Foundation\n'+before.replace(old_import,new_import)
  destination.write_text(after)
  assert after.removeprefix('import Foundation\n').replace(new_import,old_import)==before
  ...
  save('correction1.json',dict(path=changed,before=record(BASE/'candidate1'/changed),after=record(destination),onlyImportsChanged=True))
  ...
  for src,name in [(BASE/'selected-methods1.json','selected-methods1.json'),(PLAN,'plan1.md'),(Path(__file__),'prepare1.py')]:shutil.copy2(src,TASK/name)
  ...
  assert len(methods)==len(set(methods))==25 and len(new)==4 and len(old)==21 and methods==new+old
  priorContext=OLD.parent/'application-window-exchange-validation-20260926'
  ...
  save('method-limits1.json',dict(methods=limits,queueSeconds=14400,inheritedMethods=21,priorLimits=record(OLD/'method-limits1.json'),priorContextLimits=record(priorContext/'method-limits1.json'),priorSelection=record(OLD/'selected-methods1.json'),precedingComparisonsAreSeparate=True))
  ...
  pattern=r'build1|test-(0[1-9]|1[0-9]|2[0-5])'
  assert all(re.fullmatch(pattern,p) for p in ['build1']+[f'test-{i:02d}' for i in range(1,26)])
  invalid=['build0','build2','test-00','test-26','test-99','test-1','test-01x'];assert not any(re.fullmatch(pattern,p) for p in invalid)
  save('runner-controls1.json',dict(UTC=now(),monitorBodyUnchanged=True,resultPredicatesUnchanged=True,cwdPositive=1,cwdNegative=4,compilerPositive=1,compilerNegative=2,transientPositive=1,transientNegative=1,phasePositive=26,phaseNegative=7,exactMethods=25,oldMethodLimitsUnchanged=True,nativeExecuted=False,originalExecuted=False,independentReview=False))
  job.update(status='terminal',exitCode=0,files=2254,preparedMethods=len(methods),buildConfig=record(TASK/'build1-config.json'))
