#!/usr/bin/env python3
"""Read-only audit of declared installed-library menu continuation and actual return.
...
See docs/research/LIB_MENU_CONTINUATION_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 assert len(d['cases'])==len(d['installations'])==60
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-menu-continuation-finite-inputs.json').read_bytes())
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  ...
  assert len(state)==25 and len(alive)==8
   ...
   for point in points.pop(step,[]):
    ...
    for x in point['records']:
     ...
     if x['live'] is not None:
      expected=alive[x['address']] and not any(e['kind']=='free' and e['arguments'][0]==x['address'] for e in c['events'][:point['eventCount']])
  ...
  assert c['screenSP']==d['tailSP']-0xac8 and c['points'][0]['sp']==d['tailSP']==d['entrySP']-0x644
  if c['end']=='returned':
   assert c['endSP']==d['entrySP']+8 and not c['pending'] and c['endPC']==0x30000000
   assert [p['kind'] for p in c['points']]==['declaredTail','screenReturned','menuReturned','matchBeforeReturn','returned']
  ...
  else:assert [p['kind'] for p in c['points']]==['declaredTail']
 ...
 for i,c in enumerate(d['cases']):
  directory=path.parent/'lib-menu-continuation-capture1.parts'
 ...
 probe=json.loads((path.parent/'lib-menu-continuation-probe1.json').read_bytes());assert probe['cases'][0]==d['cases'][0]
