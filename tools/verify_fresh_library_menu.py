...
See docs/research/FRESH_LIBRARY_MENU_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 assert len(d['cases'])==len(d['installations'])==26
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'fresh-library-menu-finite-inputs.json').read_bytes())
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  ...
  assert len(state)==37 and len(alive)==20
  ...
  if c['end']!='nullSpark':assert c['screenSP']==d['tailSP']-0xac8
  else:assert c['screenSP'] is None and c['endPC']==0x429b21 and len(c['pending'])==1
  if c['end']=='returned':
   assert c['endSP']==d['entrySP']+8 and not c['pending'] and c['endPC']==0x30000000
   assert [p['kind'] for p in c['points']][-4:]==['screenReturned','menuReturned','matchBeforeReturn','returned']
  ...
  if c['end']=='playback':assert c['endSP']==c['screenSP']-0x718 and len(c['pending'])==2
  if c['end']=='zeroTimerRange':assert c['endPC']==0x423be8 and len(c['pending'])==3
  if c['end']=='noSelectableRow':assert c['endPC']==0x423ed0 and c['scans']=={str(0x423ed0):9} and len(c['pending'])==3
  ...
  if label.startswith('confirm-release'):assert kinds.index('free')<kinds.index('panel')
 ...
 for c in d['cases']:
  ...
  if c['screenSP'] is not None:
   h=next(h for h in c['helpers']+c['pending'] if h['entry']==0x431d10);lo=c['screenSP']-0x718+0x10;size=0x704
   first,last=h['firstStore'],h.get('lastStore',len(c['writes']));current=bytearray(size);local_reads=collections.defaultdict(list)
   for kind,items in [('instruction',c['reads']),('API',c['apiReads'])]:
    for read in items:
     if first<=read['storeCount']<=last and read['address']<lo+size and lo<read['address']+read['count']:local_reads[read['storeCount']].append((kind,read))
   for step in range(first,last+1):
    for kind,read in local_reads[step]:
     a,b=max(lo,read['address']),min(lo+size,read['address']+read['count'])
     assert all(current[a-lo:b-lo]),(c['spec']['label'],'prior helper lifetime consumed',kind,read)
     stats['screenLocal'+kind+'Reads']+=1;stats['screenLocal'+kind+'ReadBytes']+=b-a
    if step<last:
     w=c['writes'][step];a,b=max(lo,w['address']),min(lo+size,w['address']+len(bytes.fromhex(w['bytes'])))
     if a<b:current[a-lo:b-lo]=b'\1'*(b-a)
   assert sum(current)==108
   stats['screenLocalCurrentWrittenBytes']+=sum(current)

 # Independently compare every retained atomic source part, including nominal.
 for i,c in enumerate(d['cases']):
  directory=path.parent/'fresh-library-menu-capture1.parts'
 ...
 probe=json.loads((path.parent/'fresh-library-menu-probe2.json').read_bytes());assert probe['cases'][0]==d['cases'][0]
