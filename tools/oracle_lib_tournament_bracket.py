...
# ///
"""Tournament26..29 bracket/CPU simulation/human seat selection/winner output.
Pinned original NTSD EXE/lib/VC80/raw DAT/BMP/DIB; controlled Unicorn2.1.4
CW023f and declared API/COM responses. Actual instruction/read/write/stack
traces recover RNG/HP/order and retained storage. Fresh menu20 setup through
actual422ab8ret4 or BEFORE434349 Tournament match preparation. No own complete
catalog/match/Windows/device claim. ActorCA inputs explicitly model released/
previous Attack from419a60; no unknown backing or source expected after-state
is imported. Geometry is immutable EXE data. No control-pointer/protection
corruption or fault continuation. LIB_TOURNAMENT_BRACKET_PLAN.md defines finite
acceptance; new preparation/replay composition remains required next.
"""
...
def sequence(control):
 first=old_sequence(control)[0];suffix='-cpu' if control else '-human';first['label']='bracket-initial'+suffix
 first['globals'].update({0x44d020:20,0x451160:2,0x45116c:BM+0x4000,0x4513e0:29 if control else 0,0x44d024:0,0x44d028:0,0x458428:0,0x44f18c:1})
 out=[first];held=[0]*8
 ...
 for seat in range(8):
  ...
  if not control:edge('human-'+str(seat),0xcf)
  edge('ready-'+str(seat),0xd1)
 edge('no-shuffle',0xd0);edge('randomize',0xd1)
 edge('option-up1',0xcd);edge('option-start',0xcd)
 edge('start',0xd1)
 if control:
  # Start and its release advance25->27.53 natural calls reach exactly80.
  for i in range(53):frame('natural-'+str(i))
  # Each edge supplies Jump once; the release advances pair/round scanning.
  ...
  edge('claim-first',0xd1)
  frame('claim-first-held',[(0,0xd1,1)])
  frame('claim-first-release',[(0,0xd1,0)])
  edge('claim-second',0xd1,1)
  edge('dialog-no',0xd0);edge('cancel-seats',0xd1)
  edge('reclaim-first',0xd1)
  edge('reclaim-second',0xd1,1)
  edge('dialog-yes',0xcf)
  frame('prepare-match',[(0,0xd1,1)],end='tournamentMatchPreparation')
 if control:
  for name,menu,pair,difficulty,winner in [('special-first',26,[0,17],-1,10),('special-second',26,[17,0],2,10),('both-special',26,[0,4],0,10),('result-team',28,[17,21],0,10),('result-draw',28,[17,21],0,0),('winner-human',29,[17,21],0,10)]:
   item=copy.deepcopy(first);item['label']='controlled-'+name;item['controlledBracket']=True
   g=item['globals'];g.update({0x44d020:menu,0x4513e8:8,0x4513e4:0,0x4513dc:0,0x4513d8:49 if menu==29 else 79,0x4513d0:2,0x4513c8:0,0x4513cc:1,0x4513d4:0,0x451340:0,0x44d318:4 if menu==28 else 0,0x450c30:difficulty,0x450bf8:winner})
   for i in range(8):g.update({0x44d0c0+i*4:(pair+[17]*6)[i],0x44d0e0+i*4:i,0x451364+i*4:1 if menu==29 and i==0 else 0,0x451384+i*4:(7 if menu==29 and i==0 else 5 if menu==28 and i<2 else 0 if i<2 else 1),0x44d080+i*4:500-i*41,0x44d0a0+i*4:-1,0x451344+i*4:0})
   for i in range(20):g[0x4512d0+i*4]=i if menu==28 and i<2 else -1
   item['buttons']=[[seat,0xca,0] for seat in range(8)]
   if menu==29:item['buttons'].append([0,0xd1,1])
   # Declared post-fight scalar producer outputs, ordinary positive HP/state.
   item['actorWords']=[[i,off,value] for i in range(2) for off,value in [(0x300,321-i*123),(0x33c,4+i)]] if menu==28 else []
   out.append(item)
  item=copy.deepcopy(out[-1]);item['label']='controlled-held-attack';item['globals'].update({0x44d020:27,0x4513d8:0,0x451384:1});item['buttons'][0]=[0,0xca,1]
  out.append(item)
 return out

class TournamentBracket(Selection):
 ...
 def code(self,u,pc,n,data):
  if self.running and not self.prologue and self.startup_done:
   ...
   if 0x432ab0<=pc<=0x434aaa:
    ...
    if pc in (0x432af2,0x432be2,0x432c76,0x433311,0x433497,0x433550,0x433660,0x4338c3,0x433ae7,0x433bc9,0x433c22,0x434349,0x4347c5,0x434992,0x434a92):
    ...
    if pc==0x434349:
     assert self.u32(0x44d020)==27 and not self.graphics.helpers and not self.music.music_pending
     assert [h['entry'] for h in self.pending]==[0x429730,0x432ab0]
     self.end='tournamentMatchPreparation';u.emu_stop();return
    self.pcs[hex(pc)]=bytes(u.mem_read(pc,n)).hex();return
   if pc==0x417170 and self.u32(sp+4) in (0xf7,0xf8,0xf9,0xfa,0xfb,0xfc,0x100):
    tag=self.u32(sp+4);limit=self.u32(sp+8);assert self.u32(sp)=={0xf7:0x4334bc,0xf8:0x4334ca,0xf9:0x433626,0xfa:0x43406e,0xfb:0x43406e,0xfc:0x4340d3,0x100:0x4348eb}[tag]
    if tag==0xf9:
    ...
    elif tag in (0xf7,0xf8):assert limit==8
    elif tag!=0xfc:assert limit==2
...
if __name__=='__main__':
 ...
 for control in (False,True):
  vm=TournamentBracket(control,inputs,arenas);vm.capture_path=a.output
  for index,spec in enumerate(sequence(control)):
   if a.limit is not None and len(cases)>=a.limit:break
   if index>0 and not spec.get('chain'):vm=TournamentBracket(control,inputs,arenas);vm.capture_path=a.output
  ...
  if a.limit is not None and len(cases)>=a.limit:break
  assert vm.end==('returned' if control else 'tournamentMatchPreparation') and vm.u32(0x44d020)==27
