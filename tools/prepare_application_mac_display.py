#!/usr/bin/env python3
"""Prepare one isolated native display-resource candidate using existing pin/APFS clone procedure."""
...
B=(R/'build/research/application-mac-window-validation-20260926').resolve()
P=B.parent/'application-mac-display-20260926'
PLAN=R/'docs/research/APPLICATION_MAC_DISPLAY_PLAN.md'
...
def main():
 ...
 pub=read(B/'publication1.json');assert pub['status']=='all29-passed' and read(B/'external-close1.json')['taskFrozen']
 ...
 inputs=read(B/'candidate1-inputs.json');assert len(inputs)==2257
 ...
 try:
  ...
  job.update(status='terminal',exitCode=0,files=2257,bytes=sum(r['bytes'] for r in inputs),fullCloneVerified=True,distinctRegularInodes=True)
