...
def main(config_path,config_sha):
    ...
    assert P.name=='application-host-loading-regressions-20260922'
    ...
    assert A.name=='application-host-loading-correction3-20260922' and A.parent==P.parent
    assert config['artifactRoot']==str(A) and read(A/'external-close1.json')['taskFrozen']
    assert re.fullmatch(r'test-(3[5-9]|4[0-2])',phase)
    ...
    assert config['residentGuardBytes']==(12 if phase in ('test-35','test-36','test-37') else 8)*2**30
