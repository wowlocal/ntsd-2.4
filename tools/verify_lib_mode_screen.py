#!/usr/bin/env python3
"""Read-only audit of the controlled installed-library mode-screen corpus.
...
See docs/research/LIB_MODE_SCREEN_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 assert len(d['cases'])==len(d['installations'])==112
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  ...
  assert len(state)==21+(label=='enabled-panel') and len(alive)==7
  assert c['cw']==0x23f and c['end'] in ('returned','playback','enabledPanel','workerChild')
  ...
  for event in c['events']:
   ...
   if event['kind']=='fill':
    ...
    assert f['effects'][:4]==[100,0,0,0] and f['effects'][0x50:0x54]==[0x65,0x25,0x12,0]
  for x in c['after']:
 ...
 for i,c in enumerate(d['cases']):
  directory=path.parent/('lib-mode-screen-capture1.parts' if i<110 else 'lib-mode-screen-boundaries1.parts')
 ...
 probe=json.loads((path.parent/'lib-mode-screen-probe3.json').read_bytes());assert probe['cases'][0]==d['cases'][0]
