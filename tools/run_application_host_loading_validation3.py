...
def main(config_path,config_sha):
    ...
    assert P.name=='application-host-loading-completion-20260922'
    ...
    assert A.name=='application-host-loading-correction1-20260922' and A.parent==P.parent
    assert config['artifactRoot']==str(A) and read(A/'external-close1.json')['taskFrozen']
    assert re.fullmatch(r'test-(2[89]|3[0-9]|4[0-2])',phase)
    ...
    assert config['residentGuardBytes']==(12 if phase=='test-28' else 8)*2**30
