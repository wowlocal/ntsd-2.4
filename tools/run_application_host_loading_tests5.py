#!/usr/bin/env python3
"""Run the eight remaining whole methods through the existing guarded runner."""
import datetime
import json
import os
...
from run_application_host_loading_validation5 import main as run_one
...
def main(task):
    ...
    cases = []
    for index, method in enumerate(methods, 35):
        ...
        config = dict(template, phase=phase, kind='test',
            ...
            timeoutSeconds=600, residentGuardBytes=(12 if index <= 37 else 8)*2**30, selectionDeadlineUnix=deadline)
    ...
    selection = dict(schema='ntsd-host-loading-test-commands-v1', cases=cases, packageCheck=pin(artifact/'package-check1.json'),
        ...
        runner=pin(ROOT/'tools/run_application_host_loading_validation5.py'), completeResultParser='same exact named XCTest regex and one-test zero-failure summary as Catalog53 completion')
    ...
    try:
        for case in cases:
            ...
            job.update(status='terminal', exitCode=0, all8RemainingPassed=True)
