#!/usr/bin/env python3
"""Prepare one isolated retained-context candidate using existing pin/APFS clone procedure."""
...
B=(R/'build/research/application-catalog-package-validation-20260926').resolve()
P=B.parent/'application-host-delivery-context-20260926'
PLAN=R/'docs/research/APPLICATION_HOST_DELIVERY_CONTEXT_PLAN.md'
...
def main():
 ...
 pub=read(B/'publication1.json');assert pub['status']=='all76-passed' and read(B/'external-close1.json')['taskFrozen']
 ...
 inputs=read(B/'candidate1-inputs.json');assert len(inputs)==2241
 ...
 try:
  ...
  save('read-diagnostics1.json',dict(UTC=now(),errors=['cat: docs/research/APPLICATION_HOST_SESSION.md: No such file or directory','sed: tools/prepare_application_catalog_package.py: No such file or directory'],resolution='Read actual Host transaction study and existing preparation helpers found by rg --files; no execution retry or source operation',relevantOperation='pre-plan read-only local file discovery'))
  job.update(status='terminal',exitCode=0,files=2241,bytes=sum(r['bytes'] for r in inputs),fullCloneVerified=True,distinctRegularInodes=True)
