#!/usr/bin/env python3
"""Prepare exact window-exchange validation with the existing guarded host tools."""
...
NAME='application-window-exchange-validation-20260926'
TASK=Path('/Volumes/X5/ntsd-2.4-research/01a0dc49-738f-7972-8fb0-e98fb2f34408')/NAME
BASE=(ROOT/'build/research/application-window-exchange-20260926').resolve()
OLD=(ROOT/'build/research/application-host-delivery-context-validation-20260926').resolve()
PLAN=ROOT/'docs/research/APPLICATION_WINDOW_EXCHANGE_VALIDATION_PLAN.md'
...
def generate(name):
 ...
 if name==NAMES[0]:
  new=once(new,"r'build1|test-(0[1-9]|[1-7][0-9]|8[0-2])'","r'build1|test-(0[1-9]|10)'")
  marker='    def identity(pid):';assert old[old.index(marker):]==new[new.index(marker):]
 elif name==NAMES[1]:
  new=once(new,'82-method','10-method');new=once(new,'== 82','== 10');new=once(new,'all82Passed','all10Passed')
  begin,end='            done = re.findall(',"            job['completed'].append("
  assert old[old.index(begin):old.index(end)]==new[new.index(begin):new.index(end)]
 else:
  new=once(new,"'NTSDCore':192","'NTSDCore':193")
  new=once(new,"'NTSDCoreTests':272","'NTSDCoreTests':273")
...
def main():
 ...
 for parent,names in [(BASE,['prepare1.job.json','parse1.job.json','finalize1.job.json']),(OLD,['tests1-queue.job.json','finalize1.job.json'])]:
  for n in names:
 ...
 try:
  ...
  rows=read(BASE/'candidate1-inputs.json');assert len(rows)==2244
  ...
  assert len(methods)==len(set(methods))==10 and len(new)==5 and len(old)==5 and methods==new+old
  previous=read(OLD/'method-limits1.json')['methods'];inherited=[m for m in old if m in previous]
  assert len(inherited)==2 and all(m.startswith('OriginalWinMainStartupTests/') for m in inherited)
  limits={}
  for i,m in enumerate(methods,1):
   ...
   limits[m]=previous[m] if m in inherited else dict(seconds=600 if i in [1,2,3,6] else 300,residentBytes=(8 if i in [1,2,3,6] else 4)*2**30,priorOrdinal=None)
  assert all(limits[m]==previous[m] for m in inherited)
  save('method-limits1.json',dict(methods=limits,queueSeconds=14400,inheritedMethods=2,priorLimits=record(OLD/'method-limits1.json'),priorSelection=record(OLD/'selected-methods1.json'),preceding82ComparisonIsSeparate=True))
  ...
  pattern=r'build1|test-(0[1-9]|10)'
  assert all(re.fullmatch(pattern,p) for p in ['build1']+[f'test-{i:02d}' for i in range(1,11)])
  invalid=['build0','build2','test-00','test-11','test-99','test-1','test-01x'];assert not any(re.fullmatch(pattern,p) for p in invalid)
  save('runner-controls1.json',dict(UTC=now(),monitorBodyUnchanged=True,resultPredicatesUnchanged=True,cwdPositive=1,cwdNegative=4,compilerPositive=1,compilerNegative=2,transientPositive=1,transientNegative=1,phasePositive=11,phaseNegative=7,exactMethods=10,oldWinMainLimitsUnchanged=True,nativeExecuted=False,originalExecuted=False,independentReview=False))
  job.update(status='terminal',exitCode=0,files=2244,preparedMethods=10,buildConfig=record(TASK/'build1-config.json'))
