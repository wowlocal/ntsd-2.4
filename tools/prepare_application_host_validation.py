...
BASE = ROOT/'build/research/application-host-transaction-candidate-20260922'
TASK = Path('/Volumes/X5/ntsd-2.4-research/01a0c7b2-a956-7760-950e-ecf7dceaaa20/application-host-transaction-validation-20260922')
PLAN = ROOT/'docs/research/APPLICATION_HOST_TRANSACTION_VALIDATION_PLAN.md'
...
def main():
    ...
    job = dict(schema='ntsd-host-validation-prepare-job-v1', status='running', pid=os.getpid(),
    ...
    try:
        ...
        protected_paths = [PLAN, Path(__file__), ROOT/'tools/prepare_application_host_candidate.py',
            ROOT/'tools/run_catalog53_validation.py', BASE/'publication1.json', BASE/'external-close1.json',
        ...
        context = dict(schema='ntsd-host-validation-context-v1', UTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),
        ...
        command = [swift, '--package-path', str(TASK/'candidate1/native'), '--scratch-path', str(TASK/'swift'),
            ...
            '--jobs', '2', '--disable-index-store']
        config = dict(schema='ntsd-host-validation-run-config-v1', phase='build1', kind='build', command=command,
