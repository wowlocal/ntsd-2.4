...
def main(P):
    assert P.name=='application-host-loading-completion-20260922' and not (P/'external-close1.json').exists()
    ...
    try:
        ...
        diagnostic=read(P/'diagnostic1.job.json')
        assert diagnostic['status']=='terminal' and diagnostic['exitCode']==0 and absent(diagnostic['pid'])
        ...
        prior=[result(A,r,True) for r in old['completed'][:27]]
        assert [r['method'] for r in prior]==methods[:27]
        fresh=[result(P,r) for r in q['completed']]
        assert [r['method'] for r in fresh]==methods[27:27+len(fresh)] and len(fresh)<=15
        complete=len(fresh)==15 and all(r['passed'] for r in fresh)
        assert complete==bool(q.get('all15RemainingPassed')) and q['exitCode']==(0 if complete else 1)
        ...
        prior_guard=read(A/'test-28.job.json');assert prior_guard['exitCode']==-15 and prior_guard['guardReason']=='resident bound'
        ...
        verification=dict(schema='ntsd-host-loading-completion-verification-v1',UTC=now(),all42Passed=complete,verifiedMethods=len(passed),remaining=remaining,
            priorPasses=prior,newResults=fresh,oldGuard=record(A/'test-28.job.json'),oldGuardPreserved=True,
        ...
        names=['prepare_application_host_loading_completion.py','run_application_host_loading_validation3.py','run_application_host_loading_tests3.py',Path(__file__).name]
        ...
        publication=dict(schema='ntsd-host-loading-completion-v1',UTC=now(),status='all42-passed-review-open' if complete else 'partial-validation-open',
            ...
            diagnostic=record(P/'formatting-diagnosis1.json'),priorPublication=record(A/'publication1.json'),candidateManifest=record(A/'candidate1-inputs.json'),binary=package['binary'],
            ...
            limitations=['Round1 comparator correction has not received independent review.','Old assertion failure and old resident-guard termination remain immutable outcomes.',
        ...
        target=ROOT/'docs/evidence/application-host-loading-completion.json';assert not target.exists();target.write_bytes((P/'publication1.json').read_bytes())
        ...
        save(P/'external-close1.json',closure);target=ROOT/'docs/evidence/application-host-loading-completion-close.json';assert not target.exists();target.write_bytes((P/'external-close1.json').read_bytes())
