...
BASE = ROOT/'build/research/application-host-match-validation-20260922'
TASK = Path('/Volumes/X5/ntsd-2.4-research/01a0c7b2-a956-7760-950e-ecf7dceaaa20/application-host-match-correction1-20260922')
PLAN = ROOT/'docs/research/APPLICATION_HOST_MATCH_CORRECTION1_PLAN.md'
...
def main():
    ...
    for key in ['context', 'candidateManifest', 'verification', 'metadataArchive', 'failureDiagnosis']:
    ...
    try:
        ...
        changed = 'native/Sources/NTSDCore/OriginalApplicationHostSession.swift'
        f = TASK/'candidate1'/changed
        before = f.read_text();after = before
        edits = [
            ('guard let prepared,case .returned(let child,let platform) = prepared',
             'guard let prepared,case .returned(let child,let preparedPlatform) = prepared'),
            ('var candidate = try Self.copy(platform), next = application',
             'var candidate = try Self.copy(preparedPlatform), next = application'),
            ('application = next;platform = candidate;self.prepared = nil;self.pending = nil',
        ...
        corrected = []
        for item in candidate:
            v = pin(TASK/'candidate1'/item['path'])
            if item['path'] != changed:same(TASK/'candidate1'/item['path'],item)
            corrected.append(dict(path=item['path'],**{k:v[k] for k in ('bytes','sha256','mode')}))
        assert sum(x['sha256'] != y['sha256'] for x,y in zip(candidate,corrected)) == 1
        (TASK/'candidate1-inputs.json').write_text(json.dumps(corrected,indent=2)+'\n')
        roundtrip = TASK/'patch-roundtrip1';dest = roundtrip/changed
        dest.parent.mkdir(parents=True)
        shutil.copy2(BASE/'candidate1'/changed,dest)
        subprocess.run(['git','apply','--check',str(TASK/'correction1.patch')],cwd=roundtrip,check=True)
        subprocess.run(['git','apply',str(TASK/'correction1.patch')],cwd=roundtrip,check=True)
        assert dest.read_bytes() == f.read_bytes()
        (TASK/'patch-roundtrip1.json').write_text(json.dumps(dict(before=pin(BASE/'candidate1'/changed),after=pin(f),patch=pin(TASK/'correction1.patch'),onlyChangedFile=changed,exactRoundTrip=True),indent=2)+'\n')
        ...
        protected_paths = [PLAN, Path(__file__), ROOT/'tools/prepare_application_host_candidate.py',
            ROOT/'tools/run_application_host_match_validation2.py', BASE/'publication1.json', BASE/'external-close1.json',
            ...
            ROOT/'tools/run_application_host_match_tests2.py', ROOT/'tools/verify_application_host_match_package2.py',
            ROOT/'docs/research/APPLICATION_HOST_MATCH_CANDIDATE.md', BASE/'build1.log', BASE/'build1.job.json', BASE/'failure-diagnosis1.json',
            ROOT/'docs/research/APPLICATION_HOST_MATCH_VALIDATION.md',
        ...
        context = dict(schema='ntsd-host-match-validation-context-v1', UTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),
            ...
            correctionRound=1,changedPath=changed,beforeManifest=pin(TASK/'candidate-before1-inputs.json'),correctionPatch=pin(TASK/'correction1.patch'),
