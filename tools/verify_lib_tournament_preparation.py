...
LIB_TOURNAMENT_PREPARATION_PLAN.md defines finite acceptance and open boundaries.
"""
...
def verify(path):
 ...
 assert len(d['cases'])==len(d['installations'])==98
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-tournament-preparation-finite-inputs1.json').read_bytes())
 ...
 for c,install in zip(d['cases'],d['installations']):
  ...
  if c['spec'].get('chain'):
   ...
   for change in c['stimulus']+c['callerABI']:
  ...
  for ei,e in enumerate(c['events']):
   ...
   if e['kind']=='calloc':
    p,n,size=e['arguments'];assert p==0x75000020 and n==1 and size==0x630e18 and p not in state
  ...
  if c['preparationGraphics'] is not None:
   assert 0x75000020 in state and len(state[0x75000020][0])==0x630e18 and all(state[0x75000020][1]);stats['wholePreparations']+=1;stats['recordingBytes']+=0x630e18
   ...
   for r in pg['records']:
    assert state[r['address']]==(blobs[r['bytes']],blobs[r['mask']]) and live[r['address']]
 ...
 assert stats['wholePreparations']==21 and stats['constructorReturns']==21*401 and all(streams[s]>0 for s in (0xfd,0xfe,0xff))
