...
def main(task, build_phase):
    ...
    assert {k:v['originalSources'] for k,v in targets.items()} == {'NTSDCore':191,'NTSDReferenceChecks':61,'NTSDCoreTests':270}
    ...
    assert all(value not in linked for value in ['first-damage-catalog53', 'application-host-transaction-candidate-20260922', 'application-host-loading-candidate-20260922', 'application-host-match-candidate-20260922'])
    report = dict(schema='ntsd-host-gameplay-native-package-check-v1', UTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),
