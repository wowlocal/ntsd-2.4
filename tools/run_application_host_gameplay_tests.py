#!/usr/bin/env python3
"""Run the frozen 69-method host selection through the existing guarded runner."""
import datetime
import json
import os
...
from run_application_host_gameplay_validation import main as run_one
...
def main(task):
    ...
    template = json.loads((task/'build1-config.json').read_text())
    ...
    for index, method in enumerate(methods, 1):
        ...
        config = dict(template, phase=phase, kind='test',
            ...
            timeoutSeconds=1800 if index >= 64 else 900 if index >= 51 else 600,
            residentGuardBytes=(16 if index >= 51 else 12 if index in (26,28) or 35 <= index <= 37 or 43 <= index <= 50 else 8)*2**30, selectionDeadlineUnix=deadline)
        path = task/(phase+'-config.json')
        assert not path.exists()
        path.write_text(json.dumps(config, indent=2)+'\n')
        cases.append(dict(method=method, phase=phase, config=str(path), pin=pin(path)))
    selection = dict(schema='ntsd-host-gameplay-test-commands-v1', cases=cases, packageCheck=pin(task/'package-check1.json'),
        selectedMethods=pin(task/'selected-methods1.json'), queueProducer=pin(Path(__file__)),
        runner=pin(ROOT/'tools/run_application_host_gameplay_validation.py'), completeResultParser='same exact named XCTest regex and one-test zero-failure summary as Catalog53 completion')
    save(task/'tests1-commands.json', selection)
    shutil.copy2(Path(__file__), task/'tests1-queue.py')
    started = time.monotonic()
    job = dict(schema='ntsd-host-gameplay-test-queue-v1', pid=os.getpid(), status='running',
    ...
    try:
        for case in cases:
            ...
            job.update(status='terminal', exitCode=0, all69Passed=True)
