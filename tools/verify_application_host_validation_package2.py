...
def main(task, build_phase):
    ...
    first_start = datetime.datetime.fromisoformat(json.loads((task/'build1.job.json').read_text())['startedUTC']).timestamp()
    retained = {r['path']: r for r in json.loads((task/'build1-preservation.json').read_text())['files']}
    ...
    for name, source_dir in [('NTSDCore', 'native/Sources/NTSDCore'),
        ...
        for value in command['objects']+[command['moduleOutputPath']]:
            f = Path(value)
            assert f.is_file() and not f.is_symlink() and f.stat().st_size > 0, str(f)
            if f.stat().st_mtime < start:
                # Byte-identical objects from this task's first fresh build of
                # the same unchanged candidate; never an unrelated old cache.
                assert name == 'NTSDCore' and f.suffix == '.o' and f.stat().st_mtime >= first_start, str(f)
                prior = retained[str(f.relative_to(task))]
                checked(f, prior); checked(task/'build1-retained'/prior['path'], prior)
                retained_outputs.append(str(f))
        ...
        targets[name] = dict(originalSources=len(original_sources), generatedSources=sorted(generated),
                             command=command, outputsFromThisFreshTask=outputs, retainedIdenticalBuild1Objects=retained_outputs)
    ...
    assert 'first-damage-catalog53' not in linked and 'application-host-transaction-candidate-20260922' not in linked
    report = dict(schema='ntsd-host-native-package-check-v1', UTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),
