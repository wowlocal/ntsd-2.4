...
def main(task):
    assert task.name == 'application-host-match-correction1-20260922'
    ...
    job = dict(schema='ntsd-host-match-correction1-finalizer-v1', pid=os.getpid(), status='running', startedUTC=now(),
    ...
    try:
        ...
        assert len(selected) == len(set(selected)) == 53 and 1 <= count <= 53
        assert selected[:count] == [r['method'] for r in queue['completed']]
        all_passed = count == 53 and all(r['passed'] for r in queue['completed'])
        assert queue['exitCode'] == (0 if all_passed else 1)
        assert bool(queue.get('all53Passed')) == all_passed
        ...
        assert all(not (task/f'test-{i:02d}.job.json').exists() for i in range(count+1,54))
        tests = []
        ...
        previous = read(task/'candidate-before1-inputs.json')
        checked(task/'candidate-before1-inputs.json', context['beforeManifest'])
        checked(task/'correction1.patch', context['correctionPatch'])
        assert len(previous) == len(candidate) == 1039
        changes = []
        for before, after in zip(previous, candidate):
            assert before['path'] == after['path']
            checked(Path(context['candidateSource'])/before['path'], before)
            if before != after: changes.append(before['path'])
        assert changes == [context['changedPath']] and context['correctionRound'] == 1
        proof = read(task/'patch-roundtrip1.json')
        assert proof['exactRoundTrip'] and [proof['onlyChangedFile']] == changes
        for key in ('before','after','patch'):checked(Path(proof[key]['path']),proof[key])
        root_patch = ROOT/'docs/evidence/application-host-match-correction1.patch'
        assert not root_patch.exists()
        root_patch.write_bytes((task/'correction1.patch').read_bytes())
        package = read(task/'package-check1.json')
        assert package['allInputBytesVerified'] and package['sourceMembershipVerified'] and package['candidateFiles'] == 1039
        ...
        artifact_report = dict(schema='ntsd-host-match-correction1-artifact-archive-v1', UTC=now(), files=files,
        ...
        write(task/'verification1.json', dict(UTC=now(), protectedPins=len(context['protected']), rootNativeFiles=1034,
            candidateFiles=1039, sourceCodePins=55, allPreserved=True, resourceFiles=487,
            all53Passed=all_passed, passedMethods=passed_count, guardedMethods=guarded_count, failedTestMethods=sum(t['outcome'] == 'test-failure' for t in tests), unstartedMethods=selected[count:], tests=tests,
        ...
        tools = ['prepare_application_host_match_validation2.py', 'run_application_host_match_validation2.py',
                 'verify_application_host_match_package2.py', 'run_application_host_match_tests2.py', Path(__file__).name]
        for name in tools:
        ...
        publication = dict(schema='ntsd-host-match-correction1-v1', UTC=now(), status='all53-passed' if all_passed else 'first-nonpass-retained',
            ...
            corrections=[dict(round=1, changedFiles=changes, patch=dict(path=str(task/'correction1.patch'), **pin(task/'correction1.patch')), roundtrip=pin(task/'patch-roundtrip1.json'), previousFailure='docs/evidence/application-host-match-validation.json', correction='Prepared platform binding renamed; final commit explicitly installs self.platform; tests and expected unchanged', expectedSourceUnchanged=True, independentReview=False)],
            failureDiagnosis=None if all_passed else dict(path=str(task/'failure-diagnosis1.json'), **pin(task/'failure-diagnosis1.json')),
            gates=dict(nativeComparison=all_passed, packageBytes=True, archive=True, independentReview=False,
            ...
            limitations=['Short test processes can finish before any RSS sample; recorded0 is not known zero RSS.',
                         ...
                         'First gameplay input inspection is rolled back at its declared stop; retained gameplay, actual IO, full match and full game remain open.'])
        write(task/'publication1.json', publication)
        (ROOT/'docs/evidence/application-host-match-correction1.json').write_bytes((task/'publication1.json').read_bytes())
        ...
        closure = dict(schema='ntsd-host-match-correction1-close-v1', UTC=now(), largePhaseClosed=True,
        ...
        (ROOT/'docs/evidence/application-host-match-correction1-close.json').write_bytes((task/'external-close1.json').read_bytes())
        job.update(status='terminal', exitCode=0)
        print(json.dumps(dict(all53Passed=all_passed, passed=passed_count, guarded=guarded_count, unstarted=53-count, testSeconds=publication['tests']['totalTestProcessSeconds'],
