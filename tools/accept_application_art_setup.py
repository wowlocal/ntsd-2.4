#!/usr/bin/env python3
"""Publish whole43e8e0 evidence after verified isolated native raw acceptance.
...
from verify_application_art_setup import validate,digest
...
def main():
    ...
    work=json.load(open(ROOT/'build/research/application-art-setup-work.json'))
    ...
    text=log.read_text();assert re.search(r'Executed 5 tests, with 0 failures',text)
    assert "Test Suite 'OriginalApplicationArtSetupTests' passed" in text and "Test Suite 'OriginalApplicationDispatchPrefixTests' passed" in text
    # The just-tested isolated overlay and frozen original producer must match.
    package=Path(a.package_path)
    for f in ('Sources/NTSDCore/OriginalApplicationArtSetup.swift','Tests/NTSDCoreTests/OriginalApplicationArtSetupTests.swift'):
        assert (ROOT/'native'/f).read_bytes()==(package/f).read_bytes()
    assert (ROOT/'tools/oracle_application_art_setup.py').read_bytes()==path.with_name(path.stem+'-source.py').read_bytes()
    ...
    fixture=fixtures/'original-application-art-setup.json'
    ...
    result.update(exeSHA256=EXE_SHA256,nativeCompared=True,fixture=fixture.name,fixtureBytes=len(packed),fixtureSHA256=digest(packed),
        rawCorpus=str(path.relative_to(ROOT)) if path.is_absolute() else str(path),producerSHA256=digest((ROOT/'tools/oracle_application_art_setup.py').read_bytes()),
        nativeScope='231 whole controlled helper returns, live callback targets, retained locals and full globals; no initialized caller stack or full dispatcher.',
        acceptanceLog=str(log),sourceCallbacks='Declared COM/OutputDebugStringA; no Windows/device execution')
    (ROOT/'docs/evidence/application-art-setup.json').write_text(json.dumps(result,indent=2)+'\n')
    current={f.name:digest(f.read_bytes()) for f in sorted(fixtures.glob('*.json'))};assert len(current)==237
    (ROOT/'build/research/application-art-setup-fixture-pins.json').write_text(json.dumps(current,indent=2)+'\n')
    print('Published',fixture.name,len(packed),'bytes; all236 prior pins unchanged')
