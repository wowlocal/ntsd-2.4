#!/usr/bin/env python3
"""Read-only full trace audit of Team Tournament setup/bracket/paired CPU decisions/four-seat assignment/result/winner output and ret4 or preparation boundaries.
...
See docs/research/LIB_TEAM_TOURNAMENT_BRACKET_PLAN.md for finite inputs and open boundaries.
"""
...
def verify(path):
 ...
 previous=None
 assert len(d['cases'])==len(d['installations'])==377
 assert [c['spec'] for c in d['cases']]==json.loads((path.parent/'lib-team-tournament-bracket-finite-inputs1.json').read_bytes())
 for index,(c,install) in enumerate(zip(d['cases'],d['installations'])):
  ...
  previous=c
  assert c['cw']==0x23f and c['end'] in ('returned','teamTournamentMatchPreparation')
  ...
  def lifetime(prefix):
   ...
   for event in prefix:
    ...
    elif event['kind']=='allocate':
     e=next(e for e in c['bodyMusic'] if e['kind']=='allocate');p=e['response']['pointer'] or 0
  ...
  for event in c['events']:
   if event['kind']=='fill':
    ...
    assert f['effects'][:4]==[100,0,0,0] and f['effects'][0x50:0x54] in ([0x65,0x25,0x12,0],[255,255,255,0],[0,0,0,0],[0x9a,0x4d,0x32,0],[0x67,0x2f,0x19,0],[0xa7,0xa7,0xff,0])
  for x in c['after']:
  ...
  if c['end']=='returned':
   ...
   assert c['instructions']['0x436fb5']=='c20c00'
   assert sum(h['entry']==0x434ab0 and h['pop']==12 for h in c['helpers'])==1
  else:
   assert c['end']=='teamTournamentMatchPreparation' and c['endPC']==0x436747 and c['endSP']==c['characterSP']-0xa18
   assert [h['entry'] for h in c['pending']]==[0x429730,0x434ab0] and all(h['pop']==12 for h in c['pending'])
   assert c['points'][-1]['kind']=='team-tournament-0x436747' and '0x436747' not in c['instructions']
  if True:
   points=[p for p in c['points'] if p['kind'].startswith(('character-','team-tournament-'))]
   assert [p['kind'] for p in points[:3]]==['team-tournament-0x434af2','team-tournament-0x434c1f','team-tournament-0x434cb6']
   for point in points:
    assert point['sp']==c['characterSP']-(0xa18 if point['kind'].startswith('team-tournament-') else 0);stack=decode_record(next(x for x in point['records'] if x['address']==d['stackAddress']))
 ...
 assert ends=={'returned':375,'teamTournamentMatchPreparation':2}
 streams=collections.Counter();controller_labels=collections.Counter();body_music=collections.Counter()
 for c in d['cases']:
  before={x['address']:decode_record(x) for x in c['before']};after={x['address']:decode_record(x) for x in c['after']}
  before_g=before[0x44d000][0];g=after[0x44d000][0]
  word=lambda p:struct.unpack_from('<i',g,p-0x44d000)[0]
  assert word(0x451160)==3
  lo,hi=0x44d120-0x44d000,0x44d318-0x44d000
  assert before_g[lo:hi]==g[lo:hi]==exe_bytes(0x44d120,0x1f8)
  assert before[0x44d000][1][lo:hi]==after[0x44d000][1][lo:hi]==b'\1'*0x1f8
  for h in c['helpers']:
   if h['entry']==0x417170:
    assert h['stream'] in (1,0x101,0x102,0x103,0x105,0x106,0x107,0x108,0x10c);streams[h['stream']]+=1
    ...
    if h['stream']==0x103:
     ...
     e=c['events'][h['eventStart']-1];assert e['kind']=='candidates' and e['arguments'][1:]==expected and h['range']==len(expected);stats['liveCandidateLists']+=1
    if h['stream'] in (0x105,0x106,0x10c):assert h['range']==2
    if h['stream'] in (0x107,0x108):
     difficulty=struct.unpack_from('<i',live,0x450c30-0x44d000)[0];assert h['range']==15+18*difficulty
    index=(h['index']+1)%3000;counter=(h['counter']+1)%1234
    assert h['result']==(live[0x44ff90-0x44d000+index]+counter)%h['range']
    assert c['events'][h['eventEnd']]==dict(kind='random',arguments=[h['stream'],h['range'],h['result'],h['index'],h['counter'],index,counter],strings=[])
   if h['entry']==0x401290 and h['textAddress']==0x44d320:
    assert len(h['text'])==1 and h['text'][0] in [67,*range(49,57)]
    writes=[w for w in c['writes'][:h['firstStore']] if w['address']==0x44d320]
    assert writes and bytes.fromhex(writes[-1]['bytes'])==bytes(h['text']) and g[0x44d321-0x44d000]==0
    controller_labels[chr(h['text'][0])]+=1
  for e in c['bodyMusic']:
   body_music[e['kind']]+=1
   assert dict(kind=e['kind'],arguments=e['arguments'],strings=e['strings']) in c['events']
   if e['kind']=='allocate':
    assert e['arguments']==[26] and e['response']['pointer']==0x2c010020
    assert e['response']['bytes']==(list(range(26)) if c['spec']['control'] else [0xa5]*26)
   if e['kind']=='convert':
    expected=[v for b in e['strings'][0]+[0] for v in (b,0)]
    assert e['response']['bytes']==expected and e['response']['result']==len(e['strings'][0])+1
  if c['spec'].get('chain'):
   history=[v for v in c['spec']['buttons'] if v[1]==0xca];assert len(history)==8
   for seat,off,value in history:
    p=c['actorAddresses'][seat];assert before[p][0][off]==value and before[p][1][off]==1
  if c['spec']['label'] in ('prepare-match-human','prepare-match-mixed'):assert word(0x44d020)==127 and word(0x451404)==5
  if c['spec']['label']=='winner-dismiss-cpu':assert word(0x44d020)==10
  if c['spec']['label'].startswith('controlled-result'):
   assert [word(0x44d080+i*4) for i in range(4)]==[160,280,119,198]
   assert [word(0x44d0a0+i*4) for i in range(4)]==[4,5,6,7]
   assert all(word(0x4512d0+i*4)==-1 for i in range(20))
  if c['spec']['label']=='controlled-held-attack':
   held=[x for x in c['reads'] if x['pc']==0x435f2b];assert len(held)==1 and held[0]['bytes']=='01' and held[0]['known']==[1]
   assert word(0x44d020)==127 and word(0x451404)==0 and c['end']=='returned';stats['heldAttackReads']+=1
 stats['controllerLabelCalls']=sum(controller_labels.values());stats['bodyMusicEvents']=sum(body_music.values())
 assert stats['controllerLabelCalls']>0 and all(controller_labels[str(i)]>0 for i in range(1,9)) and controller_labels['C']>0
 assert all(streams[i]>0 for i in (0x105,0x106,0x107,0x108,0x10c))
 assert body_music['allocate']==body_music['convert']==3 and stats['heldAttackReads']==1


 # Derive each paired CPU outcome from declared participants, catalog IDs and
 # actual RNG returns. Neither expected winner nor HP is taken from after-state.
 def i32(n):return (n+0x80000000)%0x100000000-0x80000000
 def trunc(n,d):return (abs(n)//abs(d))*(-1 if (n<0)!=(d<0) else 1)
 controlled=set()
 for c in d['cases']:
  before={x['address']:decode_record(x)[0] for x in c['before']};after={x['address']:decode_record(x)[0] for x in c['after']}
  bg,ag=before[0x44d000],after[0x44d000]
  get=lambda g,p:struct.unpack_from('<i',g,p-0x44d000)[0]
  vector=lambda g,p:[get(g,p+i*4) for i in range(8)]
  randoms=[h for h in c['helpers'] if h['entry']==0x417170 and h['stream'] in (0x105,0x106,0x107,0x108)]
  if randoms:
   level=get(bg,0x44d318);members=[]
   levels=vector(bg,0x451384);hp=vector(bg,0x44d080)
   for i in range(0,8,2):
    if levels[i]==level:members += [i,i+1]
    if len(members)==4:break
   assert len(members)==4
   order=vector(bg,0x44d0e0);selected=vector(bg,0x44d0c0)
   ids=[d['roster']['entries'][selected[order[i]]]['id'] for i in members]
   flags=[trunc(n,10)==5 for n in ids];a,b=any(flags[:2]),any(flags[2:])
   if a==b:
    assert [h['stream'] for h in randoms]==[0x106 if a else 0x105,0x107,0x108]
    winner=randoms[0]['result']*2
   else:
    assert [h['stream'] for h in randoms]==[0x107,0x108];winner=0 if a else 2
   winning=members[winner:winner+2];losing=members[2-winner:4-winner]
   for i in winning:levels[i]=i32(levels[i]+2)
   for i in losing:levels[i]=i32(levels[i]+1)
   for i in winning:
    if levels[i]==6:levels[i]=7
   difficulty=get(bg,0x450c30);stores=[]
   for j,(index,h) in enumerate(zip(winning,randoms[-2:])):
    assert h['range']==15+18*difficulty
    hp[index]=trunc(i32(i32(h['result']+85-difficulty*18)*hp[index]),100)
    matches=[(n,w) for n,w in enumerate(c['writes']) if w['pc']==[0x4363a0,0x4363dd][j]]
    assert len(matches)==1
    n,w=matches[0];assert w['address']==0x44d080+index*4 and bytes.fromhex(w['bytes'])==struct.pack('<i',hp[index]) and n>=h['lastStore']
    stores.append(n)
   assert stores[0]<randoms[-1]['firstStore']<=stores[1]
   assert vector(ag,0x451384)==levels and vector(ag,0x44d080)==hp
   stats['pairedCPUMatches']+=1;stats['orderedCPUHealthStores']+=2
   if c['spec']['label'].startswith('controlled-cpu-mask'):
    mask=sum(int(flag)<<i for i,flag in enumerate(flags));controlled.add((mask,difficulty))
  if c['spec']['label'].startswith('controlled-result'):
   levels=vector(bg,0x451384);hp=vector(bg,0x44d080);states=vector(bg,0x44d0a0);result=get(bg,0x450bf8)
   for slot in range(20):
    index=get(bg,0x4512d0+slot*4)
    if index<0:continue
    p=c['actorAddresses'][slot];current,maximum=struct.unpack_from('<ii',before[p],0x2fc)
    expected=trunc(maximum,2) if current<=0 else maximum
    assert struct.unpack_from('<i',after[p],0x300)[0]==expected
    hp[index]=expected;states[index]=struct.unpack_from('<i',before[p],0x33c)[0]
    if current<=0:stats['defeatedActorHealthHalves']+=1
    if struct.unpack_from('<i',before[p],0x364)[0]==result:
     levels[index]+=1
     if levels[index]==6:levels[index]=7
    stats['participantResultStores']+=1
   if result==-1:
    draws=[h for h in c['helpers'] if h['entry']==0x417170 and h['stream']==0x10c];assert len(draws)==1 and draws[0]['range']==2
    for j in range(draws[0]['result']*2,draws[0]['result']*2+2):
     index=get(bg,0x4513ec+j*4);levels[index]+=1
     if levels[index]==6:levels[index]=7
   assert vector(ag,0x451384)==levels and vector(ag,0x44d080)==hp and vector(ag,0x44d0a0)==states
   assert get(ag,0x44d020)==126 and all(get(ag,0x4512d0+i*4)==-1 for i in range(20))
   assert after[d['worldAddress']][4:404]==bytes(400)
   stats['controlledResultCalls']+=1
 assert controlled=={(mask,difficulty) for mask in range(16) for difficulty in range(3)}
 assert stats['controlledResultCalls']==3 and stats['participantResultStores']==12 and stats['defeatedActorHealthHalves']==6
 stats['controlledCPUConfigurations']=len(controlled)

 return dict(scope=__doc__,bodyMusic=dict(body_music),controllerLabels=dict(controller_labels),randomStreams={hex(k):v for k,v in streams.items()},undefinedBitmapReads=[dict(pc=hex(pc),bytes=n,reads=count) for (pc,n),count in bitmap_unknown.items()],rawBytes=len(input_raw),rawSHA256=sha(input_raw),cases=len(d['cases']),ends=dict(ends),**stats,events=dict(events),instructionStarts=len(pcs),exeInstructionStarts=sum(int(p,16)<0x36000000 for p in pcs),libraryInstructionStarts=sum(0x36000000<=int(p,16)<0x36005000 for p in pcs),crtInstructionStarts=sum(int(p,16)>=0x78130000 for p in pcs),blobCount=len(blobs),decodedBlobBytes=sum(map(len,blobs.values())),writeMaskFalseReadsWithPinnedEXEProvenance=[dict(pc=hex(pc),address=hex(p),bytes=n,reads=count) for (pc,p,n),count in unknown.items()],nativeCompared=False,windowsVerified=False)
